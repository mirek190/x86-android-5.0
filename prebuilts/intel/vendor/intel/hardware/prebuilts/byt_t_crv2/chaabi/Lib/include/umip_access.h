...
//
// Maximum Android Customer Data field data size in bytes. The SEP message only
// allows a maximum of 4,096 bytes in the SEP message. This means that the
// maximum number of bytes for the ACD data is this limit minus the bytes needed
// for the ACD parameters in the SEP message. This value must be a multiple of
// four bytes because data in the UMIP must be four-byte aligned.
//
#define SEP_MSG_MAX_SIZE_IN_BYTES 4096
#define ACD_MAX_DATA_SIZE_IN_BYTES ( SEP_MSG_MAX_SIZE_IN_BYTES - \
                                     ( ACD_NUM_SEP_MSG_INDICES * 4))
