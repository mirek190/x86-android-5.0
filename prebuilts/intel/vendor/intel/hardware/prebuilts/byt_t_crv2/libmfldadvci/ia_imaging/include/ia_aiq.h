...
typedef struct
{
    ...
    ia_aiq_rect *focus_rect;                                    /*!< Optional. */
...
typedef struct
{
    ...
    ia_coordinate *manual_white_coordinate;           /*!< Optional. Manual white point coordinate. Used only if AWB scene mode 'ia_aiq_awb_operation_manual_white' is used. */
...
typedef struct
{
    ...
    float *color_gains;                              /*!< Optional. RGB gains for each color channels (4) in order Gr, R, B, Gb. These gains will be applied on top of AWB results. */
} ia_aiq_pa_input_params;

/*!
 * \brief Parameter adaptor calculations for the next frame.
 * Compute generic parameters (Lens Shading Correction parameters, Color Correction Matrix and Black Level Correction),
