...
/*!
 * Needs to be agreed with ia_imaging.
 *  \brief Describes a rectangle.
 */
typedef struct {
    unsigned int left;     /*!< Left coordinate*/
    unsigned int top;      /*!< Top coordinate */
 ...
 * This structure is filled
 */
typedef struct
{
    unsigned short width;   /*!< Width of the weight grid. */
    unsigned short height;  /*!< Height of the weight grid. */
    unsigned char *weights; /*!< Multipliers (weight) of RGB values in the grid. */
} ia_aiq_hist_weight_grid;

/*!
 * \brief Histogram of a certain ROI.
 */
typedef struct
{
    ia_aiq_rect roi;                  /*!< ROI of the histogram. */
...
/*!
 * \brief Grid block
 * As defined in the iaaiq statistics specification.
 */
typedef struct
{
...
typedef enum
{
    ...
    ia_aiq_awb_operation_mode_daylight,
    ia_aiq_awb_operation_mode_partly_overcast,
    ia_aiq_awb_operation_mode_fully_overcast,
    ia_aiq_awb_operation_mode_fluorescent,
    ia_aiq_awb_operation_mode_incandescent,
    ia_aiq_awb_operation_mode_sunset,
    ia_aiq_awb_operation_mode_video_conference,
    ia_aiq_awb_operation_mode_manual_cct_range,
    ia_aiq_awb_operation_mode_manual_white,
...
/*!
 * \brief Frame parameters which describe cropping and scaling (need to be filled by Camera HAL for every frame)
 */
typedef struct
{
...
typedef struct {
    ...
    unsigned short* ctc_gains_lut;
    unsigned int gamma_lut_size;
    unsigned int ctc_gains_lut_size;
} ia_aiq_gbce_results;

/*!
 * \brief Results from PA.
 */
typedef struct {
    cmc_parsed_lens_shading_t *weighted_lsc;          /*!< Pointer to the LSC table.*/
    float *color_conversion_matrix;                   /*!< Pointer to the CC matrix.*/
    color_channels_t *black_level_coef;               /*!< Pointer to Bayer quad of of black level coefficients.*/
    float *color_gains;                               /*!< Pointer to RGB gains for each color channels (4) in order Gr, R, B, Gb. These gains will be applied on top of AWB results. */
