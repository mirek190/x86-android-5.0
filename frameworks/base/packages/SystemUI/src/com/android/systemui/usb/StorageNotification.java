...
public class StorageNotification extends SystemUI {
    ...
    private void onStorageStateChangedAsync(String path, String oldState, String newState) {
        ...
        String storageDes = "Removable Storage";
        ...
        if (storageVolumes != null) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (path.equals(storageVolume.getPath())) {
                    ...
                    storageDes = storageVolume.getDescription(mContext);
                    break;
                }
            }
        }
        if (newState.equals(Environment.MEDIA_SHARED)) {
            ...
            if (!mStorageManager.isUsbMassStorageEnabled()) {
                if (oldState.equals(Environment.MEDIA_SHARED)) {
                     ...
                     */
                    if (isRemovable) {
                        setMediaStorageNotification_ext(
            ...
            int i = path.hashCode();
            PendingIntent pi = PendingIntent.getActivity(mContext, i, intent,
    ...
    private synchronized void setMediaStorageNotification(int titleId, int messageId, int icon, boolean visible,
        ...
        if (mEventPath != null && mNoticeId != null && visible) {
             ...
             * re-use it.
             */
            if (mNoticeId.containsKey(mEventPath)) {
    ...
    private synchronized void setMediaStorageNotification_ext(int titleId, int messageId, int icon,
                                                              boolean visible,  boolean dismissable,
                                                              PendingIntent pi,
                                                              String storageDesciption) {
        ...
        if (visible) {
            Resources r = Resources.getSystem();
            CharSequence title = r.getText(titleId);
            String tempTitle = title.toString();
            CharSequence message = r.getText(messageId);

            if (storageDesciption != null)
                tempTitle = String.format(tempTitle + "(" + storageDesciption + ")");
            ...
            mMediaStorageNotification.color = mContext.getResources().getColor(
                       com.android.internal.R.color.system_notification_accent_color);
            mMediaStorageNotification.setLatestEventInfo(mContext, (CharSequence) tempTitle,
                                                         message, pi);
            mMediaStorageNotification.visibility = Notification.VISIBILITY_PUBLIC;
            mMediaStorageNotification.category = Notification.CATEGORY_SYSTEM;
            if (mEventPath != null && mNoticeId != null) {
