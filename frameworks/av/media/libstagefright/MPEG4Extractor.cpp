    ...
    switch(chunk_type) {
        ...
        case FOURCC('c', 'h', 'a', 'n'):
        {
            ...
            if (chunk_size == sizeof(ALACChannelLayoutInfo)) {
                ...
                if (mDataSource->readAt(data_offset, &cli,
                    sizeof(ALACChannelLayoutInfo))
                    != sizeof(ALACChannelLayoutInfo)) {
                ...
                ALOGV("Ignoring potentially malformed alac channel layout \
                       info: size=0x%llx", chunk_size);
            }
        ...
        case FOURCC('a', 'l', 'a', 'c'):
        {
            // Note: parseChunk() routine handles "big box" where size is
            // stored differently. For purpose of ALAC support the size
            // should be deterministic, and stored in 32 bit. alac chunk
            // could be for AudioSampleEntry which encapsulate entire
            //  magic cookie ALACSpecificConfig has a 12-byte FullBox header
            if (chunk_size == (sizeof(FullBox) + sizeof(ALACSpecificConfig))) {
                ...
                if (mDataSource->readAt(data_offset, &buffer[0],
                ...
                mLastTrack->meta->setInt32(kKeyChannelCount,
                                           (0x000000ff & pasc->numChannels));
                mLastTrack->meta->setInt32(kKeySampleRate,
                                           ntohl(pasc->sampleRate));
                ...
                mLastTrack->meta->setData(kKeyAlacMagicCookie, 0,
                (void *)&buffer[0], sizeof(ALACSpecificConfig));
                *offset += chunk_size;
            } else if (chunk_size > sizeof(AudioSampleEntry)) {
                // Treat as Audio Sample Entry and parse recursively.
                AudioSampleEntry ase;
                if (mDataSource->readAt(*offset, &ase, sizeof(AudioSampleEntry))
                ...
                ALOGV("ase.type='%c' '%c' '%c' '%c'", ase.type & 0xff,
                      (ase.type & 0xff00) >> 8, (ase.type &0xff0000) >> 16,
                      (ase.type & 0xff000000) >> 24);
