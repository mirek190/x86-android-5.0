...
/*! Blocking wait on an event source EVENT[ID]
...
/*! Query all event sources enabled in EVENT_MASK
 
 \return any(!isempty(event_queue[...]))
 */
STORAGE_CLASS_EVENT_H bool any_event_pending(void);

/*! Add the event source EVENT[ID] to EVENT_MASK
 
 \param	ID[in]				EVENT identifier

 \return none, EVENT_MASK | event_mask[ID]
 */
STORAGE_CLASS_EVENT_H void event_set_pending_mask(
	const event_ID_t		ID);

/*! Remove the event source EVENT[ID] from EVENT_MASK
 
 \param	ID[in]				EVENT identifier

 \return none, EVENT_MASK & ~event_mask[ID]
 */
STORAGE_CLASS_EVENT_H void event_clear_pending_mask(
	const event_ID_t		ID);
