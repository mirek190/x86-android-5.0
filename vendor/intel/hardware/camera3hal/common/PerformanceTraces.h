 ...
 */
  ...
  class HalTrace {
      public:
          HalTrace(const char* func, const char* tag, const char* note = NULL, int value = -1) STUB_BODY
          ~HalTrace() STUB_BODY
          static void setTraceLevel(int) STUB_BODY
  };
 ...
 /**
  ...
  #define PERFORMANCE_HAL_ATRACE() \
      PerformanceTraces::HalTrace atrace(__FUNCTION__, LOG_TAG);

  #define PERFORMANCE_HAL_ATRACE_PARAM1(note, value) \
      PerformanceTraces::HalTrace atrace(__FUNCTION__, LOG_TAG, note, value);
