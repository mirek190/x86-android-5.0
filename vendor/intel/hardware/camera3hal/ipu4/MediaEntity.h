 ...
 */
class MediaEntity: public RefBase
{
public:
    ...
    status_t getDevice(sp<V4L2DeviceBase> &device, VideoNodeType nodeType = VIDEO_GENERIC);
...
private:
    status_t openDevice(sp<V4L2DeviceBase> &device, VideoNodeType nodeType = VIDEO_GENERIC);

private:
    struct media_entity_desc        mInfo;       /*!< media entity descriptor info */
    Vector<struct media_link_desc>  mLinks;      /*!< media entity links */
    Vector<struct media_pad_desc>   mPads;       /*!< media entity pads */

    sp<V4L2DeviceBase>              mDevice;     /*!< V4L2 video node or subdevice */
