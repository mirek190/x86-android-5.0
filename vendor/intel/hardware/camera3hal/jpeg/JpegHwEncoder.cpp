 ...
 * \return -1 failure
 */
status_t JpegHwEncoder::setInputBuffers(InputBuffer* inputBuffersArray, int inputBuffersNum)
{
    LOG1("@%s", __FUNCTION__);
    ...
    for (int i = 0;i < inputBuffersNum;i++) {
        ...
        */
        status = mHwImageEncoder->createSourceSurface(SURFACE_TYPE_USER_PTR, inputBuffersArray[i].buf, inputBuffersArray[i].width, inputBuffersArray[i].height, inputBuffersArray[i].bpl, vaFmt, &imageSeq);
        if (status != 0) {
            LOGE("create source surface failed");
            return UNKNOWN_ERROR;
        }
        mBuffer2SurfaceId.add(inputBuffersArray[i].buf, imageSeq);
        if (i == 0)
...
int JpegHwEncoder::encode(const InputBuffer &in, OutputBuffer &out)
{
    ...
    if (imgSeq == ERROR_POINTER_NOT_FOUND) {
        ...
        if (status) {
            LOGE("Encoder failed to handle dynamic buffer, falling back to SW");
...
{
    LOG1("@%s, quality=%d", __FUNCTION__,out.quality);
    int imgSeq = -1;
    int status = 0;
