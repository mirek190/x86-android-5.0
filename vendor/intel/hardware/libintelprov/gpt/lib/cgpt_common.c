...
static int Load(const int fd, uint8_t **buf,
  ...
  *buf = malloc(count);
  ...
  uint64_t where = sector * sector_bytes;
  if (-1 == lseek64(fd, where, SEEK_SET)) {
    Error("Can't lseek to  %llu: %s \n", where, strerror(errno));
    goto error_free;
  }
...
int ReadPMBR(struct drive *drive) {
  if (-1 == lseek64(drive->fd, 0, SEEK_SET))
    return CGPT_FAILED;
...
void SetPriority(GptData *gpt, int secondary, uint32_t entry_index,
  ...
  require(priority <= CGPT_ATTRIBUTE_MAX_PRIORITY);
...
void SetTries(GptData *gpt, int secondary, uint32_t entry_index, unsigned int tries) {
  GptEntry *entry;
  entry = GetEntry(gpt, secondary, entry_index);
  require(tries <= CGPT_ATTRIBUTE_MAX_TRIES);
...
void SetSuccessful(GptData *gpt, int secondary, uint32_t entry_index,
  ...
  require(success <= CGPT_ATTRIBUTE_MAX_SUCCESSFUL);
...
#define TOSTRING(A) #A
const char *GptError(unsigned int errnum) {
  ...
  if (errnum >= ARRAY_COUNT(error_string))
...
void PMBRToStr(struct pmbr *pmbr, char *str, int buflen) {
