...
/* MUST BE the same as it's in "fw/sensor_helper.c" */
...
PlatformConfig::PlatformConfig()
{
        ...
        /* init disable sensors list */
        std::string line;
        std::ifstream inFile;
        inFile.open(DISABLE_SENSOR_LIST_FILE, std::ifstream::in);
	if (! inFile.fail()) {
		while (std::getline(inFile, line)) {
			...
			addDisableSensor(line);
		}
	} else {
		ALOGD("Open '%s' failed, don't disable any sensors", DISABLE_SENSOR_LIST_FILE);
	}
...
bool PlatformConfig::isDisableSensor(std::string name) {
        return (disableSensors.find(name) != disableSensors.end());
}

bool PlatformConfig::addDisableSensor(std::string name) {
        if ( name.size() <= 0){
                ALOGE("'%s' sensor is invalid", name.c_str());
                return false;
        }
        if (isDisableSensor(name)) {
                ALOGW("'%s' sensor is already in disable list", name.c_str());
                return false;
        } else {
                ALOGD("Add '%s' sensor to disable list", name.c_str());
                disableSensors.insert(name);
                return true;
        }
}
...
bool PlatformConfig::addSensorDevice(xmlNodePtr node, std::string type, std::string category)
{
        ...
        ALOGD("Add sensor device '%s'", mSensor.getName());
        if (isDisableSensor(std::string(mSensor.getName()))) {
                ALOGW("'%s' has been disabled by added to %s", mSensor.getName(), DISABLE_SENSOR_LIST_FILE);
                return false;
        }
