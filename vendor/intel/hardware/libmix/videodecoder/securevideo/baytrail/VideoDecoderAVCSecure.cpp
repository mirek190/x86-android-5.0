...
Decode_Status VideoDecoderAVCSecure::decodeSlice(vbp_data_h264 *data, uint32_t picIndex, uint32_t sliceIndex) {
    ...
    if (sliceParam->first_mb_in_slice == 0 || mDecodingFrame == false) {
        ...
        if (mDecodingFrame) {
            ...
            for (int i = 0; i < mBufferIDCount; i++) {
                vaDestroyBuffer(mVADisplay, mBufferIDs[i]);
            }
        ...
        mBufferIDs[mBufferIDCount] = bufferIDs[bufferIDCount];
        mBufferIDCount++;
        bufferIDCount++;
        ...
        if (mIsEncryptData) {
            ...
            encryptParam.app_id = mEncParam.app_id;
...
Decode_Status VideoDecoderAVCSecure::endDecodingFrame(bool dropFrame) {
