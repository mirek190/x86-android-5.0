...
Decode_Status VideoDecoderMPEG4::decode(VideoDecodeBuffer *buffer) {
    ...
    if (useGraphicbuffer) {
        if (mIsShortHeader)
            DumpInputBuffer(buffer, "h263");
        else
            DumpInputBuffer(buffer, "mpeg4");
    }
#endif
    ...
    if ((mVideoFormatInfo.width != (int32_t)data->codec_data.video_object_layer_width ||
        ...
        ITRACE("Video size is changed. from %dx%d to %dx%d\n",mVideoFormatInfo.width,mVideoFormatInfo.height, data->codec_data.video_object_layer_width,data->codec_data.video_object_layer_height);
...
Decode_Status VideoDecoderMPEG4::decodeSlice(vbp_data_mp42 *data, vbp_picture_data_mp42 *picData) {
    ...
    if (!mIsSoftwareDecoder) {
        ...
        void *forward_ref_ptr = NULL;
        void *backward_ref_ptr = NULL;
        sliceData->cur_frame_addr = mAcquiredBuffer->mappedData->data;
        sliceData->pic_stride = mAcquiredBuffer->mappedData->pitch[0];
        sliceData->pic_height = mAcquiredBuffer->mappedData->offset[1]/sliceData->pic_stride;
        if (picParam->forward_reference_picture != VA_INVALID_SURFACE) {
            sliceData->forward_ref_addr = mLastReference->mappedData->data;
        }
        if (picParam->backward_reference_picture != VA_INVALID_SURFACE) {
            sliceData->backward_ref_addr = mLastReference->mappedData->data;
            sliceData->forward_ref_addr = mForwardReference->mappedData->data;
        }
        vbp_decode(mParserHandle,(void *)picData);
    }
...
#else
    vaProfile = (VAProfile)VAProfileSoftwareDecoding;
    mIsSoftwareDecoder = true;
#endif
