...
Decode_Status VideoDecoderAVC::startVA(vbp_data_h264 *data) {
    ...
    } else if (data->codec_data->profile_idc == 66)
...
void VideoDecoderAVC::updateFormatInfo(vbp_data_h264 *data) {
    ...
    if ((mVideoFormatInfo.width != width ||
        ...
        // the port definition's height were set according to the
        // infomation in container. In some cases, this info is not
        // right. Need to correct it by format change notification.
        if (!(mConfigBuffer.flag & USE_NATIVE_GRAPHIC_BUFFER)) {
            mSizeChanged = true;
