...
Decode_Status VideoDecoderWMV::decode(VideoDecodeBuffer *buffer) {
    ...
    if ((mVideoFormatInfo.width != data->se_data->CODED_WIDTH ||
        ...
        ITRACE("video size is changed from %dx%d to %dx%d", mVideoFormatInfo.width, mVideoFormatInfo.height, data->se_data->CODED_WIDTH, data->se_data->CODED_HEIGHT);
