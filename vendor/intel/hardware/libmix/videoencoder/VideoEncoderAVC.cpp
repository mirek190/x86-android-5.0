...
//    LOG_I("intraPeriod=%d, ipPeriod=%d, idrPeroid=%d, frame_num=%d\n", intraPeriod, mVideoParamsAVC.ipPeriod, idrPeroid, frame_num);
...
Encode_Status VideoEncoderAVC::renderMaxSliceSize() {
    ...
    if (mComParams.rcMode != RATE_CONTROL_VCM && mComParams.rcMode != RATE_CONTROL_CFS) {
        LOG_W ("Not in VCM/CFS mode, but call send_max_slice_size\n");
        return ENCODE_SUCCESS;
    }
...
Encode_Status VideoEncoderAVC::renderSequenceParams(EncodeTask *task) {
    ...
    memset(rcMiscParam, 0, sizeof(VAEncMiscParameterRateControl));
    memset(framerateParam, 0, sizeof(VAEncMiscParameterFrameRate));
    ...
    rcMiscParam->rc_flags.bits.disable_frame_skip = mComParams.rcParams.disableFrameSkip;
    rcMiscParam->rc_flags.bits.disable_bit_stuffing = mComParams.rcParams.disableBitsStuffing;
    rcMiscParam->rc_flags.bits.mb_rate_control = mComParams.rcParams.enableCARC;
    if (mComParams.rcMode == RATE_CONTROL_CFS)
        rcMiscParam->rc_flags.bits.cfs_I_frames = mComParams.rcParams.enableCFSIFrames;
    avcSeqParams.intra_period = mComParams.intraPeriod;
    ...
    LOG_I( "disable_frame_skip = %d\n", rcMiscParam->rc_flags.bits.disable_frame_skip);
    LOG_I( "disable_bit_stuffing = %d\n", rcMiscParam->rc_flags.bits.disable_bit_stuffing);
    LOG_I( "mb_rate_control = %d\n", rcMiscParam->rc_flags.bits.mb_rate_control);
