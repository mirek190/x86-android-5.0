...
VideoEncoderVP8::VideoEncoderVP8()
    :VideoEncoderBase() {
        ...
        mVideoParamsVP8.max_num_of_consecutive_drop_frames = 0;
...
Encode_Status VideoEncoderVP8::renderSequenceParams() {
    ...
    vp8SeqParam.intra_period =
        mComParams.frameRate.frameRateNum / mComParams.frameRate.frameRateDenom;
    if(mAutoRefSurfaces)
...
Encode_Status VideoEncoderVP8::renderPictureParams(EncodeTask *task,uint32_t mFrameNum) {
    Encode_Status ret = ENCODE_SUCCESS;
    VAStatus vaStatus = VA_STATUS_SUCCESS;
    ...
    if((mForceKFrame)&&(mFrameNum == mForceKFrameNumber))
        vp8PicParam.ref_flags.bits.force_kf = 1;
    else
        vp8PicParam.ref_flags.bits.force_kf = 0;
    ...
    vp8PicParam.pic_flags.bits.frame_type =
        (mFrameNum % (mComParams.frameRate.frameRateNum / mComParams.frameRate.frameRateDenom)) ? 1 : 0;
    vp8PicParam.pic_flags.bits.show_frame = 1;

    if((mComParams.numberOfLayer == 2)||(mComParams.numberOfLayer == 3)){
        unsigned int index = mFrameNum % mComParams.nPeriodicity;
        vp8PicParam.ref_flags.bits.no_ref_last = mComParams.nVP8RefFrame[index].no_ref_last;
        vp8PicParam.ref_flags.bits.no_ref_gf = mComParams.nVP8RefFrame[index].no_ref_gf;
        vp8PicParam.ref_flags.bits.no_ref_arf = mComParams.nVP8RefFrame[index].no_ref_arf;
        vp8PicParam.pic_flags.bits.refresh_last = mComParams.nVP8RefFrame[index].refresh_last;
        vp8PicParam.pic_flags.bits.refresh_entropy_probs = mComParams.nVP8RefFrame[index].refresh_entropy_probs;
        vp8PicParam.pic_flags.bits.refresh_alternate_frame = mComParams.nVP8RefFrame[index].refresh_alternate_frame;
        if(mComParams.numberOfLayer == 2)
             vp8PicParam.pic_flags.bits.refresh_golden_frame = mComParams.nVP8RefFrame[index].refresh_golden_frame;
        if(mComParams.numberOfLayer == 3)
        {
             vp8PicParam.pic_flags.bits.copy_buffer_to_golden = mComParams.nVP8RefFrame[index].refresh_golden_frame;
             if(vp8PicParam.pic_flags.bits.copy_buffer_to_golden != 0)
...
Encode_Status VideoEncoderVP8::renderRCParams(uint32_t layer_id, bool total_bitrate)
{
    ...
    misc_rate_ctrl->max_num_of_consecutive_drop_frames = mVideoParamsVP8.max_num_of_consecutive_drop_frames;
...
Encode_Status VideoEncoderVP8::renderHRDParams(void)
{
    ...
    misc_hrd->buffer_size = mVideoParamsVP8.hrd_buf_size;
    misc_hrd->initial_buffer_fullness = mVideoParamsVP8.hrd_buf_initial_fullness;
    misc_hrd->optimal_buffer_fullness = mVideoParamsVP8.hrd_buf_optimal_fullness;
...
Encode_Status VideoEncoderVP8::renderLayerStructureParam(void)
{
    ...
    LOG_E("renderLayerStructureParam misc_layer_struc->number_of_layers is %d",misc_layer_struc->number_of_layers);
...
Encode_Status VideoEncoderVP8::sendEncodeCommand(EncodeTask *task) {
    ...
    ret = renderPictureParams(task,mFrameNum);
...
Encode_Status VideoEncoderVP8::calculateForceKFrameNumber(void)
{
        int period = 0;
        if(mComParams.numberOfLayer == 1)// the current frame will be forced as k frame
             mForceKFrameNumber = mFrameNum;

        if(mComParams.numberOfLayer == 2)
        {
             period = 2;
             mForceKFrameNumber = mFrameNum + mFrameNum % period;
        }

        if(mComParams.numberOfLayer == 3)
        {
             period = 4;
             mForceKFrameNumber = mFrameNum + (period - mFrameNum % period) % period;
        }

        return ENCODE_SUCCESS;
}

Encode_Status VideoEncoderVP8::getExtFormatOutput(VideoEncOutputBuffer *outBuffer)
{
    Encode_Status ret = ENCODE_SUCCESS;
    uint32_t sizeCopiedHere = 0;
    uint32_t sizeToBeCopied = 0;
    const uint32_t BITSTREAM_OFFSET = 16;
    ...
    while (1) {
        ...
        if ((mCurSegment->size - mOffsetInSeg) <= outBuffer->bufferSize - sizeCopiedHere) {
            ...
            memcpy(outBuffer->data + sizeCopiedHere + BITSTREAM_OFFSET,
