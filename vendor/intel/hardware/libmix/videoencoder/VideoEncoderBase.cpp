...
VideoEncoderBase::VideoEncoderBase()
    ...
    ,mRenderTemporalLayerRC(false)
    ,mForceKFrameNumber(0)
    {
    ...
    const char *vendorString = vaQueryVendorString(mVADisplay);
    if (vendorString) {
        if (strstr(vendorString, "MRFL"))
            mHwPlatform = VIDENC_MRFL;
        else if (strstr(vendorString, "MFLD"))
            mHwPlatform = VIDENC_MFLD;
        else if (strstr(vendorString, "MRST"))
            mHwPlatform = VIDENC_MRST;
        else if (strstr(vendorString, "LEXINGTON"))
            mHwPlatform = VIDENC_LEXT;
        else
            mHwPlatform = VIDENC_BYT;
    }
    LOG_V("Video Encoder HW platform is %d\n", mHwPlatform);

    memset(&mStatistics, 0, sizeof(mStatistics));
    mStatistics.minBitrate = 0xFFFFFFFF;
}
...
Encode_Status VideoEncoderBase::start() {
    ...
    if (!mInitialized) {
        LOG_E("Encoder Initialize fail can not start");
    ...
    if (mComParams.resolution.width > 2048 || mComParams.resolution.height > 2048){
        LOG_E("Unsupported resolution width %d, height %d\n",
    ...
    if((vaAttrib_tmp[1].value & mComParams.rcMode) != 0)
    {
...
//            &vaAttrib[0], 3, &(mVAConfig));  //uncomment this after psb_video supports
    ...
    if (mComParams.rcMode == RATE_CONTROL_VCM || mComParams.rcMode == RATE_CONTROL_CFS) {
...
Encode_Status VideoEncoderBase::encode(VideoEncRawBuffer *inBuffer, uint32_t timeout) {
    ...
    if(mVACodedBufferList.empty()){
        if(timeout == FUNC_BLOCK)
            ...
            if(NO_ERROR != mEncodeTask_Cond.waitRelative(mCodedBuffer_Lock, 1000000*timeout)){
                ...
                LOG_W("encode return ENCODE_DEVICE_BUSY, Time out wait for Coded buffer.\n");
                return ENCODE_DEVICE_BUSY;
            }
        else {//Nonblock
            mCodedBuffer_Lock.unlock();
            LOG_E("encode return ENCODE_DEVICE_BUSY, Coded buffer is not ready now.\n");
            return ENCODE_DEVICE_BUSY;
        }
    }
    ...
    if(mVACodedBufferList.empty()){
        mCodedBuffer_Lock.unlock();
        LOG_E("encode return ENCODE_DEVICE_BUSY, Coded buffer is still not ready.\n");
        return ENCODE_DEVICE_BUSY;
    }
    ...
    mFrameNum ++;
    mEncodedFrameCount ++;
    task->frame_index = mEncodedFrameCount;
    ...
    LOG_V("encode return Success task frameindex=%d\n", task->frame_index);
...
Encode_Status VideoEncoderBase::getOutput(VideoEncOutputBuffer *outBuffer, uint32_t timeout) {
    ...
    if (mCurOutputTask == NULL) {
        ...
        if(mEncodeTaskList.empty()) {
            ...
            if(timeout == FUNC_BLOCK) {
                LOG_V("getOutput waiting for task....\n");
                mEncodeTask_Cond.wait(mEncodeTask_Lock);
            } else if (timeout > 0) {
                LOG_V("getOutput waiting for task in %d ms....\n", timeout);
                if(NO_ERROR != mEncodeTask_Cond.waitRelative(mEncodeTask_Lock, 1000000*timeout)) {
                    mEncodeTask_Lock.unlock();
                    LOG_W("getOutput Time out wait for encode task.\n");
                    return ENCODE_NO_REQUEST_DATA;
                }
            } else {//Nonblock
                mEncodeTask_Lock.unlock();
                LOG_V("getOutput return ENCODE_NO_REQUEST_DATA, no encode task running\n");
                return ENCODE_NO_REQUEST_DATA;
            }
        }
        ...
        if(mEncodeTaskList.empty()){
            mEncodeTask_Lock.unlock();
            LOG_V("getOutput return ENCODE_DATA_NOT_READY, still no encode running\n");
    ...
    if (mCurOutputTask->completed == false) {
        ...
        if (timeout == FUNC_BLOCK) {
            ...
            LOG_I ("getOutput block mode, frameindex=%d, vaMapBuffer ID = 0x%08x\n", mCurOutputTask->frame_index, mOutCodedBuffer);
            ...
            LOG_I ("getOutput non-block mode, frameindex=%d, vaQuerySurfaceStatus ID = 0x%08x\n", mCurOutputTask->frame_index, mCurOutputTask->enc_surface);
            ...
            if (vaSurfaceStatus & VASurfaceReady) {
                ...
                LOG_V("getOutput return ENCODE_DATA_NOT_READY, encode is not completed\n");
                return ENCODE_DATA_NOT_READY;
            }
    ...
    if (gLogLevel && (outBuffer->format == OUTPUT_EVERYTHING || outBuffer->format == OUTPUT_FRAME_DATA ||
        outBuffer->format == OUTPUT_LENGTH_PREFIXED || outBuffer->format == OUTPUT_NALULENGTHS_PREFIXED)) {
        VideoEncoderStatistics* stat = new VideoEncoderStatistics;
        stat->frameIndex = mCurOutputTask->frame_index;
        stat->frameSize = outBuffer->dataSize;
        mStatisticsQueue.push_back(stat);

        uint32_t frameRateNum = mComParams.frameRate.frameRateNum;
        uint32_t frameRateDenom = mComParams.frameRate.frameRateDenom;
        uint32_t fps = (uint32_t) (frameRateNum + frameRateDenom /2 ) / frameRateDenom;
        mStatistics.frameIndex = stat->frameIndex - 1;
        mStatistics.frameSize += stat->frameSize;
        mStatistics.totalframeSize += stat->frameSize;
        mStatistics.avgBitrate = (mStatistics.totalframeSize / stat->frameIndex) * 8 * fps;
        mStatistics.frameBitrate = stat->frameSize * 8 * fps;
        if (mStatistics.frameBitrate > mStatistics.maxBitrate)
            mStatistics.maxBitrate = mStatistics.frameBitrate;
        if (mStatistics.frameBitrate < mStatistics.minBitrate)
            mStatistics.minBitrate = mStatistics.frameBitrate;

        if (mStatisticsQueue.size() > fps ){
            VideoEncoderStatistics* head =  *(mStatisticsQueue.begin());
            mStatisticsQueue.erase(mStatisticsQueue.begin());
            mStatistics.frameSize -= head->frameSize;
            delete head;
        }
        mStatistics.realBitrate = mStatistics.frameSize * 8;

        if (mStatistics.frameIndex % fps == 0) {
            LOG_S("STATISTICS(%u):: bitrate: avg %u kbps, real %u kbps\n", 
                    mStatistics.frameIndex, mStatistics.avgBitrate / 1024, mStatistics.realBitrate / 1024);

            LOG_S("STATISTICS(%u):: bitrate: frame %u kbps, max/min %u/%u kbps\n", 
                    mStatistics.frameIndex, mStatistics.frameBitrate / 1024, mStatistics.maxBitrate / 1024, mStatistics.minBitrate / 1024 );
        }
    }
...
CLEAN_UP:
    ...
    LOG_V("getOutput return error=%d\n", ret);
    return ret;
}
...
CLEAN_UP:
    ...
    mFrameNum = 0;
    mEncodedFrameCount = 0;
    while(! mStatisticsQueue.empty()) {
        delete *mStatisticsQueue.begin();
        mStatisticsQueue.erase(mStatisticsQueue.begin());
    }
    memset(&mStatistics, 0, sizeof(mStatistics));
    mStatistics.minBitrate = 0xFFFFFFFF;
...
Encode_Status VideoEncoderBase::prepareForOutput(
    ...
    if((mComParams.profile == VAProfileVP8Version0_3) &&
       ((mHwPlatform == VIDENC_MRFL) || (mHwPlatform == VIDENC_MFLD)))// To check VP8 encode corrupted frame
    {
        unsigned int vp8_frame_status = 0;
        vp8_frame_status = *((unsigned int*)mCurSegment->buf - 15);
        LOG_E("To check vp8 frame statics");
        LOG_E("To check vp8 frame statics status is %d",vp8_frame_status);
        LOG_E("To check vp8 frame statics QP is %d",*((unsigned int*)mCurSegment->buf - 14));
        mCurOutputTask->priv = (void *)(unsigned long)(*((unsigned int*)mCurSegment->buf - 14));
        if(vp8_frame_status == 4) //frame is already dropped
        {
            outBuffer->flag |= ENCODE_BUFFERFLAG_DATACORRUPT;
            outBuffer->dataSize = 0;
            outBuffer->offset = 16;
            return ENCODE_CODED_DATA_CORRUPTED;
        }
    }
...
#else
            stream_memcpy(outBuffer->data + sizeCopiedHere,
#endif
...
void VideoEncoderBase::setDefaultParams() {
    ...
    mComParams.rcParams.enableCARC = 0;
    mComParams.rcParams.enableCFSIFrames = 0;
...
Encode_Status VideoEncoderBase::setParameters(
    ...
    LOG_I("setParameters type = %x\n", (int)videoEncParams->type);

    if (mStarted) {
        LOG_W("Encoder has been initialized, should use setConfig to change configurations\n");
        return ENCODE_ALREADY_INIT;
    }
    ...
    switch (videoEncParams->type) {
        ...
        case VideoParamsTypeTemporalLayer:{
            ...
            for(int i=0;i<temporallayer->nPeriodicity;i++)
            {
                ...
                memcpy(&(mComParams.nVP8RefFrame[i]), &(temporallayer->nVP8RefFrame[i]), sizeof(VideoConfigVP8ReferenceFrame));
            }
...
Encode_Status VideoEncoderBase::manageSrcSurface(VideoEncRawBuffer *inBuffer, VASurfaceID *sid) {
    ...
    if (pvinfo){
        ...
        if((mHwPlatform == VIDENC_BYT || mHwPlatform == VIDENC_CHT)
                && (pvinfo->mode == MEM_MODE_MALLOC))
            map->setAction(MAP_ACTION_COPY);
        else
...
Encode_Status VideoEncoderBase::renderDynamicBitrate(EncodeTask* task) {
    ...
    memset(bitrateControlParam, 0, sizeof(VAEncMiscParameterRateControl));
    ...
    bitrateControlParam->rc_flags.bits.mb_rate_control = mComParams.rcParams.enableCARC;
    if (mComParams.rcMode == RATE_CONTROL_CFS)
        bitrateControlParam->rc_flags.bits.cfs_I_frames = mComParams.rcParams.enableCFSIFrames;
    ...
    LOG_I("mb_rate_control = %d\n", bitrateControlParam->rc_flags.bits.mb_rate_control);
    LOG_I("cfs_I_frames = %d\n", bitrateControlParam->rc_flags.bits.cfs_I_frames);
...
Encode_Status VideoEncoderBase::renderDynamicFrameRate() {
    ...
    if (mComParams.rcMode != RATE_CONTROL_VCM && mComParams.rcMode != RATE_CONTROL_CFS) {
        ...
        LOG_W("Not in VCM or CFS mode, but call SendDynamicFramerate\n");
        return ENCODE_SUCCESS;
    }
