...
enum {
    ENCODE_CODED_DATA_CORRUPTED = -12,
...
typedef enum {
    RATE_CONTROL_NONE = 0x00000001,
    RATE_CONTROL_CBR = 0x00000002,
    RATE_CONTROL_VBR = 0x00000004,
    RATE_CONTROL_VCM = 0x00000008,
    RATE_CONTROL_CFS = 0x00000100,
    RATE_CONTROL_LAST
} VideoRateControl;
...
struct VideoRateControlParams {
    ...
    VideoRateControlParams &operator=(const VideoRateControlParams &other) {
        ...
        this->enableCARC = other.enableCARC;
        this->enableCFSIFrames = other.enableCFSIFrames;
...
struct VideoParamsCommon : VideoParamConfigSet {
    ...
    VideoConfigVP8ReferenceFrame nVP8RefFrame[32];
...
struct VideoParamsHRD : VideoParamConfigSet {
    ...
    uint32_t optimalBufferFullness;
};
...
struct VideoParamsVP8 : VideoParamConfigSet {
    ...
    uint32_t max_num_of_consecutive_drop_frames;
...
typedef struct {
    ...
    uint32_t avgBitrate;   //average bitrate for stream
    uint32_t realBitrate;  //bitrate in last 1 second stream
    uint32_t frameBitrate; //bitrate per frame
    uint32_t maxBitrate;   //max bitrate
    uint32_t minBitrate;   //min bitrate
