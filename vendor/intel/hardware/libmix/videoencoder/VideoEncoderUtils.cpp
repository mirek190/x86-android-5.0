...
#ifdef IMG_GFX_MRFL
    err = GrallocMod->Blit(GrallocMod, src, dest, w, h, 0, 0, 0, 0, 0);
#else
...
Encode_Status VASurfaceMap::doMapping() {
    ...
    if (mVASurface == VA_INVALID_SURFACE) {
        ...
        LOG_I("vinfo, width=%d, height=%d, stride=%d\n",
            mVinfo.width, mVinfo.height, mVinfo.lumaStride);

        if (mVinfo.mode == MEM_MODE_GFXHANDLE) {
            //do not trust valueinfo for gfx case, directly get from structure
            ret = GetGfxBufferInfo(mValue, mVinfo);
        ...
        if (mAction & MAP_ACTION_COLORCONVERT) {
            ...
            if (HAL_PIXEL_FORMAT_NV12 == mVinfo.format || OMX_INTEL_COLOR_FormatYUV420PackedSemiPlanar == mVinfo.format)
        ...
        if (mAction & MAP_ACTION_ALIGN64 && height % 16 != 0) {
            //check if height is not 16 aligned
            height = (height + 15 ) & ~15;
        }

        if((mAction & MAP_ACTION_ALIGN64 && width <= 320 && height <= 240) &&
           (mVinfo.mode == MEM_MODE_MALLOC || mVinfo.mode == MEM_MODE_GFXHANDLE)) {
            mAction |= MAP_ACTION_COPY;
        }
        ...
        if (mAction & MAP_ACTION_COPY) { //must allocate new vasurface(EXternalMemoryNULL, uncached)
            ...
            LOG_I("create new vaSurface for MAP_ACTION_COPY\n");
        } else {
        #ifdef IMG_GFX
            if (mGfxHandle != NULL) {
                ...
                LOG_I("map new allocated gfx handle to vaSurface\n");
            } else
        #endif
            {
...
VASurfaceID VASurfaceMap::CreateSurfaceFromExternalBuf(intptr_t value, ValueInfo& vinfo) {
    ...
    extbuf.offsets[3] = 0;
    extbuf.buffers = &data;
    // hack for GEN platform
    extbuf.flags = VA_SURFACE_ATTRIB_MEM_TYPE_ANDROID_GRALLOC;
