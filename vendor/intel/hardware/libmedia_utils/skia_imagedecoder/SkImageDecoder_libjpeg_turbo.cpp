...
public:
    SkJPEGTurboImageIndex(SkStreamRewindable* stream, SkImageDecoder* decoder)
...
class JPEGTurboAutoClean {
public:
    JPEGTurboAutoClean(): cinfo_ptr(NULL) {}
    ~JPEGTurboAutoClean() {
        if (cinfo_ptr) {
...
SkColorType SkJPEGTurboImageDecoder::getBitmapColorType(jpeg_decompress_struct* cinfo) {
    SkASSERT(cinfo != NULL);
...
bool SkJPEGTurboImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
#ifdef TIME_DECODE
    ...
    */
         ...
         (kRGB_565_SkColorType == colorType && cinfo.out_color_space == JCS_RGB_565)))
    {
        ...
        ALOGD("turboImageDecoder took %.2fms to decode %dx%d (%dx downscale)",
            (endtime - starttime)/1000000.0,
            cinfo.image_width, cinfo.image_height, sampleSize);
#endif
        return true;
    }
#endif
...
#ifdef SK_BUILD_FOR_ANDROID
bool SkJPEGTurboImageDecoder::onBuildTileIndex(SkStreamRewindable* stream, int *width, int *height) {

    SkAutoTDelete<SkJPEGTurboImageIndex> imageIndex(SkNEW_ARGS(SkJPEGTurboImageIndex, (stream, this)));
...
bool SkJPEGTurboImageDecoder::onDecodeSubset(SkBitmap* bm, const SkIRect& region) {
    if (NULL == fImageIndex) {
        return false;
    }
...
bool SkJPEGTurboImageEncoder::onEncode(SkWStream* stream, const SkBitmap& bm, int quality) {
#ifdef TIME_ENCODE
    ...
    endtime = systemTime(SYSTEM_TIME_MONOTONIC);
#ifdef LOG_DECODE_TIME
    ALOGD("turboImageEncoder took %.2fms to encode %dx%d",
        (endtime - starttime)/1000000.0,
        bm.width(), bm.height());
#endif
...
///////////////////////////////////////////////////////////////////////////////
DEFINE_DECODER_CREATOR(JPEGTurboImageDecoder);
DEFINE_ENCODER_CREATOR(JPEGTurboImageEncoder);
///////////////////////////////////////////////////////////////////////////////
...
static inline bool is_libjpeg_turbo_disabled() {
    static char property[PROPERTY_VALUE_MAX];
    if (property_get("skia.libjpegturbo.disabled", property, NULL) > 0) {
...
static SkImageDecoder* sk_libjpeg_turbo_dfactory(SkStreamRewindable* stream) {
    if (!is_libjpeg_turbo_disabled() && is_jpeg(stream))
        return SkNEW(SkJPEGTurboImageDecoder);
    return NULL;
}
...
static SkImageEncoder* sk_libjpeg_turbo_efactory(SkImageEncoder::Type t) {
    if (!is_libjpeg_turbo_disabled() && (SkImageEncoder::kJPEG_Type == t))
        return SkNEW(SkJPEGTurboImageEncoder);
    return NULL;
}

static SkImageDecoder_DecodeReg gDReg(sk_libjpeg_turbo_dfactory);
static SkImageDecoder_FormatReg gFormatReg(get_format_jpeg);
static SkImageEncoder_EncodeReg gEReg(sk_libjpeg_turbo_efactory);
