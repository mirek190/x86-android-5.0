...
struct pcm_config {
    ...
    /* Values to use for the ALSA start, stop thresholds.  Setting
...
static int set_hardware_params(struct widi_stream_out *out)
{
...
static int set_software_params(struct widi_stream_out *out)
{
...
static int open_device(struct widi_stream_out *out)
{
    int err = 0;

    ALOGD("%s: opening AndroidPlayback_widi for %d channels", __func__, out->pcm_config.channels);
    ...
    if (err == 0) {
       ALOGI("%s: Initialized widi Audio device", __func__);
       return 0;
    }
...
static int start_output_stream(struct widi_stream_out *out)
{
    struct audio_device *widi_dev = out->dev;
    int i;
...
static int widi_dev_open_output_stream(struct audio_hw_device *dev,
...
{
    struct audio_device *widi_dev = (struct audio_device *)dev;
    struct widi_stream_out *out;
    int ret;

    ALOGV("%s: entered", __func__);

    out = (struct widi_stream_out *)calloc(1, sizeof(struct widi_stream_out));
    if (!out)
        return -ENOMEM;
    ...
    if (flags & AUDIO_OUTPUT_FLAG_DIRECT) {
        ALOGV("%s: WIDI Multichannel",__func__);
        if (config->sample_rate == 0)
            config->sample_rate = pcm_config_default.rate;
        if (config->channel_mask == 0)
            config->channel_mask = AUDIO_CHANNEL_OUT_5POINT1;
    } else {
        ALOGV("%s: WIDI Stereo",__func__);
        if (config->sample_rate == 0)
            config->sample_rate = pcm_config_default.rate;
        if (config->channel_mask == 0)
...
static void widi_dev_close_output_stream(struct audio_hw_device *dev,
                                     struct audio_stream_out *stream)
{
    struct widi_stream_out *out = (struct widi_stream_out *)(&stream->common);
...
static int widi_dev_init_check(const struct audio_hw_device *dev)
{
    ALOGV("%s unsupported", __func__);
    return 0;
}

static int widi_dev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    ALOGV("%s unsupported", __func__);
    return -ENOSYS;
}

static int widi_dev_set_master_volume(struct audio_hw_device *dev, float volume)
{
    ALOGV("%s unsupported", __func__);
    return -ENOSYS;
}

static int widi_dev_set_mode(struct audio_hw_device *dev, audio_mode_t mode)
{
    ALOGV("%s unsupported", __func__);
    return 0;
}

static int widi_dev_set_mic_mute(struct audio_hw_device *dev, bool state)
{
    ALOGV("%s unsupported", __func__);
    return -ENOSYS;
}

static int widi_dev_get_mic_mute(const struct audio_hw_device *dev, bool *state)
{
    ALOGV("%s unsupported", __func__);
    return -ENOSYS;
}

static size_t widi_dev_get_input_buffer_size(const struct audio_hw_device *dev,
...
static int widi_dev_open_input_stream(struct audio_hw_device *dev,
                                  audio_io_handle_t handle,
                                  audio_devices_t devices,
                                  struct audio_config *config,
...
static void widi_dev_close_input_stream(struct audio_hw_device *dev,
                                   struct audio_stream_in *stream)
{
    ALOGV("%s unsupported", __func__);
}

static int widi_dev_dump(const audio_hw_device_t *device, int fd)
{
    ALOGV("%s unsupported", __func__);
    return 0;
}

static int widi_dev_close(hw_device_t *device)
{
    struct audio_device *widi_dev = (struct audio_device *)device;
    ALOGV("%s ", __func__);
    free(widi_dev);
    return 0;
}

static uint32_t widi_dev_get_supported_devices(const struct audio_hw_device *dev)
{
    return AUDIO_DEVICE_OUT_WIDI;
}

static int widi_dev_open(const hw_module_t* module, const char* name,
                     hw_device_t** device)
{
    struct audio_device *widi_dev;
    int ret;
    ...
    widi_dev->hw_device.get_supported_devices = widi_dev_get_supported_devices;
    widi_dev->hw_device.init_check            = widi_dev_init_check;
    widi_dev->hw_device.set_voice_volume      = widi_dev_set_voice_volume;
    widi_dev->hw_device.set_master_volume     = widi_dev_set_master_volume;
    widi_dev->hw_device.set_mode              = widi_dev_set_mode;
    widi_dev->hw_device.set_mic_mute          = widi_dev_set_mic_mute;
    widi_dev->hw_device.get_mic_mute          = widi_dev_get_mic_mute;
    widi_dev->hw_device.set_parameters        = widi_dev_set_parameters;
    widi_dev->hw_device.get_parameters        = widi_dev_get_parameters;
    widi_dev->hw_device.get_input_buffer_size = widi_dev_get_input_buffer_size;
    widi_dev->hw_device.open_output_stream    = widi_dev_open_output_stream;
    widi_dev->hw_device.close_output_stream   = widi_dev_close_output_stream;
    widi_dev->hw_device.open_input_stream     = widi_dev_open_input_stream;
    widi_dev->hw_device.close_input_stream    = widi_dev_close_input_stream;
    widi_dev->hw_device.dump                  = widi_dev_dump;

    *device = &widi_dev->hw_device.common;
