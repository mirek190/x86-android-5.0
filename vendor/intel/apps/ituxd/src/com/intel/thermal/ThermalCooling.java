...
public class ThermalCooling {
    ...
    /* List of cooling devices that need to be throttled on entry and dethrottled on exit */
    private static ArrayList<Integer> sProfCoolingDevs = new ArrayList<Integer>();
    private Hashtable <String, ArrayList<Integer>> sProfileEntryExitList = null;
    ...
    public class ThermalParser {
        ...
        ArrayList<Integer> mProfCoolingDevs = null;

        private boolean isProfEntryExit = false;
            ...
            try {
                if (name.equalsIgnoreCase(CDEVINFO)) {
                    ...
                    if (name.equalsIgnoreCase("ZoneID") && mZone != null) {
                        ...
                        mTempMaskList = new ArrayList<Integer>();
                    } else if (name.equalsIgnoreCase("CoolingDevId")) {
                        if (!isProfEntryExit && mZone != null) {
                            mZone.getLastCoolingDeviceInstance().setCoolingDeviceId(
                                    Integer.parseInt(mParser.nextText()));
                        } else {
                            if (mProfCoolingDevs == null) {
                                mProfCoolingDevs = new ArrayList<Integer>();
                            }
                            Log.i(TAG, "Adding entry/exit cooling device");
                            mProfCoolingDevs.add(Integer.parseInt(mParser.nextText()));
                        }
                    ...
                    } else if (name.equalsIgnoreCase("ProfileEntryExit")) {
                        sProfileEntryExitList = new Hashtable<String, ArrayList<Integer>>();
                        isProfEntryExit = true;
                        Log.i(TAG, "ProfileEntryExit tag found for profile:" + mCurProfileName);
                    }
                }
        ...
        void processEndElement(String name) {
            ...
            if (name.equalsIgnoreCase(CDEVINFO) && mDevice != null) {
               ...
               isProfEntryExit = false;
               sProfileEntryExitList.put(mCurProfileName, mProfCoolingDevs);
               Log.i(TAG, "Added entry/exit cooling devices for " + mCurProfileName);
    ...
    public boolean init(Context context) {
        ...
        emergencyIntentFilter.addAction(OemTelephonyIntents.ACTION_EMERGENCY_CALL_STATUS_CHANGED);
    ...
    private final class ProfileChangeReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            // Reject any profile change request while on low power mode.
            if (ThermalManager.isLowPowerMode) return;
            ...
            if (action.equals(ThermalManager.ACTION_CHANGE_THERMAL_PROFILE)) {
                ...
                if (profName.equals("BatterySaver")) ThermalManager.isLowPowerMode = true;
                if (profName != null) {
                    // If there are any cooling devices that were throttled automatically
                    // on profile switch, dethrottle them now.
                    if (sProfCoolingDevs != null) {
                        for (Integer id : sProfCoolingDevs) {
                            throttleDevice(id, ThermalManager.THERMAL_STATE_NORMAL);
                        }
                        sProfCoolingDevs = null; //Mark it null so that it is not dethrottled again
                    }
                    Log.i(TAG, "Changing to profile:" + profName);
                    ThermalManager.changeThermalProfile(profName);
                    // In low power mode, make a note of last active profile.
                    // We need to switch to that profile on exit from low power mode.
                    if (sProfileEntryExitList != null) {
                        // If there are cooling devices to be throttled automatically,
                        // throttle them here.
                        sProfCoolingDevs = sProfileEntryExitList.get(profName);
                        if (sProfCoolingDevs != null) {
                            for (Integer id : sProfCoolingDevs) {
                                throttleDevice(id, ThermalManager.THERMAL_STATE_CRITICAL);
                            }
                        }
                    }
                }
    ...
    private final class EmergencyCallReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (action.equals(OemTelephonyIntents.ACTION_EMERGENCY_CALL_STATUS_CHANGED)) {
