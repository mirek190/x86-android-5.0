...
public class ThermalService extends Service {
                ...
                if (name.equalsIgnoreCase(PINFO)) {
                    ...
                    if (mPlatformInfo != null && name.equalsIgnoreCase("PlatformThermalStates")) {
                        ...
                        if (zoneLogic.equalsIgnoreCase("VirtualSkin")) {
                            ...
                            // ModemZone class may not be required and hence absent
                            // in some devices. So try to access the class through
                            // reflection.
                            try {
                                Class cls = Class.forName("com.intel.thermal.ModemZone");
                                Constructor cons = cls.getDeclaredConstructor(Context.class);
                                Object o = cons.newInstance(mContext);
                                if (o != null) Log.i(TAG, "ModemZone instance not null");
                                mCurrZone = (ThermalZone)o;
                            } catch (Exception e) {
                                Log.i(TAG, "Could not initialize Modem zone" + e);
                            }
                        } else {
    ...
    private final BroadcastReceiver mLowPowerModeReceiver = new BroadcastReceiver() {
        @Override
         public void onReceive(Context context, Intent intent) {
             Intent profileIntent = null;
             if (mPowerManager.isPowerSaveMode()) {
                 ThermalManager.sLastProfile = ThermalManager.getCurProfileName();
                 profileIntent = new Intent(ThermalManager.ACTION_CHANGE_THERMAL_PROFILE);
                 profileIntent.putExtra(ThermalManager.EXTRA_PROFILE, "BatterySaver");
                 context.sendBroadcastAsUser(profileIntent, UserHandle.ALL);
                 Log.i(TAG, "Power Save Mode enabled");
             } else {
                 ThermalManager.isLowPowerMode = false;
                 Log.i(TAG, "Power Save Mode disabled");
                 profileIntent = new Intent(ThermalManager.ACTION_CHANGE_THERMAL_PROFILE);
                 profileIntent.putExtra(ThermalManager.EXTRA_PROFILE, ThermalManager.sLastProfile);
                 context.sendBroadcastAsUser(profileIntent, UserHandle.ALL);
             }
         }
    };
    ...
    public ThermalService() {
        ...
        Log.w(TAG, "Initializing Thermal Service empty constructor");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startid)
    {
        ...
        /* Register for ACTION_POWER_SAVE_MODE_CHANGED intent */
        mPowerManager = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
        IntentFilter lowPowerModeFilter = new IntentFilter();
        lowPowerModeFilter.addAction(PowerManager.ACTION_POWER_SAVE_MODE_CHANGED);
        mContext.registerReceiver(mLowPowerModeReceiver, lowPowerModeFilter);

        /* If the device is booted in low power mode, the intent could be missed.
         * Hence, check for low power mode first time alone.
         */
        if (mPowerManager.isPowerSaveMode()) {
            Log.i(TAG, "Switching to BatterySaver profile");
            ThermalManager.startBatterySaverProfile();
        } else {
