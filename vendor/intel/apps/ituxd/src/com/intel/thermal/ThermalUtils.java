...
public class ThermalUtils {
    ...
    private native static byte[] native_getTRTData(String path);
    ...
    public static byte[] getTRTData(String path) {
        try {
            return native_getTRTData(path);
        } catch (UnsatisfiedLinkError e) {
            Log.i(TAG, "caught UnsatisfiedLinkError in readSysfs");
            return null;
        }
    ...
    public static boolean loadAcpiConfigXML() {
        TrtParser trt = new TrtParser();
        return trt.writeConfigFiles();
    }
    ...
    public static void initialiseConfigFiles(Context context) {
        ...
        } else if ("1".equals(SystemProperties.get("persist.thermal.acpiconfig.xml", "0"))) {
            sSensorFilePath = BIOS_CONFIG_DIR_PATH + SENSOR_FILE_NAME;
            sThrottleFilePath = BIOS_CONFIG_DIR_PATH + THROTTLE_FILE_NAME;
            if (loadAcpiConfigXML()) {
                sIsConfigFiles = true;
                Log.i(TAG, "Reading thermal config files from acpi config files");
            } else {
                Log.i(TAG, "Could not get acpi config files");
            }
