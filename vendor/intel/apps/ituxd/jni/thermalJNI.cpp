...
static int getTRTDataFromDev(const char *path, char *buf, size_t size)
{
    int ret = 0, count = 0;
    if (!path || !buf) {
        return -1;
    ...
    ret = ioctl(fd, ACPI_THERMAL_REL_GET_TRT_LEN, &count);
    ALOGE("Expected %d bytes", count);
    if (count * TUPLE_SIZE > size) {
        close(fd);
        return -1;
    }
    ret = ioctl(fd, ACPI_THERMAL_REL_GET_TRT, buf);
    close(fd);
    if (ret < 0) {
        return -1;
    }
...
static jbyteArray getTRTData(JNIEnv* env, jobject obj, jstring jPath)
{
    int count = 0;
    const char *path = NULL;
    const int SIZE = 1024;
    ...
    count = getTRTDataFromDev(path, buf, SIZE);
    env->ReleaseStringUTFChars(jPath, path);
    if (count <= 0) {
        return NULL;
    }
    jbyteArray arr = env->NewByteArray(count);
    env->SetByteArrayRegion(arr, 0, count, (jbyte*)buf);
    return arr;
}
...
static JNINativeMethod sMethods[] = {
     /* name, signature, funcPtr */
        ...
        {"native_getTRTData", "(Ljava/lang/String;)[B", (void*)getTRTData},
