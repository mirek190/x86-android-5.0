...
static inline int pmic_chrg_set_cc(struct pmic_chrg_info *info, int cc)
{
	u8 reg_val;
	int ret;
	#if defined (CONFIG_WISKY_MACHINE_3G068I) || defined (CONFIG_WISKY_MACHINE_3G085I) || defined (CONFIG_WISKY_MACHINE_3G086I)
	cc = 2200;
	info->max_cc = 2200;
	#elif defined (CONFIG_WISKY_MACHINE_3G081I)
	cc = 2000;
	info->max_cc = 2000;
	#elif defined (CONFIG_WISKY_MACHINE_3G080IH) || defined (CONFIG_WISKY_MACHINE_3G080I) || defined (CONFIG_WISKY_MACHINE_3G062I)
	cc = 1800;
	info->max_cc = 2000;
	#elif defined (CONFIG_WISKY_MACHINE_3G069I)	|| defined (CONFIG_WISKY_MACHINE_H8I)
	cc = 1500;
	info->max_cc = 1800;
	#endif	
	dev_info(&info->pdev->dev, "%s: cc=%d , max_cc=%d\n", __func__, cc, info->max_cc);
	/* read CCCV register */
...
static void dc_xpwr_otg_event_worker(struct work_struct *work)
{
	...
	if (info->pdata->otg_gpio >= 0) {
		...
		if (ret < 0)
			dev_err(&info->pdev->dev, "VBUS ON/OFF FAILED: %d\n", ret);
	}
	mutex_unlock(&info->lock);
}
...
static int pmic_chrg_init_hw_regs(struct pmic_chrg_info *info)
{
	...
	ret = pmic_chrg_reg_writeb(info, DC_CHRG_VLTFC_REG, CHRG_VLTFC_0C);
	if (ret < 0) {
	...
	ret = pmic_chrg_reg_writeb(info, DC_CHRG_VHTFC_REG, CHRG_VHTFC_55C);
	if (ret < 0) {
	...
	/*set vhold to default value 4.0v to avoid PMIC over heat issue*/
	intel_mid_pmic_setb(DC_VBUS_ISPOUT_REG, VBUS_IPSOUT_VBUS_SET_4000MV);
