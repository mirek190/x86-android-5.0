...
//	#include "Dell_P801_5506_Toptocuh0xa0_Ver0x12_20131107_app.i"
//  #include "INTEL_5506_800x1280_20140227_app.i"
//  #include "INTEL_5406_1280X800_20140227_app.i"
};

static struct mutex g_device_mutex;

int ft5x0x_write_reg_ex(struct i2c_client *client, u8 regaddr, u8 regvalue)
{
...
int ft5x0x_read_reg_ex(struct i2c_client *client, u8 regaddr, u8 *regvalue)
{
...
int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	ft5x0x_write_reg_ex(client, 0, FTS_FACTORYMODE_VALUE);
	/*make sure already enter factory mode */
	msleep(100);
	/*write command to start calibration */
	ft5x0x_write_reg_ex(client, 2, 0x4);
	msleep(300);
	for (i = 0; i < 100; i++) {
		ft5x0x_read_reg_ex(client, 0, &uc_temp);
	...
	msleep(100);
	/*store CLB result */
	ft5x0x_write_reg_ex(client, 2, 0x5);
	msleep(300);
	/*return to normal mode */
	ft5x0x_write_reg_ex(client, 0, FTS_WORKMODE_VALUE);
	msleep(300);
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	*/
	ft5x0x_write_reg_ex(client, 0xfc, 0xaa);
	msleep(50);

	/*write 0x55 to register 0xfc */
	ft5x0x_write_reg_ex(client, 0xfc, 0x55);
	msleep(30);
	...
	do {
		...
		mdelay(5); /*change to avoid warning*/
	} while (i_ret <= 0 && i < 5);
	...
	dev_dbg(&client->dev,
		"[FTS] old setting: uc_i2c_addr = 0x%x, uc_io_voltage = %d",
			buf[0], buf[2]);
	dev_dbg(&client->dev, "uc_panel_factory_id = 0x%x\n",
			buf[4]);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	ft5x0x_read_reg_ex(client, FT5x0x_REG_FW_VER, &uc_tp_fm_ver);
			  ...
			  u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (DEVICE_IC_TYPE == IC_FT6208)
			ft5x0x_write_reg_ex(client, 0xbc, FT_UPGRADE_AA);
		else
			ft5x0x_write_reg_ex(client, 0xfc, FT_UPGRADE_AA);
		msleep(upgradeinfo.delay_aa);

		/*write 0x55 to register 0xfc */
		if (DEVICE_IC_TYPE == IC_FT6208)
			ft5x0x_write_reg_ex(client, 0xbc, FT_UPGRADE_55);
		else
			ft5x0x_write_reg_ex(client, 0xfc, FT_UPGRADE_55);
	...
	if (fts_ctpm_fw_read_app(client, pbt_buf, dw_lenth+6) < 0) {
		dev_err(&client->dev,
			"[FTS]--app from flash is not equal to app.bin\n");
		return -EIO;
	}
					...
					char *buf)
{
	...
	if (ft5x0x_read_reg_ex(client, FT5x0x_REG_FW_VER, &fwver) < 0)
					...
					const char *buf, size_t count)
{
	...
	if (2 == num_read_chars) {
		/*read register*/
		regaddr = wmreg;
		if (ft5x0x_read_reg_ex(client, regaddr, &regvalue) < 0)
		...
		regaddr = wmreg >> 8;
		regvalue = wmreg;
		if (ft5x0x_write_reg_ex(client, regaddr, regvalue) < 0)
	...
	const char __user *buff, unsigned long len, void *data)
{
	struct i2c_client *client =
		(struct i2c_client *)ft5x0x_proc_entry->data;
	...
	off_t off, int count, int *eof, void *data)
{
	...
	case PROC_UPGRADE:
		...
		regaddr = 0xA6;
		ret = ft5x0x_read_reg_ex(client, regaddr, &regvalue);
		if (ret < 0)
...
{ 
	fops.read = NULL;
        fops.write = NULL;
	ft5x0x_proc_entry = 
		proc_create_data(PROC_NAME, 0666, NULL, &fops, NULL);
//		create_proc_entry(PROC_NAME, 0666, NULL);
