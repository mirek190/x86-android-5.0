...
struct  hweq_s rt5645_hweq_param[] = {
	{/* NORMAL */
		{0},
		{0},
		0x0000,
...
#define RT5645_HWEQ_LEN ARRAY_SIZE(rt5645_hweq_param)
	...
	struct snd_soc_codec *codec, int channel, int mode)
{
	...
	case EQ_CH_ADC:
		...
		break;
	default:
		dev_dbg(codec->dev, "Invalid EQ channel\n");
		return -EINVAL;
	}
	snd_soc_update_bits(codec, reg, mask, rt5645_hweq_param[mode].ctrl);
