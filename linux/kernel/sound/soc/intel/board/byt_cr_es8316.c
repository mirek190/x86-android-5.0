...
/* #define NO_FW_SUPPORT_ES8316 */
#define ES8316_SPK_CON 151
...
static int byt_check_jack_type(void)
{
	...
	/* jd high indicates some accessory has been connected */
	if (status) {
		dev_dbg(codec->dev, "Jack insert intr");
		/* Do not process button events until accessory is
		   detected as headset*/
		ctx->process_button_events = false;
		byt_set_mic_bias(codec, true);
		/* read flag register to check headphone / headset type / or none */
		jack_type = es8316_headset_detect(codec);
		dev_dbg(codec->dev, "jack_type:%d\n", jack_type);
		gpio_set_value(ctx->es8316_spk_con, 0);
		ctx->hs_spk_work_mode = false;
...
static int byt_hs_detection(void)
{
	...
	/* Ack interrupt first */
	val = snd_soc_read(codec, ES8316_GPIO_FLAG);
...
static void byt_check_hs_remove_status(struct work_struct *work)
{
	...
	if (jack->status) {
		...
		if (!status) {
			...
			byt_set_mic_bias(codec, false);
			snd_soc_write(codec, 0x22, 0x30);
			gpio_set_value(ctx->es8316_spk_con, 1);
			...
			/* Jack is still connected. We may come here if
			   ...
			   headset */
			pr_info(" spurious Jack remove event for headset re-enable button events");
			ctx->process_button_events = true;
		}
...
static inline struct snd_soc_codec *byt_get_codec(struct snd_soc_card *card)
{
	...
	list_for_each_entry(codec, &card->codec_dev_list, card_list) {
		if (!strstr(codec->name, "es8316.2-0011")) {
		...
		struct snd_kcontrol *k, int  event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		vlv2_plat_set_clock_freq(VLV2_PLAT_CLK_AUDIO, 1); /* MCLK = 25MHZ */
		udelay(100);
		...
		pr_info("%d Jack_type detected = %d\n", __LINE__, ret);
	} else {
...
static int byt_compr_set_params(struct snd_compr_stream *cstream)
{
	...
	pr_debug("Enter:%s", __func__);
	/* Setecodec DAI confinuration */
	return byt_set_dai_fmt_pll(codec_dai, 0, 48000);
}
		...
		enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_ON:
		if (ctx->hs_spk_work_mode)
			gpio_set_value(ctx->es8316_spk_con, 1);
	case SND_SOC_BIAS_PREPARE:
	case SND_SOC_BIAS_STANDBY:
		if (ctx->hs_spk_work_mode == false)
			gpio_set_value(ctx->es8316_spk_con, 0);
	...
	pr_debug("card(%s)->bias_level %u  ctx->hs_spk_work_mode = %d\n", card->name,
			card->dapm.bias_level, ctx->hs_spk_work_mode);
	return 0;
}
		...
		struct snd_pcm_hw_params *params)
{
	...
	pr_info("BYT Comms Machine: slot_width = %d\n",
			slot_width);
	pr_info("BYT Comms Machine: tx_mask = %d\n",
			tx_mask);
	pr_info("BYT Comms Machine: rx_mask = %d\n",
			rx_mask);
	pr_info("BYT Comms Machine: tristate_offset = %d\n",
			tristate_offset);
...
static int byt_comms_dai_link_prepare(struct snd_pcm_substream *substream)
{
	...
	pr_info("%s substream->runtime->rate %d\n",
			__func__,
...
static int snd_byt_mc_probe(struct platform_device *pdev)
{
	...
	pmc_pc_set_freq(VLV2_PLAT_CLK_AUDIO, 1); /*19.2m*/
	udelay(100);
	...
	pr_info("%s: GPIOs - codec %d\n", __func__, codec_gpio);
	hs_gpio.gpio = codec_gpio;

#ifdef NO_FW_SUPPORT_ES8316
	drv->es8316_spk_con = ES8316_SPK_CON;
	gpio_direction_output(drv->es8316_spk_con, 1);
#else
	drv->es8316_spk_con = acpi_get_gpio_by_index(&pdev->dev, 2, NULL);
#endif

	drv->hs_spk_work_mode = true;
	pr_info("%s: GPIOs - codec_reset %d\n", __func__, drv->es8316_spk_con);

	spk_status = gpio_get_value(drv->es8316_spk_con);
...
static void add_device(void)
{
	...
	int ret;
	pdev = platform_device_alloc("byt_es8316", -1);
	if (!pdev) {
		pr_err("failed to allocate byt_es8316 platform device\n");
		return;
	}

	ret = platform_device_add(pdev);
	if (ret) {
		pr_err("failed to add byt_es8316 platform device\n");
...
static int __init snd_byt_driver_init(void)
{
	...
	if (ret)
		pr_err("Fail to register Baytrail Machine driver byt_es8316\n");
	else
		pr_info("Baytrail Machine Driver byt_es8316 registerd\n");
	return ret;
}
...
MODULE_ALIAS("platform:byt-es8316-audio");
