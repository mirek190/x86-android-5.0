...
#ifdef H350
        strncpy(config_name, "XMM6260_CONF_h", SFI_NAME_LEN - 1);
#else
...
#ifdef H350
    pr_info("minjie skip PMIC in retrieve_acpi_modem_data");
#else
	/* PMIC */
...
#ifdef H350
    pr_info("minjie skip EPWR in retrieve_acpi_modem_data");
#else
...
#ifdef H350
    pr_info("%s: zou juan cpu info setup\n", __func__);
    cpu_data->entries[0] = 125;
    cpu_data->entries[1] = 160;
    cpu_data->entries[2] = 158;
    cpu_data->entries[3] = 159;
    pr_info("zou juan set finished, entries[0]: %s, entries[1]: %s, entries[2]: %s, entries[3]: %s\n",cpu_data->entries[0], cpu_data->entries[1], cpu_data->entries[2], cpu_data->entries[3]);
#endif
...
#ifdef H350
        pr_info("minjie into mcd_finalize_cpu_data BOARD_AOB");
        cpu_data->entries[0] = 125;
        cpu_data->entries[1] = 160;
        cpu_data->entries[2] = 158;
        cpu_data->entries[3] = 159;
