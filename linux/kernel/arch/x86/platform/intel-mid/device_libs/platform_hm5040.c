/*
 * platform_hm5040.c: hm5040 platform data initilization file
 *
...
/*
 * hm5040 platform data
 */

static int hm5040_gpio_ctrl(struct v4l2_subdev *sd, int flag)
{
	int ret;
	int pin;
 ...
 * set MCLK failed.
 */
static int hm5040_flisclk_ctrl(struct v4l2_subdev *sd, int flag)
{
	...
	if (flag) {
		...
		if (ret)
			pr_err("hm5040 clock set failed.\n");
	}
	...
	return intel_scu_ipc_osc_clk(OSC_CLK_CAM0,
				     flag ? clock_khz : 0);
#else
	pr_err("hm5040 clock is not set.\n");
	return 0;
#endif
}
 ...
 */
static int hm5040_power_ctrl(struct v4l2_subdev *sd, int flag)
{
	int ret = 0;
...
#else
			pr_err("hm5040 power is not set.\n");
#endif
...
static int hm5040_csi_configure(struct v4l2_subdev *sd, int flag)
{
	return camera_sensor_csi(sd, ATOMISP_CAMERA_PORT_PRIMARY, 2,
		ATOMISP_INPUT_FORMAT_RAW_10, atomisp_bayer_order_bggr, flag);
}

static struct camera_sensor_platform_data hm5040_sensor_platform_data = {
	.gpio_ctrl	    = hm5040_gpio_ctrl,
	.flisclk_ctrl	= hm5040_flisclk_ctrl,
	.power_ctrl	    = hm5040_power_ctrl,
	.csi_cfg	    = hm5040_csi_configure,
};

void *hm5040_platform_data(void *info)
{
	camera_reset = -1;
	#ifdef CONFIG_MRD8
		camera_power_down = -1;
	...
	return &hm5040_sensor_platform_data;
}
