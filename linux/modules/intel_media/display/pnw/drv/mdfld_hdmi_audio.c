...
void mid_hdmi_audio_signal_event(struct drm_device *dev, enum had_event_type event)
{
	struct drm_psb_private *dev_priv = dev->dev_private;
	if (dev_priv->mdfld_had_event_callbacks)
		(*dev_priv->mdfld_had_event_callbacks)(event, dev_priv->had_pvt_data);
}
 ...
 *
 */
static int mid_hdmi_audio_rmw(uint32_t reg, uint32_t val, uint32_t mask)
{
	struct drm_device *dev = hdmi_priv->dev;
	int ret = 0;
	uint32_t val_tmp = 0;
 ...
 */
static int mid_hdmi_audio_get_caps(enum had_caps_list get_element, void *capabilities)
{
	struct drm_device *dev = hdmi_priv->dev;
	...
	struct hdmi_audio_query_set_ops *query_ops)
{
	...
	reg_ops->hdmi_audio_read_register = (mid_hdmi_audio_reg_ops.hdmi_audio_read_register);
	reg_ops->hdmi_audio_write_register = (mid_hdmi_audio_reg_ops.hdmi_audio_write_register);
	reg_ops->hdmi_audio_read_modify = (mid_hdmi_audio_reg_ops.hdmi_audio_read_modify);
	query_ops->hdmi_audio_get_caps = mid_hdmi_audio_get_set_ops.hdmi_audio_get_caps;
	query_ops->hdmi_audio_set_caps = mid_hdmi_audio_get_set_ops.hdmi_audio_set_caps;
...
int mid_hdmi_audio_register(struct snd_intel_had_interface *driver, void *had_data)
{
	struct drm_device *dev = hdmi_priv->dev;
	struct drm_psb_private *dev_priv = (struct drm_psb_private *) dev->dev_private;
