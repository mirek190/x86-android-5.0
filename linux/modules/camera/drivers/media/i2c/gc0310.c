...
#define GC0310_ANALOG_GAIN_2 87  // 1.36x
#define GC0310_ANALOG_GAIN_3 125  // 1.96x
#define GC0310_ANALOG_GAIN_4 167  // 2.62x
#define GC0310_ANALOG_GAIN_5 258  // 4.03x
#define GC0310_ANALOG_GAIN_6 335  // 5.23x
#define GC0310_ANALOG_GAIN_7 483  // 7.55x
...
static enum atomisp_bayer_order gc0310_bayer_order_mapping[] = {
...
*/
	...
	if (num_msg >= 0)
        ...
        dev_err(&client->dev, "write error: wrote 0x%x to reg 0x%x error %d",
                val, reg, num_msg);
...
static int gc0310_set_suspend(struct v4l2_subdev *sd)
{
	...
	gc0310_write_reg(client, MISENSOR_8BIT,  0x10, 0x86); /*10bit raw disable*/
...
static int gc0310_set_streaming(struct v4l2_subdev *sd)
{
	...
	gc0310_write_reg(client, MISENSOR_8BIT, 0xfe, 0x30);

	gc0310_write_reg(client, MISENSOR_8BIT,  0xfe, 0x03);
	gc0310_write_reg(client, MISENSOR_8BIT,  0x10, 0x96); /*10bit raw enable*/
	gc0310_write_reg(client, MISENSOR_8BIT,  0xfe, 0x00);
...
static int gc0310_init_common(struct v4l2_subdev *sd)
{
	...
	ret = gc0310_write_reg(client, MISENSOR_8BIT, 0xfe,0xf0);
	if (ret != 0)
		return ret;
	...
	gc0310_write_reg(client, MISENSOR_8BIT, 0xf7,0x33);//1f
	gc0310_write_reg(client, MISENSOR_8BIT, 0xf8,0x03);
	gc0310_write_reg(client, MISENSOR_8BIT, 0xf9,0x8e);
	...
	gc0310_write_reg(client, MISENSOR_8BIT, 0x0a,0x45);
	gc0310_write_reg(client, MISENSOR_8BIT, 0xfe,0x00);
	...
	gc0310_write_reg(client, MISENSOR_8BIT, 0x08,0x4e);
	...
	//////////////////	ISP reg   ///////////////////
	/////////////////////////////////////////////////
	gc0310_write_reg(client, MISENSOR_8BIT, 0x40,0x02);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x41,0x01);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x42,0x00);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x44,0x17);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x45,0x00);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x46,0x32); //sync

	gc0310_write_reg(client, MISENSOR_8BIT, 0x49,0x03);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x4c,0xa0);//2zhen sheng xiao
	gc0310_write_reg(client, MISENSOR_8BIT, 0x4d,0x01);
	...
	///////////////////   GAIN	 ////////////////////
	/////////////////////////////////////////////////
	gc0310_write_reg(client, MISENSOR_8BIT, 0x70,0x70);// global gain 1.2*1.36
	...
	gc0310_write_reg(client, MISENSOR_8BIT, 0x44,0x02);

	//////////////////////////////////////////////
	////////////////  DD  ////////////////////
	//////////////////////////////////////////////
	gc0310_write_reg(client, MISENSOR_8BIT, 0xfe,0x00);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x80,0xc1);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x84,0x10);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x86,0x50);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x87,0x30);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x88,0x15);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x89,0xf0);
	//////////////////////////////////////////////
	gc0310_write_reg(client, MISENSOR_8BIT, 0xdd,0x00);

	//////////////////////////////////////////////
	////////////////dark sun////////////////////
	//////////////////////////////////////////////
	gc0310_write_reg(client, MISENSOR_8BIT, 0xfe,0x01);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x45,0xf7);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x46,0xff);//sun vaule th
	gc0310_write_reg(client, MISENSOR_8BIT, 0x47,0x15);//sun vaule th
	...
	gc0310_write_reg(client, MISENSOR_8BIT, 0x04, 0x01); // fifo_prog
	...
	gc0310_write_reg(client, MISENSOR_8BIT, 0x29, 0x02);
	gc0310_write_reg(client, MISENSOR_8BIT, 0x2a, 0x02);
	gc0310_write_reg(client, MISENSOR_8BIT, 0xfe, 0x00);
...
static int gc0310_res2size(unsigned int res, int *h_size, int *v_size)
{
	...
	switch (res) {
#if 0
	case gc0310_RES_QCIF:
		hsize = gc0310_RES_QCIF_SIZE_H;
		vsize = gc0310_RES_QCIF_SIZE_V;
		break;
	case gc0310_RES_QVGA:
		hsize = gc0310_RES_QVGA_SIZE_H;
		vsize = gc0310_RES_QVGA_SIZE_V;
		break;
	case gc0310_RES_CIF:
				...
				const struct gc0310_res_struct *res)
{
	...
	val = 0;
	ret = gc0310_read_reg(client, MISENSOR_8BIT, REG_DUMMY_H, &reg_val);
	val = reg_val;
	val = (val & 0x0F) << 8;
	ret = gc0310_read_reg(client, MISENSOR_8BIT, REG_H_DUMMY_L, &reg_val);
	val += reg_val;
...
//	buf->line_length_pck = buf->output_width + val + 4;
	...
	val = 0;
	ret = gc0310_read_reg(client, MISENSOR_8BIT, REG_V_DUMMY_H, &reg_val);
	val = reg_val;
	val = (val & 0x0F) << 8;
	ret = gc0310_read_reg(client, MISENSOR_8BIT, REG_V_DUMMY_L, &reg_val);
	val += reg_val;
...
//	printk("%s:vt_pix_clk_freq_mhz = %d, line_length_pck = %d, frame_length_lines = %d\n ",
//		__func__, buf->vt_pix_clk_freq_mhz, buf->line_length_pck, buf->frame_length_lines);
	return 0;
}
				...
				struct v4l2_mbus_framefmt *fmt)
{
	...
	if (ret) {
		v4l2_err(c, "%s: gc0310 power down err", __func__);
		return ret;
	}

	ret = gc0310_s_power(sd, 1);
	if (ret) {
		v4l2_err(c, "%s: gc0310 power-up err", __func__);
		return ret;
	}*/
...
/*	case gc0310_RES_CIF:
		gc0310_write_reg(c, MISENSOR_8BIT,  0x15, 0x8A);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x62, 0xBD);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x63, 0x01);

		gc0310_write_reg(c, MISENSOR_8BIT,  0x06, 0x00);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x08, 0x05);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x09, 0x01);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x0A, 0xD0);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x0B, 0x02);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x0C, 0x40);
		break;*/
	case gc0310_RES_VGA:
/*		gc0310_write_reg(c, MISENSOR_8BIT,  0x15, 0x0A);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x62, 0x20);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x63, 0x03);

		gc0310_write_reg(c, MISENSOR_8BIT,  0x06, 0x00);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x08, 0x00); //05
		gc0310_write_reg(c, MISENSOR_8BIT,  0x09, 0x01); //484
		gc0310_write_reg(c, MISENSOR_8BIT,  0x0A, 0xE8);
		gc0310_write_reg(c, MISENSOR_8BIT,  0x0B, 0x02); //644
		gc0310_write_reg(c, MISENSOR_8BIT,  0x0C, 0x88);
*/
		break;
	default:
			       ...
			       struct atomisp_exposure *exposure)
{
	...
	iReg = exposure->gain[0];

	if(iReg < 0x40)
		iReg = 0x40;

	if((GC0310_ANALOG_GAIN_1<= iReg)&&(iReg < GC0310_ANALOG_GAIN_2))
	{
		//analog gain
		gc0310_write_reg(client, MISENSOR_8BIT, 0x48, 0x00);
		temp = iReg/2;
		gc0310_write_reg(client, MISENSOR_8BIT, 0x71, temp); // pregain
	}
	else if((GC0310_ANALOG_GAIN_2<= iReg)&&(iReg < GC0310_ANALOG_GAIN_3))
	{
		//analog gain
		gc0310_write_reg(client, MISENSOR_8BIT, 0x48, 0x01);
		temp = 32*iReg/GC0310_ANALOG_GAIN_2;
		gc0310_write_reg(client, MISENSOR_8BIT, 0x71, temp); //  pregain
	}

	else if((GC0310_ANALOG_GAIN_3<= iReg)&&(iReg < GC0310_ANALOG_GAIN_4))
	{
		//analog gain
		gc0310_write_reg(client, MISENSOR_8BIT, 0x48, 0x02);//
		temp = 32*iReg/GC0310_ANALOG_GAIN_3;
		gc0310_write_reg(client, MISENSOR_8BIT, 0x71, temp); // pregain
	}
	else if(GC0310_ANALOG_GAIN_4<= iReg)
	{
		//analog gain
		gc0310_write_reg(client, MISENSOR_8BIT, 0x48, 0x03);//
		temp = 32*iReg/GC0310_ANALOG_GAIN_4;
		gc0310_write_reg(client, MISENSOR_8BIT, 0x71, temp); // pregain
	}
...
static int gc0310_detect(struct gc0310_device *dev, struct i2c_client *client)
{
	...
	gc0310_read_reg(client, MISENSOR_8BIT, (u8)gc0310_PID, &retvalue);
	dev->real_model_id = retvalue;
	printk("detect module ID = %x\n", retvalue);

	if (retvalue != gc0310_MOD_ID) {
...
gc0310_translate_bayer_order(enum atomisp_bayer_order code)
{
	switch (code) {
...
static int gc0310_t_hflip(struct v4l2_subdev *sd, int value)
{
	...
	if (value){
		val |= GC0310_HFLIP_BIT;
	} else {
		val &= ~GC0310_HFLIP_BIT;
	}
	ret = gc0310_write_reg(client, MISENSOR_8BIT,
		GC0310_IMG_ORIENTATION, val);
	if (ret)
		return ret;
	g_flip = val;
	gc0310_info = v4l2_get_subdev_hostdata(sd);
	if (gc0310_info) {
		val &= (GC0310_VFLIP_BIT|GC0310_HFLIP_BIT);
		if (val == 0x00)
			ret = gc0310_write_reg(client, MISENSOR_8BIT, 0x46, 0x32);
		else if (val == 0x01)
			ret = gc0310_write_reg(client, MISENSOR_8BIT, 0x46, 0x12);
		else if (val == 0x02)
			ret = gc0310_write_reg(client, MISENSOR_8BIT, 0x46, 0x22);
		else
			ret = gc0310_write_reg(client, MISENSOR_8BIT, 0x46, 0x02);
		if (ret)
			return ret;

		gc0310_info->raw_bayer_order = gc0310_bayer_order_mapping[val];
		dev->format.code = gc0310_translate_bayer_order(
		gc0310_info->raw_bayer_order);
	}
