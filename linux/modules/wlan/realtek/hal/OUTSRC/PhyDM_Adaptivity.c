...
)
{
	...
	ODM_RT_TRACE(pDM_Odm,PHYDM_COMP_ADAPTIVITY, ODM_DBG_LOUD, ("IGI=0x%x, TH_L2H_dmc = %d, TH_H2L_dmc = %d, adaptivity_flg = %d, bAdaOn = %d, DynamicLinkAdaptivity = %d\n", 
...
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	PADAPTER		pAdapter = pDM_Odm->Adapter;
	u8Byte 			diffTime = 0, curTime = 0, oldestTime = 0;
	u1Byte			queueIdx, delayCount;
	...
	*/
	...
	if(pDM_Odm->SupportAbility & ODM_BB_ADAPTIVITY)
	{
		RT_TRACE(COMP_INIT, DBG_WARNING, ("==> Phydm_AdaptivityBSOD()\n"));
		...
		}while((diffTime/1000000) >= 4 && oldestTime != ~0);
