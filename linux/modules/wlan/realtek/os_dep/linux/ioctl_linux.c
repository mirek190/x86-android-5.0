...

{
	...
	#else
	iwe.u.qual.level = (u8)odm_SignalScaleMapping(&(GET_HAL_DATA(padapter)->odmpriv), ss);
	#endif
	...
	if ( strncmp(extra, "dlfw", 4) == 0)
	{
		...
		if( (pMptCtx->mptOutBuf[4]==0x00) && (pMptCtx->mptOutBuf[5]==0x00))
			...
			else
			{
				...
				rtw_write16(padapter, 0x948, 0x0280);
				goto exit;
			}
		goto exit;
	}
...
static struct iw_statistics *rtw_get_wireless_stats(struct net_device *dev)
{
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED) != _TRUE)
	{
		...
		#else
		tmp_level = (u8)odm_SignalScaleMapping(&GET_HAL_DATA(padapter), padapter->recvpriv.signal_strength);	
		#endif
