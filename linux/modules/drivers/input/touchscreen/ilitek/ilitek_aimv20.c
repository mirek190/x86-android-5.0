...
//static struct acpi_device_id ilitek_acpi_match[] = {
//	{ "ATML1000", 0 },
//	{ },
//};
...
//MODULE_DEVICE_TABLE(acpi, ilitek_acpi_match);
...
static struct i2c_driver ilitek_i2c_driver = {
	...
	.driver = {.name = ILITEK_I2C_DRIVER_NAME,
			   .owner = THIS_MODULE,
//			   .acpi_match_table = ACPI_PTR(ilitek_acpi_match),
			   },
	...
	const struct i2c_device_id *id)
{
	...
	printk("%s, ACPI IRQ: %d\n", __func__, client->irq);
//	i2c.client->addr = 0x41;
//	i2c.client->irq = 389;
//	printk("%s, New manual IRQ: %d\n", __func__, client->irq);
	printk(ILITEK_DEBUG_LEVEL "%s, client.addr: 0x%X\n", __func__, (unsigned int)i2c.client->addr);
		...
		if(ret==1)	printk("ilitek Do not need update\n"); 
		else if(ret==2) printk("ilitek update end\n");
		else if(ret==3) printk("ilitek i2c communication error\n");
		else if(ret==4) printk("ilitek unsupported panel to update FW\n");
		//shawn
        if(i2c.reset_request_success){
            ilitek_reset(i2c.reset_gpio);
        }
...
static int ilitek_read_ic_type(void){
	int ret=0;
	unsigned char buffer[128]={0};
//	struct i2c_msg msgs[] = {
...
//	msgs[0].len = 1;
//	ret = ilitek_i2c_transfer(i2c.client, msgs, 1);
	ret = ilitek_i2c_read_info(i2c.client,cmd,buffer,5);
	if(ret < 0){
        printk(ILITEK_DEBUG_LEVEL "%s, read ic type , ret is %d\n", __func__,ret);
        return ret;
    }
	printk(ILITEK_DEBUG_LEVEL "%s, read value is %d.%d\n", __func__, buffer[0], buffer[1]);
    if((0x05 == buffer[0]) && (0x13 == buffer[1])){
        printk(ILITEK_DEBUG_LEVEL "%s, this ic is 2139\n", __func__);
		return CHIP_ILITEK_2139;
    }
    if((0x05 == buffer[0]) && (0x81 == buffer[1])){
        printk(ILITEK_DEBUG_LEVEL "%s, this ic is 2839\n", __func__);
		return CHIP_ILITEK_2839;
    }
    if((0x03 == buffer[0]) && (0x22 == buffer[1])){
        printk(ILITEK_DEBUG_LEVEL "%s, this ic is 2303\n", __func__);
		return CHIP_ILITEK_2303;
    }
...
static int ilitek_upgrade_firmware(void)
{
	...
	switch (ilitek_read_ic_type()) {
		case CHIP_ILITEK_2139:
			CTPM_FW = CTPM_FW2139;
			break;
		case CHIP_ILITEK_2839:
			CTPM_FW = CTPM_FW2839;
			break;
		case CHIP_ILITEK_2303:
			CTPM_FW = CTPM_FW2303;
			break;
		default:
			return 4;
	...
	printk("ilitek ap_startaddr=0x%d,ap_endaddr=0x%d,ap_checksum=0x%d\n",ap_startaddr,ap_endaddr,ap_checksum);
	printk("ilitek df_startaddr=0x%d,df_endaddr=0x%d,df_checksum=0x%d\n",df_startaddr,df_endaddr,df_checksum);	
	buffer[0]=0xc0;
	msgs[0].len = 1;
...
struct ilitek_platform_data ilitek_data = {
    .irqflags = IRQF_TRIGGER_FALLING,
	.irq_gpio  = IRQ_GPIO,//133 on CR
	.reset = RESET_GPIO,//128 on CR
};

static struct i2c_board_info __initdata ilitek_i2c_device = {
		I2C_BOARD_INFO(ILITEK_I2C_DRIVER_NAME, ILITEK_I2C_ADDRESS),
		.platform_data = &ilitek_data,
};
...
static int __init ilitek_platform_init(void)
{
	...
	int i2c_busnum = 4;/*4 on CR */

	printk(ILITEK_DEBUG_LEVEL "%s:\n", __func__);

	ilitek_i2c_device.irq = gpio_to_irq(ilitek_data.irq_gpio);

	return i2c_register_board_info(i2c_busnum, &ilitek_i2c_device, 1);
}

fs_initcall(ilitek_platform_init);
