...
#define LIB_ARGS_PORT_STATUS_PROPERTY   "rild.port_status"
...
/*
* For some AT command (such as  +CGMR ...) in some modems 
* */
...
static void processLine(const char *line)
{
    ...
    if (sp_response == NULL) {
        ...
        case SINGLELINE:
          /*
                   handle  line without prefix 
          */
            if(sp_response->p_intermediates == NULL && isNoPrefixAT( line))  {
                addIntermediate(line);
            }
            else if (sp_response->p_intermediates == NULL
            ...
            break;
        case MULTILINE:
            ALOGD("MULTILINE,line:%s,s_responsePrefix:%s",line,s_responsePrefix);
            if ((strStartsWith (line, s_responsePrefix))
                ||((0 == strncmp(s_responsePrefix, "+CMGR",strlen("+CMGR"))) && !strStartsWith (line, "OK"))) {
                addIntermediate(line);
            } else {
                handleUnsolicited(line);
            }
                                        ...
                                        long long timeoutMsec, ATResponse **pp_outResponse)
{
    ...
    //RLOGD("at_send_command_full_nolock,responsePrefix:%s,s_responsePrefix:%s",responsePrefix,s_responsePrefix);
    s_smsPDU = smspdu;
...
{
    int err;
    RLOGD("at_send_command_multiline,responsePrefix:%s",responsePrefix);
