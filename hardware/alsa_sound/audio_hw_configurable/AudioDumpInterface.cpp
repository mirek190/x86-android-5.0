...
ssize_t AudioStreamOutDump::write(const void* buffer, size_t bytes)
{
    ...
    if(!mFile) {
        if (mInterface->fileName() != "") {
            char name[255];
            // A new dump file
            sprintf(name, "/data/%s_out_%d_%d.pcm", mInterface->fileName().string(), mId, ++mFileCount);
			mFile = fopen(name, "wb+");
			if (mFile == NULL)
				LOGE("Cannot open file %s, errno %d, reason: %s", name, errno, strerror(errno));

            LOGV("Output stream dump file %s, fh %p opened", name, mFile);
        }
    }

    // Check that file size does not exceed 50 MB
    if (mFile && fstat(fileno(mFile), &stDump) == 0 && (stDump.st_size + bytes) < MAX_DUMP_FILE_SIZE) {
        if (fwrite(buffer, bytes, 1, mFile) !=1)
            LOGV("Error writing PCM in audio output stream dump file : %s", strerror(errno));
    } else if (mFileCount >= 2) {
        // Roll on 2 files, to keep at least the last 50MB of dumps
        char fileToRemove[255];
        sprintf(fileToRemove, "/data/%s_out_%d_%d.pcm", mInterface->fileName().string(), mId, mFileCount-1);
        remove(fileToRemove);
        Close();
    }
    else
...
status_t AudioStreamOutDump::standby()
{
    ...
    // Don't close the audio dump file of the stream when it goes into standby,
    // so that the dump file is used for the whole audio hardware session.
    // Close();
...
ssize_t AudioStreamInDump::read(void* buffer, ssize_t bytes)
{
    ...
    if (mFinalStream) {
        ...
        if(!mFile) {
            if (mInterface->fileName() != "") {
                char name[255];
                // A new dump file is created for each stream Id (mId)
                sprintf(name, "/data/%s_in_%d_%d.pcm", mInterface->fileName().string(), mId, ++mFileCount);
                mFile = fopen(name, "wb");
				if (mFile == NULL)
					LOGE("Cannot open file %s, errno %d, reason: %s", name, errno, strerror(errno));
                LOGV("Input stream dump file %s, fh %p opened.", name, mFile);
            }
        }
        ...
        if (mFile && fstat(fileno(mFile), &stDump) == 0 && (stDump.st_size + bytes) < MAX_DUMP_FILE_SIZE) {
            if (fwrite(buffer, bytes, 1, mFile) !=1)
                LOGV("Error writing PCM in audio input stream dump file : %s", strerror(errno));
        } else if (mFileCount >= 2) {
            // Roll on 2 files, to keep at least the last 50MB of dumps
            char fileToRemove[255];
            sprintf(fileToRemove, "/data/%s_in_%d_%d.pcm", mInterface->fileName().string(), mId, mFileCount-1);
            remove(fileToRemove);
            Close();
        }
        else
