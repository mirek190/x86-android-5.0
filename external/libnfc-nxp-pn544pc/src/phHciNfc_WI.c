...
/*  WI gate specific Commands definition */
#define NXP_SMX_GET_KS                  (0x18U)
...
#define NXP_SMX_DEFAULTMODE_INDEX       (0x01)
#define NXP_SMX_PRESENCE_INDEX          (0x02)
#define NXP_SMX_EVENTS_INDEX            (0x05)

/* Set Bit 0 and Bit 1 to report Start of transaction and End of transaction*/
#define WI_ENABLE_EVENTS                (NXP_NFC_SMX_TRANSACTION_EVENTS)
                          ...
                          )
{
    ...
    else
    {
        ...
        {
            p_pipe_info->reg_index = NXP_SMX_DEFAULTMODE_INDEX;
                              ...
                              )
{
    ...
    else
    {
        ...
        {
            status = phHciNfc_Send_WI_Command(psHciContext,pHwRef,
                                            p_wi_info->pipe_id,
                                            (uint8_t)NXP_SMX_GET_KS);
        }/* End of else part*/
    }
    return status;
}
                              ...
                              )
{
    ...
    else
    {
        ...
        {
            p_wi_info->p_default_mode = p_mode;
                              ...
                              )
{
    ...
    else
    {
        ...
        else
        {
            ...
            p_pipe_info->reg_index = NXP_SMX_PRESENCE_INDEX;
                                ...
                                )
{
    ...
    else
    {
        ...
        else
        {
            ...
            p_pipe_info->reg_index = NXP_SMX_EVENTS_INDEX;
            p_pipe_info->param_info =(void*)&param ;
            p_pipe_info->param_length = sizeof(param) ;
                       ...
                       )
{
    ...
    else
    {
        ...
        if( NXP_SMX_GET_KS == command)
        {
            hcp_packet = (phHciNfc_HCP_Packet_t *) psHciContext->send_buffer;
            /* Construct the HCP Frame */
            phHciNfc_Build_HCPFrame(hcp_packet,HCP_CHAINBIT_DEFAULT,
                                    (uint8_t) pipe_id, HCP_MSG_TYPE_COMMAND, command);
            hcp_message = &(hcp_packet->msg.message);
            phHciNfc_Append_HCPFrame((uint8_t *)hcp_message->payload,
                                            i, (uint8_t *)p_pipe_info->param_info,
                                            p_pipe_info->param_length);
        ...
        {
            status = PHNFCSTVAL( CID_NFC_HCI, NFCSTATUS_INVALID_HCI_INSTRUCTION );
            HCI_DEBUG("%s: Invalid Send Command Request \n","phHciNfc_Send_WI_command");
        }

        if( NFCSTATUS_SUCCESS == status )
        {
            p_pipe_info->sent_msg_type = HCP_MSG_TYPE_COMMAND;
            p_pipe_info->prev_msg = command;
            psHciContext->tx_total = length;
                          ...
                          )
{
    ...
    else
    {
        ...
        {
            prev_cmd = p_wi_info->p_pipe_info->prev_msg ;
            switch(prev_cmd)
            {
                case ANY_GET_PARAMETER:
                {
                    if (length > HCP_HEADER_LEN)
                    {
                        status = phHciNfc_WI_InfoUpdate (psHciContext,
                                        p_wi_info->p_pipe_info->reg_index,
                ...
                case NXP_SMX_GET_KS:
                {
                    HCI_PRINT("WI Get Key Session Complete\n");
                    status = phHciNfc_EmuMgmt_Update_Seq(psHciContext,
                                                        UPDATE_SEQ);
                    break;
                }
            ...
            if( NFCSTATUS_SUCCESS == status )
            {
                p_wi_info->p_pipe_info->prev_status = NFCSTATUS_SUCCESS;
                p_wi_info->current_seq = p_wi_info->next_seq;
            }
        }
    }
...
{
    NFCSTATUS                   status = NFCSTATUS_SUCCESS;
    phHal_sEventInfo_t          EventInfo = {phHal_eHostController,
    ...
    else
    {
        ...
        EventInfo.eventInfo.aid.buffer = NULL;
        EventInfo.eventInfo.aid.length = 0;
        ...
        switch(EventType)
        {
            ...
            case NXP_EVT_SE_TRANSACTION:
            {
                ...
                if((length > HCP_HEADER_LEN)
                      && (MAX_AID_LEN >= ( length - HCP_HEADER_LEN )))
                {
                ...
                {
                    if (!phOsalNfc_MemCompare(paypass_removal, EventInfo.eventInfo.aid.buffer, EventInfo.eventInfo.aid.length))
                    {
                       ...
                       )
{
    ...
    p_wi_info = psHciContext->p_wi_info;

    if ((NXP_SMX_DEFAULTMODE_INDEX == index) &&
        (sizeof(*reg_value) == reg_length))
    {
        p_wi_info->default_type = *reg_value;
        if ( NULL != p_wi_info->p_default_mode)
            *p_wi_info->p_default_mode = p_wi_info->default_type;
    }
#if ( ES_HW_VER >= 33 )
    else if ((NXP_SMX_PRESENCE_INDEX == index) &&
        (sizeof(*reg_value) == reg_length))
    {
        p_wi_info->default_type = *reg_value;
        psHciContext->p_hw_ref->smx_connected = !(!(*reg_value));
        psHciContext->p_hw_ref->se_if_info.wi_link_status =
                                                 !(!(*reg_value));
    }
#endif  /* #if ( ES_HW_VER >= 33 ) */
    else
