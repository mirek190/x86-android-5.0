...
typedef struct phHciNfc_AdminGate_Info{
    ...
    uint8_t                         max_pipe;
    /* Pipe Creation Index to verify the Pipe Creation Sequence */
    uint8_t                         pipe_create_index;
    ...
    else
    {
        ...
        else
        {
            switch(p_admin_info->current_seq)
            {
...
#elif defined (UICC_SESSION_RESET)
                ...
                {
//HECI Mod
        // NOTE : Intel ME will fake clear all pipes.
        // When Session ID match with Driver session ID, the pipe are
        // not recreate, but in libnfc pipe ID are defined in a static way.
        // With ME, Pipe IDs are the same values as the static values of libnfc.
        // So we recreate the pipes to force libnfc to gather correct value.
        // But internaly, the ME does not recreate pipes.
...
//                            {
//                                p_admin_info->next_seq = ADMIN_GET_MAX_PIPE;
...
// END HECI Mod
                ...
                {
                    static uint8_t uicc_session_index = 0;
                    uint8_t config = 0xFF;
                    p_pipe_info = p_admin_info->admin_pipe_info;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            (NFC_ADDRESS_UICC_SESSION + uicc_session_index) , config , NULL);
                    uicc_session_index++;
                    if((NFCSTATUS_PENDING == status )
                        && (SESSIONID_SIZE <= uicc_session_index )
                        )
                    {
                        uicc_session_index = 0;
                        p_admin_info->next_seq = ADMIN_CLEAR_PIPES;
                    }
                    break;
                }
                ...
                case ADMIN_CREATE_PIPES:
                {
                    ...
                    if(status == NFCSTATUS_SUCCESS)
                    {
                        p_admin_info->next_seq = ADMIN_CONTINUE_CREATE_PIPES;
                    }
                    break;
                }
                case ADMIN_CONTINUE_CREATE_PIPES:
                {
                    status = phHciNfc_Create_All_Pipes( psHciContext, pHwRef,
                                                        &p_admin_info->pipe_seq );
                    if(status == NFCSTATUS_SUCCESS)
            ...
            switch(prev_cmd)
            {
                ...
                case ADM_CREATE_PIPE:
                {
...
#ifdef  NXP_NFC_VERIFY_CREATE_PIPE_INDEX
                    if(p_admin_info->pipe_create_index != pipe_id)
                    {
                    ...
                    if(NFCSTATUS_SUCCESS == status )
                    {
                        ...
                        p_admin_info->pipe_create_index++;
                    }
                    break;
                }
                case ADM_DELETE_PIPE:
                ...
                case ADM_CLEAR_ALL_PIPE:
                {
                    p_admin_info->pipe_create_index = HCI_DYNAMIC_PIPE_ID;
                    break;
                }
                default:
                {
    ...
    else
    {
        ...
        switch( cmd )
        {
            ...
            case ADM_NOTIFY_PIPE_CREATED:
            {
                pipe_id = hcp_message->payload[RESPONSE_PIPEID_OFFSET];
                if( ( NULL == p_pipe_info )
                    && (phHciNfc_Allocate_Resource((void **)(&p_pipe_info),
                    sizeof(phHciNfc_Pipe_Info_t))== NFCSTATUS_SUCCESS)
                    )
                {
