                                ...
                                )
{
    ...
    else if(!gpphLibContext->CE_Info.Active )
    {
#endif
        if( (NULL!=gpphLibContext->sNfcIp_Context.pClientNfcIpRxCb)
        ...
        if ((LLCP_STATE_RESET_INIT != gpphLibContext->llcp_cntx.sLlcpContext.state)
    ...
    {
        if( NULL != gpphLibContext->CE_Info.pCeRxCb )
        {
            RetVal = NFCSTATUS_REJECTED;
        }
    }
    ...
    if(RetVal == NFCSTATUS_SUCCESS)
    {
...
#ifdef HOST_EMULATION
            gpphLibContext->CE_Info.pCeRxCb = NULL;
#endif
        ...
        else
        {
            ...
            PHDBG_INFO("LibNfc: Receive In Progress");
        ...
        {
#ifdef HOST_EMULATION
            if(gpphLibContext->CE_Info.Active)
            {
                gpphLibContext->CE_Info.pCeRxCb = pReceiveRspCb;
                gpphLibContext->CE_Info.pCeRxCntx = pContext;
                                    ...
                                    )
{
    ...
    }else if(eLibNfcHalStateShutdown == gpphLibContext->LibNfcState.next_state)
    ...
    {
        pClientCb = gpphLibContext->CE_Info.pCeRxCb;
        pUpperLayerContext = gpphLibContext->CE_Info.pCeRxCntx;

        gpphLibContext->CE_Info.pCeRxCb = NULL;
        gpphLibContext->CE_Info.pCeRxCntx = NULL;
                        ...
                        )
{
    ...
    if(RetVal == NFCSTATUS_SUCCESS)
    {
        ...
        if(NFCSTATUS_PENDING == RetVal)
        {
            ...
            if(gpphLibContext->CE_Info.Active)
            {
                gpphLibContext->CE_Info.pCeTxCb = pSendRspCb;
                gpphLibContext->CE_Info.pCeTxCntx = pContext;
            }
            else
#endif
            {
                            ...
                            )
{
    ...
    else
    {
       ...
       {
            pClientCb = gpphLibContext->CE_Info.pCeTxCb;
            pUpperLayerContext = gpphLibContext->CE_Info.pCeTxCntx;
            gpphLibContext->CE_Info.pCeTxCb = NULL;
            gpphLibContext->CE_Info.pCeTxCntx = NULL;
