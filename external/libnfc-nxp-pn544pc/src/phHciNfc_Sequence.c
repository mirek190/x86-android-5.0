...
// HCI WRAPPER
extern NFCSTATUS phHciNfc_ReleaseHciWrapperStack(phHciNfc_sContext_t *psHciContext,void *pHwRef);
// END HCI WRAPPER
static
NFCSTATUS
 ...
 * \ingroup grp_hci_nfc
 *
 *  The phHciNfc_MultiEmuCfg_Sequence function sequence configures the
 *  device to support Multiple Secure Elements Enabled at the same time.
 *
 *  \param[in]  psHciContext            psHciContext is the context of
 *                                      the HCI Layer.
 *  \param[in]  pHwRef                  pHwRef is the Information of
 *                                      the Device Interface Link .
 *
 *  \retval NFCSTATUS_SUCCESS           HCI Multi Emulation Configuration
 *                                      sequence successful.
 *  \retval NFCSTATUS_INVALID_PARAMETER One or more of the supplied parameters
 *                                      could not be interpreted properly.
 *  \retval Other errors                    Other related errors
                          ...
                          )
{
    ...
    if(TRUE == psHciContext->target_release)
    {
        ...
        status = phHciNfc_ReaderMgmt_Deselect(
            psHciContext, pHwRef, phHal_eISO14443_A_PICC, NFC_RF_DISCOVERY_REPOLL);
        ...
        switch(psHciContext->hci_state.next_state)
        {
            ...
            case hciState_Config:
            {
                if ((hciState_Disconnect == psHciContext->hci_state.cur_state)
                      && (SMX_WI_MODE == psHciContext->config_type)
                   )
                {
                    /* Continue if the SmartMX configure Mode fails */
            ...
            case hciState_Select:
            {
                ...
                {
                    status = phHciNfc_ReaderMgmt_Deselect(
                        psHciContext, pHwRef, phHal_eISO14443_A_PICC, NFC_RF_DISCOVERY_CONTINUE);
                }
                phHciNfc_FSM_Rollback(psHciContext);
#endif
                break;
        ...
        case hciState_Initialise:
        {
            switch (psHciContext->hci_state.cur_state)
            {
                ...
                case hciState_Initialise:
                {
                    if( TRUE == psHciContext->target_release )
                    {
                        HCI_PRINT(" Release the Pending Target Release \n");
                        status = phHciNfc_Pending_Sequence(psHciContext, pHwRef );
                    }
                }
                /* fall through */
                             ...
                             )
{
    ...
    switch(psHciContext->hci_seq)
    {
...
#else
                        psHciContext->hci_seq = IDENTITY_FULL_INFO_SEQ;
        ...
        case IDENTITY_INFO_SEQ:
        {
            ...
            if(NFCSTATUS_SUCCESS == status)
            {
                psHciContext->hci_seq = ADMIN_INIT_CONTINUE_SEQ;
            }
            break;
        }
        case ADMIN_INIT_CONTINUE_SEQ:
        {
            status = phHciNfc_Admin_Initialise( psHciContext,pHwRef );
            if(NFCSTATUS_SUCCESS == status)
            {
                ...
                {
                    psHciContext->hci_seq = DEV_HW_ADDR_SEQ;
                }
                else
                {
                    psHciContext->hci_seq = IDENTITY_FULL_INFO_SEQ;
        ...
        case DEV_HW_ADDR_SEQ:
        {
            phHal_sHwConfig_t   *p_hw_config = ((phHal_sHwConfig_t *)
                            psHciContext->p_config_params);
            if ( ZERO != p_hw_config->addr_table.hwAddrEntries)
            {
                status = phHciNfc_DevMgmt_Update_Config( psHciContext, pHwRef,
                           NFC_HW_TABLE_CONFIG,
                                (void *)(&p_hw_config->addr_table) );
                if(NFCSTATUS_SUCCESS == status )
                {
            ...
            {
               /* Intentionally fall through to the next sequence */
               psHciContext->hci_seq = READER_MGMT_INIT_SEQ;
            }
        }
        /* fall through */
        ...
        case EMULATION_INIT_SEQ:
        {
            ...
            if(NFCSTATUS_SUCCESS == status)
            {
                ...
                if(NFCSTATUS_SUCCESS == info_status)
                {
                    psHciContext->hci_seq = EMULATION_INFO_SEQ;
                }
#endif /* #ifdef ESTABLISH_SESSION */
            }
            break;
        ...
        case IDENTITY_FULL_INFO_SEQ:
        {
            ...
            if(NFCSTATUS_SUCCESS == status)
            {
                    ...
                    && (HCI_NFC_DEVICE_TEST != psHciContext->init_mode))
                {
                    ...
                    {
                        // HECI MOD
                        // JG: Patch to remove SWP support
        ...
        case EMULATION_INFO_SEQ:
        {
            status = phHciNfc_EmuMgmt_Info_Sequence( psHciContext, pHwRef );
            if(NFCSTATUS_SUCCESS == status)
            {
                    psHciContext->hci_seq = HCI_END_SEQ;
            }
...
mem_index = 0;
        ...
        case HCI_END_SEQ:
        {
...
#if defined( NXP_PROD_TEST_CHECK )
                && (HCI_SELF_TEST != psHciContext->init_mode )
#endif
                )
            {
            ...
            if(NFCSTATUS_SUCCESS == status)
            {
                ...
                (void) phHciNfc_Uicc_Hiz_State ( psHciContext,
                        pHwReference, &pHwReference->se_if_info.swp_hiz_state);
                 phHciNfc_Notify(psHciContext->p_upper_notify,
                        psHciContext->p_upper_context, pHwRef,
    ...
    switch(psHciContext->hci_seq)
    {
        ...
        case HCI_END_SEQ:
        {
            ...
            // status = plower_if->release((void *)plower_if->pcontext,
                                            //(void *)pHwRef);
            phHciNfc_ReleaseHciWrapperStack(psHciContext,pHwRef);
            // END HCI WRAPPER
            phHciNfc_Release_Resources( &psHciContext );
    ...
    switch(psHciContext->config_type)
    {
...
#if 0
        {
            ...
            if (NULL != p_emulation_cfg)
            {
                ...
                {
                    status = phHciNfc_Uicc_Connect_Status(psHciContext,pHwRef, TRUE );
                    if( NFCSTATUS_PENDING == status )
                    {
                        break;
        ...
        case UICC_STATUS_INFO:
        {
            ...
            if (NULL != p_emulation_cfg)
            {
                ...
                if( TRUE == uicc_config->enableUicc )
                {
                    status = phHciNfc_Uicc_Connect_Status(psHciContext,pHwRef, FALSE );
                    if( NFCSTATUS_PENDING == status )
                    {
                        break;
                    } /* Or Else Fall through to notify the above layer */
        ...
        case SE_MULTI_EMU_CFG:
        {
            status = phHciNfc_MultiEmuCfg_Sequence(psHciContext, pHwRef);
            break ;
        }
        default:
        {
                           ...
                           )
{
    ...
    if (NULL != p_poll_config)
    {
        ...
        switch(psHciContext->hci_seq)
        {
#if (NXP_HAL_PL_STATE_DEBUG & 0x01U )
            case PL_RF_STATE_DBG_SEQ:
            {
                status = phHciNfc_DevMgmt_Info_Sequence( psHciContext, pHwRef );
                if(NFCSTATUS_SUCCESS == status)
                {
            ...
            case PL_DURATION_SEQ:
            {
                ...
                if(NFCSTATUS_SUCCESS == status)
                {
#if defined (ENABLE_P2P) && (NXP_NFC_TGT_SPEED_CONFIG >= 0x01)
                    if (0 != p_poll_config->NfcIP_Tgt_Mode_Config)
                    {
                        psHciContext->hci_seq =  TARGET_SPEED_SEQ;
                    }
                    else
                    {
                        psHciContext->hci_seq = (((~(PL_RD_PHASES_DISABLE)) &
                            p_poll_config->PollDevInfo.PollEnabled)!= 0)? INITIATOR_SPEED_SEQ :
#if defined (NFCIP_TGT_DISABLE_CFG)
                            PL_TGT_DISABLE_SEQ
#else
                            PL_CONFIG_PHASE_SEQ
#endif /* #if defined (NFCIP_TGT_DISABLE_CFG) */
                        ;
                    }
#elif defined (ENABLE_P2P) && (NXP_NFC_INITIATOR_SPEED_CONFIG >= 0x01)
                    psHciContext->hci_seq = (((~(PL_RD_PHASES_DISABLE)) &
                        p_poll_config->PollDevInfo.PollEnabled)!= 0)? INITIATOR_SPEED_SEQ :
#if defined (NFCIP_TGT_DISABLE_CFG)
                    PL_TGT_DISABLE_SEQ
...
#elif (NXP_NFC_POLL_FELICA_SYSCODE >= 0x01)
                    psHciContext->hci_seq = PL_FELICA_SYSCODE_SEQ;
#else
...
#define NFCIP_PASSIVE_MASK  0x07U
                ...
                if (0 != p_poll_config->NfcIP_Tgt_Mode_Config)
                {
                    mode = p_poll_config->NfcIP_Tgt_Mode_Config;
                }
                ...
                if(NFCSTATUS_PENDING == status)
                {
#if (NXP_NFC_INITIATOR_SPEED_CONFIG >= 0x01)
                    psHciContext->hci_seq = (((~(PL_RD_PHASES_DISABLE)) &
                        p_poll_config->PollDevInfo.PollEnabled)!= 0)? INITIATOR_SPEED_SEQ :
                    ...
                    PL_TGT_DISABLE_SEQ
#elif (NXP_NFC_POLL_FELICA_SYSCODE >= 0x01)
                    PL_FELICA_SYSCODE_SEQ
#else
                    PL_CONFIG_PHASE_SEQ
#endif /* #if defined (NFCIP_TGT_DISABLE_CFG) */
                    ; /* End of the Ternary Operation */
...
#endif
#if defined (ENABLE_P2P) && (NXP_NFC_INITIATOR_SPEED_CONFIG >= 0x01)
            case INITIATOR_SPEED_SEQ:
            {
#define NFCIP_ACTIVE_SHIFT  0x03U
#define NFCIP_PASSIVE_MASK  0x07U
                uint8_t mode = ( NXP_NFCIP_ACTIVE_DEFAULT << NFCIP_ACTIVE_SHIFT ) |
                                    ( speed & NFCIP_PASSIVE_MASK );
                /* Disable NFCIP active mode in Reader mode */
                if (FALSE == p_poll_config->PollDevInfo.PollCfgInfo.EnableNfcActive)
                {
                    mode &= NFCIP_PASSIVE_MASK;
                }
            ...
            case PL_TGT_DISABLE_SEQ:
            {
                ...
                status = phHciNfc_PollLoop_Cfg( psHciContext, pHwRef,
                (uint8_t)PL_DISABLE_TARGET, p_poll_config);
                if(NFCSTATUS_SUCCESS == status)
                {
#if (NXP_NFC_POLL_FELICA_SYSCODE >= 0x01)
                    psHciContext->hci_seq = PL_FELICA_SYSCODE_SEQ;
#else
                    psHciContext->hci_seq = PL_CONFIG_PHASE_SEQ;
#endif
...
#endif
#if (NXP_NFC_POLL_FELICA_SYSCODE >= 0x01)
            case PL_FELICA_SYSCODE_SEQ:
            {
                uint8_t   i = 0;
                uint8_t   sys_code[NXP_FEL_SYS_CODE_LEN] = {0,0};

                sys_code[i++] = (uint8_t )(p_poll_config->FelicaPollSystemCode
                                                            >> BYTE_SIZE);
                sys_code[i] = (uint8_t )(p_poll_config->FelicaPollSystemCode
                                                            & BYTE_MASK);
                /* Configure the Felica System Code to be Polled for */
                status = phHciNfc_Felica_Syscode_Config( psHciContext, pHwRef,
                                sys_code );
                if(NFCSTATUS_SUCCESS == status)
                {
                    psHciContext->hci_seq = PL_CONFIG_PHASE_SEQ;
            ...
            case PL_CONFIG_PHASE_SEQ:
            {
                ...
                if(NFCSTATUS_SUCCESS == status)
                {
                    ...
                    else if ( ((uint8_t)hciState_Connect)
                                 !=  psHciContext->hci_state.cur_state)
                    {
            ...
            case READER_DISABLE_SEQ:
            {
                ...
                if(NFCSTATUS_SUCCESS == status)
                {
                    ...
                    {
#if defined(NXP_NFC_POLL_AUTONOMOUS)&&( NXP_NFC_POLL_AUTONOMOUS >= 0x01 )
                        if(TRUE == p_poll_config->Autonomous_Emulation)
                        {
                            psHciContext->hci_seq = PL_AUTONOMOUS_SEQ;
                        }
                        else
#endif /* defined(NXP_NFC_POLL_AUTONOMOUS)&&( NXP_NFC_POLL_AUTONOMOUS>=0x01 ) */
                        {
                            psHciContext->hci_seq = HCI_END_SEQ;
                        }
                    }
...
#if defined(NXP_NFC_POLL_AUTONOMOUS)&&( NXP_NFC_POLL_AUTONOMOUS >= 0x01 )
            case PL_AUTONOMOUS_SEQ:
            {
                status = phHciNfc_DevMgmt_Autonomous(
                         ...
                         )
{
    ...
    phHal_MultiEmulationCfg_t           *p_multi_emu_cfg =
                (phHal_MultiEmulationCfg_t *)psHciContext->p_config_params;

    NFCSTATUS                           status = NFCSTATUS_SUCCESS;
    uint8_t                             i = 0x00;

    if (NULL != p_multi_emu_cfg)
    {
        switch(psHciContext->hci_seq)
        {
            case DEV_AID_DEFAULT_SEQ:
            {
                status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                        NFC_ADDRESS_MULTI_EMU_DEFAULT ,
                            (uint8_t) (p_multi_emu_cfg->defaultRoute),
                                   NULL );
                if ( (NFCSTATUS_PENDING == status)
                      && ( ZERO < p_multi_emu_cfg->numAidEntries ))
                {
                    psHciContext->hci_seq = DEV_AID_SEQ;
                }
                else if( (NFCSTATUS_PENDING == status)
                      && ( ZERO < p_multi_emu_cfg->numProtocolEntries )
                      && ( NFC_EMU_PROTOCOL_MIFARE_PROPRIETARY
                            == p_multi_emu_cfg->protocolInfo[i].protocolType) )
                {
                    psHciContext->hci_seq = DEV_MIFARE_CLT_SEQ;
                }
                else
                {
                    psHciContext->hci_seq = HCI_END_SEQ;
            ...
            {
                status = phHciNfc_DevMgmt_Update_Config(psHciContext, pHwRef,
                               NFC_SE_AID_CONFIG, p_multi_emu_cfg);
                if(NFCSTATUS_SUCCESS == status)
                {
                    if( (NFCSTATUS_PENDING == status)
                      && ( ZERO < p_multi_emu_cfg->numProtocolEntries )
            ...
            case DEV_MIFARE_CLT_SEQ:
            {

                status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                    NFC_ADDRESS_MIFARE_EMU_DEFAULT ,
                      (uint8_t)( NFC_EMU_LOCATION_EMBEDDED_SE
                          == p_multi_emu_cfg->protocolInfo[i].location ),
                                   NULL );
                if(NFCSTATUS_PENDING == status)
                {
                    psHciContext->hci_seq = HCI_END_SEQ;
                          ...
                          )
{
    ...
    if (NULL != p_poll_config)
    {
        switch(psHciContext->hci_seq)
        {
            case EMULATION_WI_SESSION_SEQ:
            {
                status = phHciNfc_WI_Get_Session ( psHciContext, pHwRef );
                if(NFCSTATUS_PENDING == status)
                {
                    /* Override the Configuration to set to Trigger
                     * the Special Polling Mode
                     */
                    p_poll_config->PollDevInfo.PollEnabled = PL_RD_PHASES_DISABLE;
        ...
        switch(psHciContext->hci_seq)
        {
            ...
            case READER_SELECT_SEQ:
            {
                ...
                if(( phHal_eMifare_PICC != target_type )
                    ...
                    &&( phHal_eISO14443_3B_PICC != target_type )
#endif
#endif
#ifdef TYPE_FELICA
                         ...
                         )
{
    ...
    static phNfc_sCompletionInfo_t      comp_info = {FALSE, 0 , NULL};
    phHal_eRemDevType_t         target_type = phHal_eUnknown_DevType;
    uint8_t             discovery_type = NFC_RF_DISCOVERY_CONTINUE;
    ...
    if( NULL != psHciContext->p_target_info )
    {
        ...
        switch(psHciContext->hci_seq)
        {
            ...
            {
                discovery_type = (uint8_t) ( NULL != psHciContext->p_config_params )?
                                *((uint8_t *)psHciContext->p_config_params):NFC_RF_DISCOVERY_CONTINUE;
                status = phHciNfc_ReaderMgmt_Deselect(
                                psHciContext, pHwRef, target_type, discovery_type);
                if(NFCSTATUS_PENDING == status)
                {
                    psHciContext->hci_seq = HCI_END_SEQ;
...
#endif
#if ( ES_HW_VER >= 33 ) && ( NXP_NFC_EXT_APDU_SUPPORT == 0x01 )
        {
            if((phHal_eMifare_PICC == psHciContext->p_target_info->RemDevType)
                 || (phHal_eISO14443_3A_PICC == psHciContext->p_target_info->RemDevType)
                  /* For the backward Compatibility With NFC Controller Version 3.2 */
                       || (NXP_NFC_ROM_INFO(((phHal_sHwReference_t *)pHwRef)->device_info.fw_version)
                                 < NFC44_ROM33_VER )
                    )
            {
                HCI_PRINT(" HCI MIFARE/3A Transceive \n");
                /* The break is omitted intentionally to fall through
                 *
                 */
            }
            else
            {
                HCI_PRINT(" HCI Extended APDU Send operation Completed. \n");
                transact_info.status = NFCSTATUS_SUCCESS;
                transact_result = NFC_NOTIFY_SEND_COMPLETED;
                break;
            }
        }
#endif /* ( ES_HW_VER >= 33 ) && ( NXP_NFC_EXT_APDU_SUPPORT == 0x01 ) */
        /* fall through */
