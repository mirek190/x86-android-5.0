...
#define NXP_JEWEL_READALL_UID               0x00U
#define NXP_JEWEL_READALL_UID_CMD_LENGTH    0x06U
#define NXP_JEWEL_UID_LENGTH                0x07U
#define NXP_JEWEL_READALL_LENGTH            0x7AU
#define NXP_JEWEL_HEADER_ROM0_TOPAZ_STATIC  0x11U
#define NXP_JEWEL_HEADER_ROM0_TOPAZ_DYN     0x12U
        ...
        else
        {
            switch(ps_jewel_info->current_seq)
            {
                case JEWEL_READID_SEQUENCE:
                {
                    ...
                    ps_pipe_info->param_length = (uint16_t)
                                    (NXP_JEWEL_READID_LENGTH + 1);

                    status = phHciNfc_Send_Jewel_Command(psHciContext,
                                            pHwRef, pipeid, NXP_JEWEL_RAW);

                    if(NFCSTATUS_PENDING == status )
                    {
                        ps_jewel_info->next_seq = JEWEL_READUID_SEQUENCE;
                    }
                    break;
                }
                case JEWEL_READUID_SEQUENCE:
                {
                    ...
                    pipeid = ps_pipe_info->pipe.pipe_id;
                    ps_pipe_info->reg_index = NXP_JEWEL_READALL_UID;
                    paraminfo[index] = NXP_JEWEL_READALL_UID;
                    index = (uint8_t)(index + 1);
                    paraminfo[index] = 0;
                    index = (uint8_t)(index + 1);
                    paraminfo[index] = 0;
                    index = (uint8_t)(index + 1);
                    (void)memcpy ((void *)&(paraminfo[index]),
                                    (void *)ps_jewel_tag_info->Uid,
                                    ps_jewel_tag_info->UidLength);
                    index = (uint8_t)(index + ps_jewel_tag_info->UidLength);

                    ps_pipe_info->param_info = (void *)&paraminfo;
                    ps_pipe_info->param_length = index;
    ...
    switch(index)
    {
        case NXP_JEWEL_READID:
        {
            ...
            if (NXP_JEWEL_READID_LENGTH == reg_length)
            {
                ...
                ps_jewel_tag_info->UidLength = (uint8_t)
                                            (NXP_JEWEL_READID_LENGTH - i);
                (void)memcpy((void *)ps_jewel_tag_info->Uid,
                            (void *)(reg_value + i),
                            ps_jewel_tag_info->UidLength);
                switch (ps_jewel_tag_info->HeaderRom0)
                {
                    case NXP_JEWEL_HEADER_ROM0_TOPAZ_STATIC:
                    case NXP_JEWEL_HEADER_ROM0_TOPAZ_DYN:
                    {
                        /* Change the next sequence to JEWEL_END_SEQUENCE, because for these
                            header rom bytes CHECK NDEF gives the UID */
                        ps_jewel_info->next_seq = JEWEL_END_SEQUENCE;
                        break;
                    }
        ...
        case NXP_JEWEL_READALL_UID:
        {
            HCI_PRINT_BUFFER("\tJewel 7 Byte UID", reg_value, reg_length);
            if (NXP_JEWEL_READALL_LENGTH == reg_length)
            {
                uint8_t     i = 0;
                ps_jewel_tag_info->HeaderRom0 = reg_value[i++];
                ps_jewel_tag_info->HeaderRom1 = reg_value[i++];
                            ...
                            )
{
    ...
    if ((NXP_JEWEL_READID == ps_jewel_info->p_pipe_info->reg_index)
        || (NXP_JEWEL_READALL_UID == ps_jewel_info->p_pipe_info->reg_index)
        )
    {
