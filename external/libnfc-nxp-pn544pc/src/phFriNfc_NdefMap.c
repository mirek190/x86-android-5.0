    ...
    else
    {
        switch (NdefMap->CardType)
        {
            ...
            case PH_FRINFC_NDEFMAP_MIFARE_STD_4K_CARD:
            {
                ...
                result = phFriNfc_MifareStdMap_ConvertToReadOnly (NdefMap);
...
NFCSTATUS phFriNfc_NdefMap_EraseNdef(phFriNfc_NdefMap_t *NdefMap)
{
    ...
    /* PktData size is assigned to a variable in global structure
       to overcome some of potential issues if it is used later via pointer NdefMap*/
    NdefMap->ReadingForWriteOperation = sizeof(PktData);
        ...
        switch ( NdefMap->CardType)
        {
            ...
            case PH_FRINFC_NDEFMAP_ISO15693_CARD:
                ...
                status =  phFriNfc_NdefMap_WrNdef( NdefMap,
                    PktData,
                    (uint32_t *)&NdefMap->ReadingForWriteOperation,
                    MemOffset);
                break;
