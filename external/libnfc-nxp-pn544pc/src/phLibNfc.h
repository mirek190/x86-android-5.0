...
typedef enum
{
    ...
    phLibNfc_SE_ActModeVirtualVolatile = 0x04 /**< Enabled virtual mode communication for SE through an event
                                             This can be applied to UICC as well as SmartMX.
                                             Introduced to be able to use the NFC-WI or SWP switch mode mechanism to avoid EEPROM writes.*/
...
*       This flag should be ORed with phLibNfc_eSE_ActivationMode
*       for enabling SE being visible in power by the field mode.
* \note In case of multiple SEs, only one will be visible to in
*       Power by Field. The SE visible will be the
*       one for which phLibNfc_SE_LowPowerMode_On is set last.
*/
typedef enum
{
    phLibNfc_SE_LowPowerMode_Off= 0x00, /**< Indicates no SE to be selected in low power mode */
    phLibNfc_SE_LowPowerMode_On = 0x80  /**< Indicates requested SE to be  selected in low power mode */
...
typedef enum
{
    ...
    phLibNfc_eSE_EvtSessionReset,     /* Reset the validity of the authenticated session */
...
*\ingroup grp_lib_nfc
*
*\brief Emulation Configuration details.
*/

typedef phHal_sEmulationCfg_t phLibNfc_sEmulationCfg_t;
...
* \ingroup grp_lib_nfc
*
*\brief Defines Multi Emulation Configuration Table type.
*/
typedef phHal_MultiEmulationCfg_t phLibNfc_MultiEmulationCfg_t ;
...
*\ingroup grp_lib_nfc
*
*\brief LLCP link status. Refer to \ref phLibNfc_Llcp_eLinkStatus_t
*
*/
...
*
* \param[in]    pDriverHandle   Hardware context currently application is using.
*
...
*\endmsc
*
*\note Return response callback parameters details for this interface are as listed below.
*\ref pphLibNfc_RspCb_t
*
*               \li <b> NFCSTATUS_SUCCESS Discovery </b>   Configuration  successful.
*               \li <b> NFCSTATUS_SHUTDOWN </b>       Shutdown in progress.
*               \li <b> NFCSTATUS_FAILED </b>         Request failed.
*/
...
*\endmsc
*
*\note Reurn response callback parameters details for this interface are as listed below.
*\ref pphLibNfc_RspCb_t
*
*                  \li <b> NFCSTATUS_SUCCESS </b>     Successful,indicates tag is present in RF field.
*                  \li <b> NFCSTATUS_TARGET_LOST </b> Indicates target is lost.
*                  \li <b> NFCSTATUS_SHUTDOWN </b>    Shutdown in progress.
*                  \li <b> NFCSTATUS_FAILED </b>      Request failed.
*
*/
...
*\ref pphLibNfc_RspCb_t
*
*                  <b> NFCSTATUS_SUCCESS </b>            Successful and tag is NDEF compliant .
*                  <b> NFCSTATUS_TARGET_LOST </b>        NDEF check operation is failed because of target is
*                                                        lost.
*                  <b> NFCSTATUS_SHUTDOWN </b>           Shutdown in progress.
*                  <b> NFCSTATUS_ABORTED </b>            Aborted due to disconnect operation in between.
*                  <b> NFCSTATUS_FAILED </b>             Request failed.
*/
...
* \retval NFCSTATUS_REJECTED             Rejected due to NDEF read issued on non
...
*\ref pphLibNfc_RspCb_t
*
*                  \li <b> NFCSTATUS_SUCCESS </b>             NDEF read operation successful.
*                  \li <b> NFCSTATUS_SHUTDOWN </b>            Shutdown in progress.
*                  \li <b> NFCSTATUS_ABORTED </b>             Aborted due to disconnect operation in between.
*                  \li <b> NFCSTATUS_FAILED </b>              Request failed.
*/
...
*                                   callback when the request is completed.
*
*\note If \ref phNfc_sData_t.length is 0 bytes,  this function will erase all
...
*\ref pphLibNfc_RspCb_t
*
*                  \li <b> NFCSTATUS_SUCCESS </b>             NDEF write operation is successful.
*                  \li <b> NFCSTATUS_SHUTDOWN </b>            Shutdown in progress.
*                  \li <b> NFCSTATUS_ABORTED  </b>            Aborted due to disconnect operation in between.
*                  \li <b> NFCSTATUS_NOT_ENOUGH_MEMORY </b>   Requested no of bytes to be writen exceeds size of the memory available on the tag.
*                  \li <b> NFCSTATUS_FAILED </b>              Request failed.
*/
...
*\ref pphLibNfc_RspCb_t
*
*                  \li <b> NFCSTATUS_SUCCESS </b>             NDEF formatting operation is successful.
*                  \li <b> NFCSTATUS_SHUTDOWN </b>            Shutdown in progress.
*                  \li <b> NFCSTATUS_ABORTED </b>             Aborted due to disconnect operation in between.
*                  \li <b> NFCSTATUS_FAILED </b>              Request failed.
*/
...
*\ref pphLibNfc_RspCb_t
*
*                  \li <b> NFCSTATUS_SUCCESS </b>             Converting the tag to READ ONLY NDEF is successful.
*                  \li <b> NFCSTATUS_SHUTDOWN </b>            Shutdown in progress.
*                  \li <b> NFCSTATUS_ABORTED </b>             Aborted due to disconnect operation in between.
*                  \li <b> NFCSTATUS_FAILED </b>              Request failed.
*/
...
*                                   same as as handle obtained for specific remote device
*                                   during device discovery.
*\param[in] pParam                  Input parameter required to convert the tag to read-only
*\param[in] pNdefReadOnly_RspCb     Response callback defined by the caller.
*\param[in] pContext                Client context which will be included in
*                                   callback when the request is completed.
*
...
NFCSTATUS phLibNfc_ConvertToReadOnlyNdef_Extension (phLibNfc_Handle       hRemoteDevice,
                                                  void*                   pParam,
                                                  pphLibNfc_RspCb_t       pNdefReadOnly_RspCb,
                                                  void*                   pContext
                                                  );
...
*\ref pphLibNfc_RspCb_t
*
*           \li <b>  NFCSTATUS_SUCCESS </b>            Receive operation  successful.
*           \li <b>  NFCSTATUS_SHUTDOWN </b>       Receive operation failed because Shutdown in progress.
*           \li <b>  NFCSTATUS_ABORTED </b>        Aborted due to initiator issued disconnect request.
...
*           \li <b>   NFCSTATUS_DESELECTED </b>    Receive operation is not possible due to
*                                           initiator issued disconnect or intiator
*                                           physically removed from the RF field.
*/
extern
...
*\ref pphLibNfc_RspCb_t
*
*           \li <b> param NFCSTATUS_SUCCESS </b>             Send operation  successful.
*           \li <b> NFCSTATUS_SHUTDOWN </b>       Send operation failed because Shutdown in progress.
*           \li <b> NFCSTATUS_ABORTED </b>        Aborted due to initiator issued disconnect request.
*                                           or intiator removed physically from the RF field.
*                                           This status code reported,to indicate P2P session
*                                           closed and send and receive requests not allowed
...
*\ref pphLibNfc_RspCb_t
*
*                  \li <b> NFCSTATUS_SUCCESS </b>            configuration operation is successful.
*                  \li <b> NFCSTATUS_SHUTDOWN </b>           Shutdown in progress.
*                  \li <b> NFCSTATUS_FAILED </b>             Request failed.
*
*/
...
* sockets.
*
* \param[in]  hRemoteDevice         handle of remote device to get socket options.
* \param[in]  hSocket               Socket handle obtained during socket creation.
...
* \param[in]  nSap                  Service acess point number
* \param[in] psServiceName          pointer to service name
...
*
* \param[in]  hRemoteDevice         handle of the remote device to be rejected.
* \param[in]  hSocket               Socket handle obtained in the listening callback.
...
*
* \param[in]  hRemoteDevice      handle to remote device to be connected.
* \param[in]  hSocket            Socket handle obtained during socket creation.
...
* socket is not bound to a local SAP, it is implicitly bound to a free SAP.
*
* \param[in]  hRemoteDevice      handle to remote device to be connected by URI.
* \param[in]  hSocket            Socket handle obtained during socket creation.
...
*
* \param[in]  hRemoteDevice      handle to remote device to be disconnected.
* \param[in]  hSocket            Socket handle obtained during socket creation.
...
*
*
* \param[in]  hRemoteDevice      handle to remote device to be read.
* \param[in]  hSocket            Socket handle obtained during socket creation.
...
*
*
* \param[in]  hRemoteDevice      handle to remote device to receive data from.
* \param[in]  hSocket            Socket handle obtained during socket creation.
* \param[in]  psBuffer           The buffer receiving the data.
...
*
*
* \param[in]  hRemoteDevice      handle to remote device to send.
* \param[in]  hSocket            Socket handle obtained during socket creation.
...
*
*
* \param[in]  hRemoteDevice      handle to remote device for a connectionless socket.
* \param[in]  hSocket            Socket handle obtained during socket creation.
...
*                               in initialization.
* \param[in] eTstmode           Configuration test mode phLibNfc_TstMode_Off or phLibNfc_TstMode_On.
*
* \param[in] pContext           Client context which will   be included in
*                               callback when the request is completed.
*
...
/**
* \ingroup grp_lib_nfc
* \brief <b>Interface to configure Multiple Emulation Support Parameters</b>.
* The setting will be typically take effect for the next cycle of the relevant phase of discovery.
* For optional configuration internal defaults will be used in case the configuration is not set.
*
*
* \param[in]  pMultiSeConfig          Structure containing Multiple Emulation Support configuration details
*                                     as in \ref phLibNfc_MultiEmulationCfg_t.
* \param[in]  pMultiSeConfig_RspCb    This callback will be called once the function completes the Configuration
...
*
* \note Response callback parameters details for this interface are as listed below
*    Generic Response Callback definition.
*    NFCSTATUS_SUCCESS configuration operation is successful.
*    NFCSTATUS_SHUTDOWN Shutdown in progress.
*    NFCSTATUS_FAILED Request failed.
*
*/
extern NFCSTATUS phLibNfc_Mgt_MultiEmulation_Config(
                                           phLibNfc_MultiEmulationCfg_t*    pMultiSeConfig,
                                           pphLibNfc_RspCb_t                pMultiSeConfig_RspCb,
                                           void*                            pContext
);
...
/**
* \ingroup grp_lib_nfc
* \brief <b>Interface to configure Card Emulation Parameters</b>.
* The setting will be typically take effect for the next cycle of the relevant phase of discovery.
* For optional configuration internal defaults will be used in case the configuration is not set
*
*
* \param[in]  pConfigInfo        Union containing CE configuration details as in \ref phLibNfc_sEmulationCfg_t.
* \param[in]  pConfigRspCb        This callback has to be called once LibNfc completes the Configuration
* \param[in]  pContext            Upper layer context to be returned in the callback.
*
* \retval NFCSTATUS_PENDING                   Config operation is in progress
* \retval NFCSTATUS_INVALID_PARAMETER        One or more of the supplied parameters
...
extern
NFCSTATUS
phLibNfc_Mgt_SetCE_ConfigParams  ( phLibNfc_sEmulationCfg_t *  pConfigInfo,
  pphLibNfc_RspCb_t  pConfigRspCb,
  void *  pContext
 );
...
/**
* \ingroup grp_lib_nfc
* \brief <b>This interface registers notification handler for for Card Emulation activaction and deactivation</b>.
*
* This function allows libNfc client to register for notifications based technology type
* it is interested in emulating as a Card. When the registered CE activation is received,
* LibNfc notifies registered notification callback.
*
* \note :In case this API is called multiple times, most recent request registry details will be used for registration.
*
* \param[in]  pCEHostNotification   Notification callback.This callback will be notified once registered CE activation
*                                    or deactivation is received.
* \param[in]  pContext       Client context which will be included in callback when the request is completed.
*
* \retval NFCSTATUS_SUCCESS                  Indicates registration successful
* \retval NFCSTATUS_INVALID_PARAMETER        One or more of the supplied parameters could not be
*                                             properly interpreted.
...
extern
NFCSTATUS
phLibNfc_CardEmulation_NtfRegister  ( phLibNfc_NtfRegister_RspCb_t  pCEHostNotification,
  void *  pContext
 );

/**
* \ingroup grp_lib_nfc
* \brief <b>This interface unregisters notification handler for Card Emulation activaction and deactivation</b>.
*
* This function allows libNfc client to un-register for notification which was already registered
* for the emulation notifications.
*
*
*
* \retval NFCSTATUS_SUCCESS                  Indicates un-registration successful
*/

extern
NFCSTATUS
phLibNfc_CardEmulation_NtfUnregister(void);
