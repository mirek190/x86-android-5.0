                                 ...
                                 )
{
    ...
    switch(IoctlCode)
    {
        ...
        {/* Set power status */
            ...
            {
                if( pInParam->length == 4 )
                {
                    uint8_t ver_index = 0;

                    for ( ver_index = 0; ver_index < pInParam->length; ver_index++)
                    {
                      phLibNfc_Ioctl_Cntx.psHwReference->device_info.fw_version
                            |=  pInParam->buffer[ver_index] << (ver_index * BYTE_SIZE);
                    }
                }
            }
            ...
            StatusCode = phHal4Nfc_Ioctl( phLibNfc_Ioctl_Cntx.psHwReference,
                                          ...
                                          &phLibNfc_Ioctl_Cntx );
            if((NFCSTATUS_SUCCESS == StatusCode)
                    || (NFCSTATUS_PENDING != PHNFCSTATUS(StatusCode))
                    )
            {
                 phOsalNfc_FreeMemory(phLibNfc_Ioctl_Cntx.psHwReference);
            }
            break;
        }
        case NFC_MEM_READ:
    ...
    if(
       (PHNFCSTATUS(StatusCode) != NFCSTATUS_PENDING)
        && (StatusCode != NFCSTATUS_SUCCESS)
     )
    {
    ...
    {
        StatusCode = PHNFCSTATUS(StatusCode);
        if((IoctlCode!= NFC_FW_DOWNLOAD)
            && (IoctlCode!= NFC_ENABLE_DTA_MODE) )
        {
                              ...
                              NFCSTATUS      status )
{
    ...
    if( pIoctlCntx !=NULL)
    {
        switch(pIoctlCntx->IoctlCode)
        {
            ...
            case PHLIBNFC_WI_TEST:
            {
                if((pOutData->length == 1) && (pOutData->buffer != NULL) && (*(pOutData->buffer) == 1))
                {
