...
* $Author: ravindrau $                                                         *
* $Revision: 1.111 $                                                           *
* $Aliases: NFC_FRI1.1_WK1139_R37_1 $
*                                                                             *
 *
...
typedef struct tLibNfcHciWrapperContext
{
    void *hciWrapperContext;
}tLibNfcHciWrapperContext_t;

static tLibNfcHciWrapperContext_t *gNfcHciApiContext = NULL;
NFCSTATUS phHciNfc_InitHciWrapperStack(phHciNfc_sContext_t *psHciContext,void *pHwRef);
NFCSTATUS phHciNfc_ReleaseHciWrapperStack(phHciNfc_sContext_t *psHciContext,void *pHwRef);
// END HCI WRAPPER
/*
################################################################################
****************************** Macro Definitions *******************************
                ...
                )
{
    ...
    phHciNfc_sContext_t * psHciContext = gpsHciContext ;
#endif
            ...
            && (resp_timer_id == hci_resp_timer_id ))
    {
        ...
        {
            phHciNfc_Reset_Pipe_MsgInfo(psHciContext->p_pipe_list[i]);
        }
                     ...
                     )
{
    ...
    if (
        ...
        &&(nxp_nfc_hci_response_timeout != NXP_NFC_HCI_TIMER_DISABLE)
       )
    {
        hci_response_timeout = nxp_nfc_hci_response_timeout;

        if ( hci_response_timeout <   HCI_DEFAULT_RESPONSE_TIMEOUT)
        {
            hci_response_timeout = HCI_DEFAULT_RESPONSE_TIMEOUT;
        }

        /* Start the HCI Response Timer */
        phOsalNfc_Timer_Start( hci_resp_timer_id,
                hci_response_timeout, (ppCallBck_t ) phHciNfc_Response_Timeout
#ifdef NFC_TIMER_CONTEXT
                , (void *) psHciContext
#endif
                  );
                    ...
                    )
{
    ...
    /*plower_if = &(psHciContext->lower_interface);
        if( (NULL != plower_if)  && (NULL != plower_if->receive))
        {
            psHciContext->hci_transact_state = NFC_TRANSACT_RECV_IN_PROGRESS;
            status = plower_if->receive((void *)plower_if->pcontext,
                                    (void *)pHwRef, pdata, length);
        }*/
    status = phHciWrapperCore_ReceiveHCI(gNfcHciApiContext->hciWrapperContext,pdata,length);
    //END HCI WRAPPER
    }
    return status;
}
...
*/
   ...
   // phHciNfc_InitHciNfcApiStack(psHciContext,psHciContext->p_hw_ref);
    // NOTE : tx_data cast directly to uint8_t* as original code ?!
#if  (NXP_NFC_HCI_TIMER == 1)
    ...
    status = phHciWrapperCore_SendHCI(gNfcHciApiContext->hciWrapperContext,(uint8_t*)tx_data,tx_length);
    // END HCI WRAPPER
    return status;
}
...
// HCI WRAPPER
void
extern_phHciNfc_Build_HCPHeader(
                                phHciNfc_HCP_Packet_t *hcp_packet,
                                uint8_t             chainbit,
                                uint8_t             pipe_id
                          )
...
NFCSTATUS
 ...
 {
    return phHciNfc_Send(psContext, pHwRef, pdata,length);
 }
        ...
        )
    {
        ...
        else
        {
                ...
                &&( psHciContext->tx_remain > HCP_ZERO_LEN ))
            {
                ...
                if (NFCSTATUS_NOT_ALLOWED == PHNFCSTATUS(status))
                {
                     /* Ignore this and retry again */
                     /* Revert the acknowledgmenet for retry */
                     psHciContext->tx_remain += length - 1;
                     psHciContext->tx_hcp_frgmnt_index -= length - 1;
                     HCI_DEBUG("HCI (Chaining) Send Ignored: Status = %02X\n", status);
                }
...
#else
        status = phHciNfc_Process_HCP( psHciContext, pHwRef, pdata, length );
#endif // def ONE_BYTE_LEN
// END CG FIX : SC29599
            if( ( NFCSTATUS_SUCCESS != status )
                && (NFCSTATUS_PENDING != status )
              )
        ...
        )
    {
        ...
        switch(type)
        {
            ...
            case NFC_NOTIFY_TARGET_DISCOVERED:
            {
                ...
                p_rdr_a_info = ( phHciNfc_ReaderA_Info_t * )psHciContext->p_reader_a_info;
#endif
                ...
                if ( hciState_Unknown == psHciContext->hci_state.next_state )
                {
                    ...
                    /* If the State machine does not allow
                                               to change to Select State */
...
#if    ( NXP_KOVIO == 1 )
                    else if ( phHal_eKovio_PICC == p_rdr_a_info->reader_a_info.RemDevType )
                    {
                        status = phHciNfc_FSM_Complete ( psHciContext );
                        psHciContext->event_pending = FALSE;
                        psCompInfo->info = &( p_rdr_a_info->reader_a_info );
...
#endif
                    else if ( ( NFCSTATUS_MULTIPLE_TAGS == psCompInfo->status )
                            && ( NXP_NFC_DISCOVERY_SPM == psHciContext->p_hw_ref->discovery_mode)
                        )
                    {
                      status = phHciNfc_FSM_Complete ( psHciContext );
                      psHciContext->event_pending = FALSE;
                      psCompInfo->info = NULL;
                ...
                else
                {
                    ...
                    /*status = phHciNfc_ReaderMgmt_Deselect(
                        psHciContext, pHwRef, phHal_eISO14443_A_PICC, NFC_RF_DISCOVERY_CONTINUE); */
                    if (phHal_eNfcIP1_Initiator == psHciContext->host_rf_type)
                    {
                        HCI_PRINT(" Pending Target Release \n");
                    }
...
#if    ( NXP_KOVIO == 1 )
            {
                phNfc_sCompletionInfo_t *psCompInfo = (phNfc_sCompletionInfo_t *)pInfo;
                psHciContext->event_pending=FALSE;
                phHciNfc_Notify(psHciContext->p_upper_notify,
                                psHciContext->p_upper_context,
                                pHwRef,
                ...
                if ( hciState_Unknown == psHciContext->hci_state.next_state )
                {
                    switch (psHciContext->host_rf_type)
                    {
                        ...
                        case phHal_eISO14443_4B_PICC:
                        {
                            ...
                            if (NFCSTATUS_SUCCESS == status)
                            {
                                phHciNfc_FSM_Complete(psHciContext);
                                phHciNfc_Notify(psHciContext->p_upper_notify,
                                        psHciContext->p_upper_context, pHwRef,
                        ...
                        case phHal_eNfcIP1_Initiator:
                        {
                            ...
                            if (NFCSTATUS_SUCCESS != status)
                            {
                                status = phHciNfc_ReaderMgmt_Deselect(
                                        psHciContext, pHwRef,
                                                    phHal_eISO14443_A_PICC, NFC_RF_DISCOVERY_CONTINUE);
                            }
                            break;
                        }
            ...
            case NFC_NOTIFY_DEVICE_DEACTIVATED:
            {
                ...
                else
                {
                        ...
                        || (hciState_Transact == psHciContext->hci_state.next_state))
                    {
                        ...
                        /* Roll Back to the Default State after
...
// HCI WRAPPER
NFCSTATUS
phHciNfc_InitHciWrapperStack(phHciNfc_sContext_t *psHciContext,void *pHwRef)
{
    ALOGD("--> %s",__FUNCTION__);
    if (gNfcHciApiContext == NULL)
    {
        gNfcHciApiContext                     = (tLibNfcHciWrapperContext_t*)phOsalNfc_GetMemory(sizeof(tLibNfcHciWrapperContext_t));
        gNfcHciApiContext->hciWrapperContext  = NULL;

        return phHciWrapperCore_Init(&(gNfcHciApiContext->hciWrapperContext),psHciContext,pHwRef);
    }else
    {
        // Outstanding !
        return phHciWrapperCore_Reset(gNfcHciApiContext->hciWrapperContext,psHciContext,pHwRef);
    }
    ALOGD("<-- %s",__FUNCTION__);
}

NFCSTATUS
phHciNfc_ReleaseHciWrapperStack(phHciNfc_sContext_t *psHciContext,void *pHwRef)
{
    if (gNfcHciApiContext != NULL)
    {
        phHciWrapperCore_DeInit(gNfcHciApiContext->hciWrapperContext,psHciContext,pHwRef);
        gNfcHciApiContext = NULL;
    }
    return NFCSTATUS_SUCCESS;
}

void phHciNfc_EmergencyRelease()
{
    if (gNfcHciApiContext != NULL)
    {
        phHciWrapperCore_EmergencyRelease(gNfcHciApiContext->hciWrapperContext);
    }
}
