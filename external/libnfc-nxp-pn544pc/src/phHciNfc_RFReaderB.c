                        ...
                        )
{
    ...
    if(NXP_NFC_ROM_INFO(((phHal_sHwReference_t *)pHwRef)->device_info.fw_version)
                 >= NFC44_ROM33_VER )
    {
        status = phHciNfc_ReaderB_Send_Data(psHciContext,pHwRef,
                                                  p_send_param );
    }
    else
    {
                         ...
                         )
{
    ...
    else
    {
        ...
        else
        {
            ...
            HCI_PRINT_BUFFER("HCI Reader B Send Data: ", p_send_param->tx_buffer, p_send_param->tx_length);
                  ...
                  )
{
    ...
    else
    {
        ...
        /* p_rdr_b_info = (phHciNfc_ReaderB_Info_t *)
                            psHciContext->p_reader_b_info ; */
        HCI_PRINT(" Reader B Extended APDU received bytes :");
        ...
        {
            HCI_PRINT(" Reader B Extended Data receive RF ERROR ");
            type = (uint8_t)NFC_NOTIFY_RECV_EVENT;
        ...
        else
        {
            ...
            case ANY_GET_PARAMETER:
                ...
                case NXP_3B_RAW_XCH:
                {
                    ...
                    {
                        uint8_t         i = 0;
                        HCI_PRINT("ISO 14443-3B RAW received \n");
                        phHciNfc_Append_HCPFrame(psHciContext->recv_buffer,
                                                0, pResponse, length);
                        psHciContext->rx_total = (length - i);
                        status = phHciNfc_Recv_Iso_B_Packet(psHciContext,
                            ...
                            )
{
    ...
    else
    {
        ...
        switch(instruction)
        {
            ...
            {
                HCI_DEBUG ("HCI : Multiple tag B found : %02X\n", message->payload[i]);
                    ...
                    )
                {
                    ...
                    HCI_DEBUG ("HCI : p_rdr_b_info->reader_a_info.RemDevType : %02X\n", p_rdr_b_info->reader_b_info.RemDevType);
                    HCI_DEBUG ("HCI : p_rdr_b_info->current_seq : %02X\n", p_rdr_b_info->current_seq);
            ...
            {
                status = phHciNfc_ReaderB_Recv_Data(psHciContext,
                                    pHwRef,
                           ...
                           )
{
    NFCSTATUS       status = NFCSTATUS_SUCCESS;
    uint8_t         i = 1;
    phHciNfc_ReaderB_Info_t     *p_rdr_b_info=(phHciNfc_ReaderB_Info_t *)psHciContext->p_reader_b_info ;
...
#ifdef NFC_TYPE_3B_RAW
    if((phHal_eISO14443_3B_PICC == p_rdr_b_info->reader_b_info.RemDevType)
       &&((uint8_t)0x00 != pResponse[0]))
    {
        status = PHNFCSTVAL(CID_NFC_HCI,
                            NFCSTATUS_RF_ERROR);
    }
...
#ifdef NFC_TYPE_3B_RAW
NFCSTATUS
phHciNfc_ReaderBRaw_Get_Pupi(
                   phHciNfc_sContext_t   *psHciContext,
                   void                  *pHwRef
                   )
{
    NFCSTATUS           status = NFCSTATUS_SUCCESS;
    static uint8_t      pres_chk_data[7] = {0};
    if( (NULL == psHciContext) || (NULL == pHwRef) )
    {
        status = PHNFCSTVAL(CID_NFC_HCI, NFCSTATUS_INVALID_PARAMETER);
    }
    else
    {
        ...
        p_rdr_b_info = (phHciNfc_ReaderB_Info_t *)
                            psHciContext->p_reader_b_info ;
        ps_pipe_info = p_rdr_b_info->p_pipe_info;
        ...
        {
            pres_chk_data[i++] = 0x00; /* china ID poll */
            pres_chk_data[i++] = 0x36;
            pres_chk_data[i++] = 0x00;
            pres_chk_data[i++] = 0x00;
            pres_chk_data[i++] = 0x08;
            pres_chk_data[i++] =0x57;
            pres_chk_data[i++] =0x44;
