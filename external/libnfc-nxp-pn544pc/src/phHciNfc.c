...
// HCI WRAPPER
#include "ext/src/core/phHciWrapperCore.h"
extern NFCSTATUS phHciNfc_InitHciWrapperStack(phHciNfc_sContext_t *psHciContext,void *pHwRef);
// END HCI WRAPPER
    ...
    else
    {
        ...
        if(psHciContext != NULL)
        {
            ...
            (void)memcpy(pHwConfig->session_id,
                DEFAULT_SESSION, ((DEFAULT_SESSION_SIZE > 0x01)
                                    && (DEFAULT_SESSION_SIZE <= 0x08 )) ?
            ...
            if( (NFCSTATUS_SUCCESS == status) && (NULL != plower_if->init) )
            {
                ...
                if(NFCSTATUS_SUCCESS == status)
                {
                   ...
                   //                       (void *)psHciContext->p_hw_ref);
                    status = phHciNfc_InitHciWrapperStack(psHciContext,pHwRef);
                    // END HCI WRAPPER
    ...
    else
    {
        switch(config_type)
        {
            ...
            case NFC_SE_AID_CONFIG:
            {
                ...
                if (NFCSTATUS_SUCCESS == status)
                {
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_MULTI_EMU_SUPPORT ,
                                pConfig->multiEmuConfig.multiEmulationEnable ,
                                       NULL );
                    if( NFCSTATUS_PENDING != status )
                    {
                        /* Roll Back the State Machine to its Original State */
                    ...
                    {
                        psHciContext->config_type = SE_MULTI_EMU_CFG;
                        psHciContext->p_config_params = &(pConfig->multiEmuConfig);
                        psHciContext->hci_seq = DEV_AID_DEFAULT_SEQ;
                    }
                }
                break;
            }
            case NFC_HW_TABLE_CONFIG:
            {
                ...
                if (NFCSTATUS_SUCCESS == status)
                {
                    psHciContext->config_type = NFC_ADDR_CFG;
                     ...
                     )
{
    ...
    else
    {
        ...
        if (NFCSTATUS_SUCCESS == status)
        {
            ...
            psHciContext->hci_seq = PL_RF_STATE_DBG_SEQ;
#else
                     ...
                     )
{
    ...
    else
    {
        ...
        else if (phHal_eHostController == pEmulationCfg->hostType )
        {
...
{
    phHciNfc_sContext_t  *psHciContext = ((phHciNfc_sContext_t *)psHciHandle);
    static phHal_sADD_Cfg_t hci_poll_config;
    NFCSTATUS   status = NFCSTATUS_SUCCESS;
    ...
    else
    {
        ...
        if (NFCSTATUS_SUCCESS == status)
        {
            (void)memcpy( (void *)(&hci_poll_config),
                 (void *)pPollConfig,
                    sizeof(hci_poll_config));
                ...
                )
            {
                 ...
                 */
                 ...
                 if( ( NXP_NFC_DISCOVERY_SPM
                        ...
                        && (eSmartMx_Off ==  smx_mode)
                     )
                {
                    psHciContext->hci_seq = EMULATION_WI_SESSION_SEQ;
                }
                else
                {
            ...
            psHciContext->p_config_params = &hci_poll_config;
 ...
 */
                 ...
                 )
 {
    ...
    static  uint8_t      rf_discovery_type = NFC_RF_DISCOVERY_CONTINUE;
    ...
    {
        psHciContext->p_hw_ref = pHwRef;
        rf_discovery_type = discovery_type;
        psHciContext->p_config_params = &rf_discovery_type;
        /* psHciContext->hci_seq = HCI_END_SEQ; */
        ...
        if (NFCSTATUS_SUCCESS == status)
        {
            if ( (( NFC_RF_DISCOVERY_HALT ==  discovery_type )
                || ( NFC_RF_DISCOVERY_CONTINUE ==  discovery_type )
                  || ( NFC_RF_DISCOVERY_REPOLL ==  discovery_type ) )
    ...
    else
    {
        ...
        if (NFCSTATUS_SUCCESS == status)
        {
            switch (psHciContext->host_rf_type)
            {
                ...
                case phHal_eISO14443_4B_PICC:
                {
                    status = phHciNfc_Emulation_Send( psHciContext,
                                   pHwRef,  p_remote_dev_info, p_send_param );
                    break;
                }
#endif
#ifdef ENABLE_P2P
                ...
                case phHal_eISO14443_B_PCD:
                {
                    status = phHciNfc_ReaderMgmt_Send( psHciContext,
                                   pHwRef,  p_remote_dev_info, p_send_param );
                    break;
                }
                case phHal_eUnknown_DevType:
                 ...
                 )
{
    ...
    else
    {
        ...
        {
            status = phHciNfc_DevMgmt_Configure(psHciContext, pHwRef,
                config_type, config_value, p_value_set);
                 ...
                 )
{
    ...
    else
    {
        ...
        if (NFCSTATUS_SUCCESS == status)
        {
            if ( phHal_eUICCHost == host_link)
            {
                status = phHciNfc_SWP_Get_Status(psHciContext, pHwRef, link_status );
            }
            else
            {
