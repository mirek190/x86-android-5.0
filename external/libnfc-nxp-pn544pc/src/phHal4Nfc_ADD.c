...
static uint8_t type3B_switch=TRUE;
#endif
                                       ...
                                       )
{
    ...
    Hal4Ctxt->Hal4NextState  = eHal4StateInvalid;
    uint8_t  pupi_Type3B_Raw[4]={0,0,0,0};
            ...
            case phHal_eISO14443_A_PICC:/*for TYPE A*/
            {
                    ...
                    || (TRUE == Hal4Ctxt->psADDCtxtInfo->smx_discovery))
                {
                    ...
                    if((0 == Sak)&& (0 == Count))
                    {
                        ...
                        {
                            aRemoteDevTypes[Count] = phHal_eMifare_PICC;
#if defined (NXP_NFC_DETECT_TYPE_4A_WITH_3A)
                            Count++;
#endif
                        }
#if !defined (NXP_NFC_DETECT_TYPE_4A_WITH_3A)
...
#if    (NXP_KOVIO == 1)
            case phHal_eKovio_PICC:/*P2P target detected*///kovio
            {
                aRemoteDevTypes[Count] = phHal_eKovio_PICC;
                Count++;
                break;
            }
#endif
...
#ifdef TYPE_B
#ifdef NFC_TYPE_3B_RAW
                if(!memcmp(psRemoteDevInfo->RemoteDevInfo.Iso14443B_Info.AtqB.AtqResInfo.Pupi,pupi_Type3B_Raw,4))
                {
                    if(type3B_switch == TRUE)
                    {
                        aRemoteDevTypes[Count] = phHal_eISO14443_3B_PICC;
                        Count++;
                        type3B_switch=FALSE;
                    }
                    else
...
#if    ( NXP_KOVIO == 1 )
void phHal4Nfc_TargetReleaseComplete(
                                     phHal4Nfc_Hal4Ctxt_t *Hal4Ctxt,
                                     void                 *pInfo
                                    )
{
    NFCSTATUS status = NFCSTATUS_SUCCESS;
    PHDBG_INFO("Hal4:target release complete Notification");
    Hal4Ctxt->psADDCtxtInfo->nbr_of_devices = 0;
    status = phHciNfc_Restart_Discovery((void *)Hal4Ctxt->psHciHandle,(void *)gpphHal4Nfc_Hwref,FALSE);
    Hal4Ctxt->Hal4NextState = ( NFCSTATUS_PENDING == status ? eHal4StateConfiguring : Hal4Ctxt->Hal4NextState );
}
#endif/*NXP_KOVIO*/
                        ...
                        )
{
    ...
    else
    {
        ...
        {
            case eRegisterSPMDiscovery:
                /* Hal4Ctxt->hal_discovery_mode = NXP_NFC_DISCOVERY_SPM; */
                                  ...
                                  )
{
    ...
    else if ((((phHal4Nfc_Hal4Ctxt_t *)psHwReference->hal_context)->Hal4CurrentState
                < eHal4StateOpenAndReady)
            || (((phHal4Nfc_Hal4Ctxt_t *)psHwReference->hal_context)->Hal4NextState
             == eHal4StateClosed))
    {
        RetStatus = PHNFCSTVAL(CID_NFC_HAL , NFCSTATUS_NOT_INITIALISED);
    }
