...
phLlcNfc_CreateTimers(void)
{
    ...
    while (index < PH_LLCNFC_MAX_TIMER_USED)
    {
        ...
        PH_LLCNFC_DEBUG("LLC: TIMER ID CREATED : 0x%0X\n", gpphLlcNfc_Ctxt->s_timerinfo.timer_id[index]);
#endif /* #ifdef LLC_TIMER_ENABLE */
        index++;
    }
    return;
...
)
{
    ...
    switch(TimerType)
    {
        case PH_LLCNFC_CONNECTIONTIMER:
        {
            ...
            PH_LLCNFC_DEBUG("Connection Timer id: 0x%0X\n", timerid);
            PH_LLCNFC_DEBUG("Connection Timer Status, Timer Started: 0x%0X\n", timerstarted);
            PH_LLCNFC_DEBUG("RESOLUTION VALUE : 0x%04X\n", PH_LLCNFC_RESOLUTION);
            PH_LLCNFC_DEBUG("TIME-OUT VALUE : 0x%04X\n", PH_LLCNFC_CONNECTION_TO_VALUE);
            break;
        }
        ...
        case PH_LLCNFC_ACKTIMER:
        {
            ...
            PH_LLCNFC_DEBUG("Ack Timer id: 0x%0X\n", timerid);
            PH_LLCNFC_DEBUG("Ack Timer Status, Timer Started: 0x%02X\n", timerstarted);
            PH_LLCNFC_DEBUG("RESOLUTION VALUE : 0x%04X\n", PH_LLCNFC_RESOLUTION);
            PH_LLCNFC_DEBUG("TIME-OUT VALUE : 0x%04X\n", PH_LLCNFC_ACK_TO_VALUE);
            break;
        }
    ...
    if ((NFCSTATUS_SUCCESS == result) &&
        (FALSE == timerstarted))
    {
        PH_LLCNFC_DEBUG("OSAL START TIMER CALLED TIMER ID : 0x%0X\n", timerid);
        phOsalNfc_Timer_Start (timerid,
                          timer_resolution, Callback
#ifdef NFC_TIMER_CONTEXT
                             ,NULL
#endif
                              );
...
)
{
    ...
    switch(TimerType)
    {
        case PH_LLCNFC_CONNECTIONTIMER:
        {
            ...
            PH_LLCNFC_DEBUG("CONNECTION TIMER ID : 0x%0X\n", timerid);
            PH_LLCNFC_DEBUG("CONNECTION TIMER FLAG : 0x%02X\n", ps_timer_info->timer_flag);
            break;
        }
        ...
        case PH_LLCNFC_GUARDTIMER:
        {
            ...
            PH_LLCNFC_DEBUG("GUARD TIMER FLAG : 0x%0X\n", ps_timer_info->timer_flag);
            break;
        }
        ...
        case PH_LLCNFC_ACKTIMER:
        {
            ...
            PH_LLCNFC_DEBUG("ACK TIMER ID : 0x%0X\n", timerid);
            PH_LLCNFC_DEBUG("ACK TIMER FLAG : 0x%02X\n", ps_timer_info->timer_flag);
            break;
        }
#endif /* #ifdef PIGGY_BACK */
    ...
    {
        PH_LLCNFC_DEBUG("OSAL STOP TIMER CALLED TIMER ID : 0x%0X\n", timerid);
...
phLlcNfc_StopAllTimers (void)
{
    ...
    if (0 != timer_started)
    {
        ...
        PH_LLCNFC_DEBUG("GUARD TIMER ID: 0x%0X\n", timerid);
    ...
    PH_LLCNFC_DEBUG("ACK TIMER ID: 0x%02X\n", timerid);
   ...
   )
{
    ...
    PH_LLCNFC_DEBUG("GUARD PARAMETER TIMER ID : 0x%0X\n", TimerId);
    PH_LLCNFC_DEBUG("GUARD TIMER FLAG : 0x%02X\n", gpphLlcNfc_Ctxt->s_timerinfo.timer_flag);
        ...
        PH_LLCNFC_GUARD_TO_BIT_VAL)))
    {
        ...
        phOsalNfc_Timer_Start(
                    ps_timer_info->timer_id[PH_LLCNFC_GUARDTIMER],
                    PH_LLCNFC_RESOLUTION, phLlcNfc_GuardTimeoutCb
#ifdef NFC_TIMER_CONTEXT
                             ,NULL
#endif
                             );
   ...
   )
{
    ...
    PH_LLCNFC_DEBUG("ACK PARAMETER TIMER ID : 0x%0X\n", TimerId);
    PH_LLCNFC_DEBUG("ACK TIMER FLAG : 0x%02X\n", gpphLlcNfc_Ctxt->s_timerinfo.timer_flag);
        ...
        PH_LLCNFC_CON_TO_BIT_VAL)))
    {
        ...
        if (ps_timer_info->con_to_value > 0)
        {
            ...
            else
            {
                ...
                phOsalNfc_Timer_Start(
                            ps_timer_info->timer_id[PH_LLCNFC_CONNECTIONTIMER],
                            ps_timer_info->con_to_value, phLlcNfc_ConnectionTimeoutCb
#ifdef NFC_TIMER_CONTEXT
                             ,NULL
#endif
                             );
