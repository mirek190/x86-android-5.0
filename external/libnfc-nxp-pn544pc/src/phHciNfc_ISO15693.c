...
NFCSTATUS
phHciNfc_ISO15693_Presence_Check(
                              phHciNfc_sContext_t   *psHciContext,
                              void                  *pHwRef
                              )
{
    ...
    else
    {
        ...
        {
#define INVENTORY_BUFFER 0x0B
                    uint8_t inventory[INVENTORY_BUFFER];
                    uint8_t i = 0;

                    pipeid = ps_pipe_info->pipe.pipe_id;

                    ps_pipe_info->param_info = &inventory;
                    ps_pipe_info->param_length = sizeof(inventory);
                    /* masked inventory command:
                     set #slots to 1 to use mask without padding,
                     need to set inventory flag to enable setting #slots */
                    inventory[i++] = 0x04 | 0x20; // FLAG_INVENTORY | FLAG_SLOTS
                    inventory[i++] = 0x01; // CMD_INVENTORY
                    inventory[i++] = 64; // mask bit-length
                    memcpy(inventory + i,
                        &(psHciContext->p_target_info->RemoteDevInfo.Iso15693_Info.Uid),
                                       PHHAL_15693_UID_LENGTH);
                    status = phHciNfc_Send_ISO15693_Command(
                        psHciContext,  pHwRef
                        ,pipeid, NXP_ISO15693_CMD );
        }
    }
