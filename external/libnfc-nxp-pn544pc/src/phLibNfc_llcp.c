...
NFCSTATUS phLibNfc_Llcp_Close( phLibNfc_Handle hSocket )
{
   ...
   LLCP_PRINT("phLibNfc_Llcp_Close - start");
   ...
   if (result != NFCSTATUS_SUCCESS)
   {
      LLCP_PRINT("phLibNfc_Llcp_Close - state check failed - returning");
      return result;
   }
   ...
   if (hSocket == 0)
   {
      LLCP_PRINT("phLibNfc_Llcp_Close - Invalid Parameter -  returning");
      return NFCSTATUS_INVALID_PARAMETER;
   }

   LLCP_PRINT("phLibNfc_Llcp_Close - calling phFriNfc_LlcpTransport_Close()");
   /* Get local infos */
