                       ...
                       )
{
    ...
    phHciNfc_sContext_t         *psHciContext =
                                (phHciNfc_sContext_t *)psContext ;
    static phHal_sEventInfo_t   EventInfo = {phHal_eHostController,
                                             phHal_eUnknown_DevType};
    ...
    else
    {
        ...
        switch(instruction)
        {
            case EVT_TARGET_DISCOVERED:
            {
                HCI_DEBUG ("HCI : Multiple tag A found : %02X\n", message->payload[i]);
...
#if    ( NXP_KOVIO == 1 )
            case NXP_EVT_KOVIO_TAG_EVENT:
            {
                phNfc_sCompletionInfo_t pCompInfo;
                phHal_sIsoKovioInfo_t   *p_tag_a_info  = NULL;
                p_rdr_a_info->multiple_tgts_found      = FALSE;
                p_rdr_a_info->reader_a_info.RemDevType = phHal_eKovio_PICC;
                p_tag_a_info                           = &(p_rdr_a_info->reader_a_info.RemoteDevInfo.kovio_Info);
                p_tag_a_info->UidLength                = 16;
                (void)memcpy(p_tag_a_info->Uid,
                             &(message->payload[0]),
                             p_tag_a_info->UidLength);
                pCompInfo.status = NFCSTATUS_SUCCESS;
                psHciContext->host_rf_type = phHal_eISO14443_A_PCD;
                    phHciNfc_Notify_Event(psHciContext,
                                          pHwRef,
                                            NFC_NOTIFY_TARGET_DISCOVERED,
                                            &pCompInfo);
                 break;
            }
            case NXP_EVT_KOVIO_TAG_LOST:
            {
                phNfc_sCompletionInfo_t pCompInfo;
                ...
                p_tag_a_info                           = &(p_rdr_a_info->reader_a_info.RemoteDevInfo.kovio_Info);
                p_tag_a_info->UidLength                = 16;
                (void)memset(p_tag_a_info->Uid,
                             0x00,
                             p_tag_a_info->UidLength);
                p_tag_a_info->UidLength                = 0;
                p_rdr_a_info->reader_a_info.RemDevType = phHal_eInvalid_DevType;
                pCompInfo.status = NFCSTATUS_SUCCESS;
                phHciNfc_Notify_Event(psHciContext,
                                      pHwRef,
                                      NFC_NOTIFY_TARGET_RELEASED,
                                      &pCompInfo);
               break;
            }
#endif
            case NXP_EVT_WI_WTX:
            {
                ...
                EventInfo.eventType = NFC_EVT_ESE_WTX_EVENT;
                phHciNfc_Notify_Event(  psHciContext,
                                        pHwRef,
                                        NFC_NOTIFY_EVENT,
                                        (void*)&EventInfo);
            ...
            case NXP_EVT_WR_RCV_DATA:
            {
                status = phHciNfc_ReaderA_Recv_Data(psHciContext,
                                    pHwRef,
                                    &pEvent[HCP_HEADER_LEN],
                                    (length - HCP_HEADER_LEN));
                break;
            }
            case NXP_EVT_WR_CONTINUE_MI:
            {
                psHciContext->event_pending = FALSE;
                break;
            }
            case NXP_EVT_TARGET_REMOVED:
            {
                psHciContext->event_pending = FALSE;
                break;
            }
...
NFCSTATUS
phHciNfc_ReaderA_Get_DataRateMax(
                               void         *psContext,
                               void         *pHwRef
                               )
{
                        ...
                        )
{
    ...
    if(NXP_NFC_ROM_INFO(((phHal_sHwReference_t *)pHwRef)->device_info.fw_version)
                 >= NFC44_ROM33_VER )
    {
        status = phHciNfc_ReaderA_Send_Data(psHciContext,pHwRef,
                                                  p_send_param );
    }
    else
    {
                         ...
                         )
{
    ...
    if( (NULL == psHciContext) || (NULL == pHwRef) || (NULL == p_send_param) ||
        (NULL == p_send_param->tx_buffer) || (0 == p_send_param->tx_length))
    {
        status = PHNFCSTVAL(CID_NFC_HCI, NFCSTATUS_INVALID_PARAMETER);
    }
    else if((NULL == psHciContext->p_reader_a_info) ||
    ...
    else
    {
        ...
        else
        {
            ...
            HCI_PRINT_BUFFER("HCI Reader A Send Data: ", p_send_param->tx_buffer, p_send_param->tx_length);

            psHciContext->tx_total = 0 ;
            pipeid = p_pipe_info->pipe.pipe_id;
            hcp_packet = (phHciNfc_HCP_Packet_t *) psHciContext->send_buffer;
            hcp_message = &(hcp_packet->msg.message);
            hcp_message->payload[i] = (uint8_t)p_send_param->timeout;
            i++;
            hcp_message->payload[i] = p_send_param->more_info;
            i++;
            ...
            phHciNfc_Build_HCPFrame(hcp_packet,(uint8_t)HCP_CHAINBIT_DEFAULT,
                            ...
                            (uint8_t)NXP_EVT_WR_SND_DATA);

            phHciNfc_Append_HCPFrame((uint8_t *)hcp_message->payload,
                            i, (uint8_t *)p_send_param->tx_buffer,
                            (uint8_t)p_send_param->tx_length);

            length =(uint16_t)(length + i + p_send_param->tx_length);

            p_pipe_info->sent_msg_type = (uint8_t)HCP_MSG_TYPE_EVENT;
            p_pipe_info->prev_msg = NXP_EVT_WR_SND_DATA;
            ...
            if ((TRUE == p_send_param->more_info) &&
                  ...
                  )
{
    ...
    {
        phNfc_sTransactionInfo_t    transInfo = {0};
        /* phHciNfc_ReaderA_Info_t     *p_rdr_a_info=NULL; */
        uint8_t                     type = 0;

        /* p_rdr_a_info = (phHciNfc_ReaderA_Info_t *)
                            psHciContext->p_reader_a_info ; */
        HCI_PRINT(" Reader A Extended APDU received bytes :");
        if (0x00 == pResponse[index])
        {
            HCI_PRINT_BUFFER(" Buffer ", &pResponse[index], (length - index));
        ...
        {
            HCI_PRINT(" Reader A Extended Data receive RF ERROR ");
            type = (uint8_t)NFC_NOTIFY_RECV_EVENT;
...
{
    NFCSTATUS                   status = NFCSTATUS_SUCCESS;
    uint16_t                    timeout = NXP_MIFARE_XCHG_TIMEOUT;
...
#ifdef ENABLE_MIFARE_RAW
                        /* Time out */
                        if(timeout < NXP_MIFARE_XCHG_TIMEOUT && timeout != 0)
                        {
                            timeout = NXP_MIFARE_XCHG_TIMEOUT;
                        }
