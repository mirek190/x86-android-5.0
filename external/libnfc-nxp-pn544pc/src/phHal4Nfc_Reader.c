    ...
    else
    {
            ...
            && (NULL == Hal4Ctxt->sTgtConnectInfo.psConnectedDevice))
        {
            ...
            else
            {
                ...
                {
                    Hal4Ctxt->rem_dev_list[RemoteDevCount]
                          ->ConnectWithPresence = !(NXP_NFC_DISCOVERY_DTA
                                                       == Hal4Ctxt->hal_discovery_mode);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                switch(Hal4Ctxt->sTgtConnectInfo.psConnectedDevice->RemDevType)
                {
                    ...
                    {
                        PHDBG_INFO("ISO14443 Cmd received");
#if ( ES_HW_VER < 33 ) || ( NXP_NFC_EXT_APDU_SUPPORT == 0x00 )
                        Hal4Ctxt->psTrcvCtxtInfo->
                            XchangeInfo.params.tag_info.cmd_type
                                = (uint8_t)psTransceiveInfo->cmd.Iso144434Cmd;
                        break;
                    ...
                    {
                        PHDBG_INFO("NfcIP1 Or Extended APDU Transceive");
        ...
        if(RetStatus == NFCSTATUS_PENDING)
        {
            if((phHal_eNfcIP1_Target ==
               ...
               && (NXP_NFC_ROM_INFO(((phHal_sHwReference_t *)psHwReference)->device_info.fw_version)
                           >= NFC44_ROM33_VER ))
#endif
           )
            {
                ...
                /* update timeout */
                Hal4Ctxt->psTrcvCtxtInfo->XchangeInfo.timeout = psTransceiveInfo->timeout;
                if(Hal4Ctxt->RemTransactLen
                    >= psTransceiveInfo->sSendData.length)
                {
                    Hal4Ctxt->psTrcvCtxtInfo->
                        XchangeInfo.more_info = FALSE;
                ...
                {
                    Hal4Ctxt->psTrcvCtxtInfo->
                        XchangeInfo.more_info = TRUE;
                    Hal4Ctxt->psTrcvCtxtInfo->XchangeInfo.tx_buffer
                        = Hal4Ctxt->psTrcvCtxtInfo->psUpperSendData->buffer;
                    Hal4Ctxt->psTrcvCtxtInfo->XchangeInfo.tx_length
                                                = Hal4Ctxt->RemTransactLen;
...
#endif/*TRANSACTION_TIMER*/
                {
                    ...
                    if(NFCSTATUS_PENDING == RetStatus)
                    {
                        Hal4Ctxt->psTrcvCtxtInfo->Send_In_Progress = TRUE;
                    }
                }
            }
            else if(psTransceiveInfo->sSendData.length >
                                    (PHHAL_MAX_DATASIZE + NFC_APDU_HDR_LEN) )
            {
                RetStatus = PHNFCSTVAL(CID_NFC_HAL,NFCSTATUS_INVALID_PARAMETER);
            }
            ...
            else/*No more bytes remaining in Hal.Read from device*/
            {
                ...
                Hal4Ctxt->psTrcvCtxtInfo->XchangeInfo.params.tag_info.addr
                                                    = psTransceiveInfo->addr;
                Hal4Ctxt->psTrcvCtxtInfo->XchangeInfo.timeout
                                = psTransceiveInfo->timeout;
                Hal4Ctxt->psTrcvCtxtInfo->XchangeInfo.tx_length
...
void phHal4Nfc_TrcvTimeoutHandler(uint32_t TrcvTimerId)
{
    ...
    Hal4Ctxt->Hal4NextState = eHal4StateInvalid;
    /*For a P2P target*/
    if(Hal4Ctxt->psTrcvCtxtInfo->pRecvCb != NULL)
    {
        pUpperRecvCb = Hal4Ctxt->psTrcvCtxtInfo->pRecvCb;
        Hal4Ctxt->psTrcvCtxtInfo->pRecvCb = NULL;
        (*pUpperRecvCb)(
                        ...
                        )
{
    ...
    else
    {
        ...
        {
            /* Register upper layer context */
            Hal4Ctxt->sUpperLayerInfo.psUpperLayerDisConCtxt = pContext;
            /*Register upper layer callback*/
                   ...
                   &&(NFC_DISCOVERY_RESTART != ReleaseType))))
            {
                ...
                RetStatus = PHNFCSTVAL(CID_NFC_HAL,NFCSTATUS_NOT_ALLOWED);
                    ...
                    &&(TRUE == Hal4Ctxt->psTrcvCtxtInfo->Send_In_Progress))
            {
                            ...
                            )
{
    ...
    switch(Hal4Ctxt->sTgtConnectInfo.ReleaseType)
    {
        ...
        case NFC_SMARTMX_RELEASE:
            ...
            break;
        case NFC_SMARTMX_RELEASE_SPM:
                  /* For the backward Compatibility With NFC Controller Version 3.2 */
            if(NXP_NFC_ROM_INFO(((phHal_sHwReference_t *)psHwReference)->device_info.fw_version)
                           >= NFC44_ROM33_VER )
            {
                psHwReference->discovery_mode =
                                NXP_NFC_DISCOVERY_SPM;
                SmxMode = eSmartMx_Off;
                RetStatus = phHciNfc_Switch_SmxMode (
                    Hal4Ctxt->psHciHandle,
                    psHwReference,
                    SmxMode,
                               ...
                               )
{
        ...
        ||(eHal4StateTargetConnected == Hal4Ctxt->Hal4CurrentState))
    {
        ...
        Hal4Ctxt->sTgtConnectInfo.pUpperConnectCb = NULL;
        switch(psRmtTgtConnected->RemDevType)
        {
            case phHal_eFelica_PICC:
            {
                felicaRePoll = (Hal4Ctxt->hal_discovery_mode
                                      != NXP_NFC_DISCOVERY_DTA)? TRUE: FALSE;
                Hal4Ctxt->RemTransactLen = PHHAL_MAX_DATASIZE;
                break;
            }
            case phHal_eISO14443_A_PICC:
            case phHal_eISO14443_4A_PICC:
            {
#ifdef ISO_4A_TYPE_IDENTIFICATION
                if ((0 != psRmtTgtConnected->RemoteDevInfo.Iso14443A_Info.AppDataLength) &&
                    (psRmtTgtConnected->RemoteDevInfo.Iso14443A_Info.AppData))
                {

                }
#endif /* #ifdef ISO_4A_TYPE_IDENTIFICATION */
                Hal4Ctxt->RemTransactLen = PHHAL_MAX_DATASIZE - NFC_APDU_HDR_LEN;
                break;
            }
            case phHal_eISO14443_B_PICC:
            ...
            {
                Hal4Ctxt->RemTransactLen =
                    psRmtTgtConnected->RemoteDevInfo.NfcIP_Info.MaxFrameLength
                    - ( NFC_DEP_HDR_LEN + NXP_NFCIP_DID_DEFAULT) ;
                break;
            }
            default:
            {
    ...
    else/*Remote device Disconnect successful*/
    {
        ...
        if(NULL != Hal4Ctxt->psTrcvCtxtInfo)
        {
            ...
            else
            {
                if((phHal_eNfcIP1_Target
                    == Hal4Ctxt->sTgtConnectInfo.psConnectedDevice->RemDevType) &&
                    (NULL != Hal4Ctxt->psTrcvCtxtInfo->pUpperTranceiveCb))
                {
                    pUpperTrcvCb = Hal4Ctxt->psTrcvCtxtInfo->pUpperTranceiveCb;
        ...
        if((NXP_NFC_DISCOVERY_SPM
               == gpphHal4Nfc_Hwref->discovery_mode)
               && ( NFC_SMARTMX_RELEASE_SPM
                    != Hal4Ctxt->sTgtConnectInfo.ReleaseType)
               )
        {
           gpphHal4Nfc_Hwref->discovery_mode =
                        NXP_NFC_DISCOVERY_NORMAL;
        }
        ...
        if(NULL != pUpperDisconnectCb)
        {
            ...
            (*pUpperDisconnectCb)(
                        Hal4Ctxt->sUpperLayerInfo.psUpperLayerDisConCtxt,
                        psConnectedDevice,
                        ConnectStatus
                        );
        }
