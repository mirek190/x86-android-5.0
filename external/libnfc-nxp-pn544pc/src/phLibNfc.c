...
/* MultiEmulation Cb */
STATIC void phLibNfc_Mgt_MultiEmulation_Config_Cb(void     *pcontext,
                                                  NFCSTATUS status);
...
NFCSTATUS phLibNfc_HW_Reset ()
{
    ...
    Status = phDal4Nfc_Reset(NFC_HIF_MODE_RST);
...
NFCSTATUS phLibNfc_Download_Mode ()
{
    ...
    Status = phDal4Nfc_Reset(NFC_HIF_MODE_FW);
...
#ifdef HOST_EMULATION
            (void)memset((void*)&gpphLibContext->CE_Info,0,sizeof(phLibNfc_CEInfo_t));
#endif
...
#if (NXP_SWP_INIT_ACTIVATION >= 0x01 )
                                    eInitDefault,
#else
                                    eInitCustom,
#endif
                                ...
                                )
{
    ...
    {
        if(NFCSTATUS_BOARD_COMMUNICATION_ERROR == status)
        {
            ...
            {
                NXP_NFC_SMX_SECONDS_DELAY(NXP_SMX_RECOVERY_DELAY);
            }
            phOsalNfc_RaiseException(phOsalNfc_e_UnrecovFirmwareErr,1);
        }
        else if (NULL == info.psEventInfo)
        {
            phLibNfc_UpdateCurState(NFCSTATUS_FAILED,gpphLibContext);
        }
        else
        {
            ...
            {
#ifdef HOST_EMULATION
                if( (info.psEventInfo->eventSource == phHal_eISO14443_A_PICC) ||
                    (info.psEventInfo->eventSource == phHal_eISO14443_B_PICC))
                {
                                   ...
                                   )
{
    ...
    else
    {
...
#ifdef HOST_EMULATION
            if(pLibContext->CE_Info.pCE_dev.psRemoteDevInfo!=NULL)
            {
                  phOsalNfc_FreeMemory(pLibContext->CE_Info.pCE_dev.psRemoteDevInfo);
            }
#endif
            phOsalNfc_FreeMemory(pLibContext);
            gpphLibContext=NULL;
        ...
        {
 //HECI Mod : deinit simplify : only release resources, no cmd send
        phHal4Nfc_Hal4Reset(pLibContext->psHwReference,(void *)pLibContext);
            if(pLibContext->psHwReference!=NULL)
            {
                phOsalNfc_FreeMemory(pLibContext->psHwReference);
                                                  ...
                                                  NFCSTATUS status)
{
    ...
    else
    {
        ...
        pClientCb = gpphLibContext->CBInfo.pClientMultiEmulationCb;
        pUpperLayerContext = gpphLibContext->CBInfo.pClientMultiEmulationCntx;

        gpphLibContext->CBInfo.pClientMultiEmulationCb = NULL;
        gpphLibContext->CBInfo.pClientMultiEmulationCntx  = NULL;
        if (NULL != pClientCb)
        {
...
NFCSTATUS phLibNfc_Mgt_MultiEmulation_Config(
...
)
{
    ...
    /*Check application has sent the valid parameters*/
    else if((NULL == pMultiSeConfig)
        || (NULL == pMultiSeConfig_RspCb)
        || (NULL == pContext))
    {
        RetVal= NFCSTATUS_INVALID_PARAMETER;
    }
    else if (NXP_NFC_MAX_AID_SUPPORT < pMultiSeConfig->numAidEntries)
    {
        RetVal= NFCSTATUS_INVALID_PARAMETER;
    }
    else if(gpphLibContext->LibNfcState.next_state == eLibNfcHalStateShutdown)
    ...
    else if(TRUE == gpphLibContext->status.GenCb_pending_status)
    {
        ...
        RetVal = NFCSTATUS_BUSY;
                                 ...
                                 < NFC44_ROM33_VER )
    {
        RetVal = NFCSTATUS_FEATURE_NOT_SUPPORTED;
    }
    else
    {
        phHal_uConfig_t uConfig;
        memcpy(&uConfig.multiEmuConfig,pMultiSeConfig,sizeof(phLibNfc_MultiEmulationCfg_t));
        RetVal = phHal4Nfc_ConfigParameters(
                                            gpphLibContext->psHwReference,
                                            NFC_SE_AID_CONFIG,
                                            &uConfig,
                                            phLibNfc_Mgt_MultiEmulation_Config_Cb,
                                            (void *)gpphLibContext);
        if(RetVal == NFCSTATUS_PENDING)
        {
            gpphLibContext->CBInfo.pClientMultiEmulationCb = pMultiSeConfig_RspCb;
            gpphLibContext->CBInfo.pClientMultiEmulationCntx = pContext;
