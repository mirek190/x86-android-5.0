                                ...
                                )
{
    ...
    else if ((NXP_NFC_DISCOVERY_SPM == gpphLibContext->psHwReference->discovery_mode )
        && (( NFCSTATUS_MULTIPLE_TAGS == status ) &&(NULL != pClientCb) )
        )
    {
            pClientCb(
    ...
    else if(NULL == info.psDiscoveryInfo)
    {
        Status = NFCSTATUS_FAILED;
    }
    else
    ...
    else if(PHNFCSTATUS(status)==NFCSTATUS_DESELECTED)
    {
        ...
        && (( NFCSTATUS_MULTIPLE_TAGS == status ) || ( NFCSTATUS_SUCCESS == status ))
        )
    {
    }
                        ...
                        )
{
    ...
    else
    {
        ...
        if (TRUE == gpphLibContext->RegNtfType.SPM)
#if (NXP_NFC_SPECIAL_POLLING_MODE == 0x01)
        {
            gpphLibContext->RegNtfType.ISO14443_4A = TRUE;
            gpphLibContext->RegNtfType.ISO14443_4B = TRUE;
            gpphLibContext->RegNtfType.MifareStd = FALSE;
            gpphLibContext->RegNtfType.MifareUL = FALSE;
            gpphLibContext->RegNtfType.ISO15693 = FALSE;
            gpphLibContext->RegNtfType.Felica = FALSE;
            gpphLibContext->RegNtfType.Jewel = FALSE;
            gpphLibContext->RegNtfType.NFC = FALSE;

            /*Register for Special discovery Mode */
            RetVal = phHal4Nfc_RegisterNotification(
                                gpphLibContext->psHwReference,
                                eRegisterSPMDiscovery,
...
#ifdef NXP_NFC_RESUME_DISCOVERY
           (void) phHal4Nfc_ConfigureDiscovery ( gpphLibContext->psHwReference,
                                            NFC_DISCOVERY_RESUME,
                                            &(gpphLibContext->sADDconfig),
                                            phLibNfc_config_discovery_cb,
                                            gpphLibContext);
    ...
    else
    {
        if((eLibNfcHalStateRelease == gpphLibContext->LibNfcState.next_state)
            ||((gpphLibContext->sSeContext.eActivatedMode == phLibNfc_SE_ActModeWired)&&
                    ((ReleaseType != NFC_SMARTMX_RELEASE) && (ReleaseType != NFC_SMARTMX_RELEASE_SPM)) )
                    ||((gpphLibContext->sSeContext.eActivatedMode != phLibNfc_SE_ActModeWired)&&
                        ((ReleaseType == NFC_SMARTMX_RELEASE) || (ReleaseType == NFC_SMARTMX_RELEASE_SPM)))
                        )
...
#if (NXP_NFC_SPECIAL_POLLING_MODE == 0x00)
        else if (ReleaseType == NFC_SMARTMX_RELEASE_SPM)
        {
                RetVal = NFCSTATUS_NOT_ALLOWED;
        }
#endif
        else
        {
                                    ...
                                    )
{
    ...
    if((phLibNfc_LibContext_t *)context == gpphLibContext)
    {
        ...
        {
            trans_status = NFCSTATUS_MORE_INFORMATION;
        }
        else if((PHNFCSTATUS(status) != NFCSTATUS_SUCCESS) &&
        ...
        if(NFCSTATUS_PENDING != trans_status)
        {
            ...
            if (NULL != pUpper_TagNtfCb)
            {
                 ...
                 )
                {
                    ...
                    pUpper_Context = gpphLibContext->CBInfo.pClientTranseCntx;
#ifndef  NXP_NFC_SW_MOREINFO
                    trans_resp->buffer = response->buffer;
#else
                    (void)memcpy((void *)(trans_resp->buffer),
                            (void *)response->buffer, response->length);
#endif
