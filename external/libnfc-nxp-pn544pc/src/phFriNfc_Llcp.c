...
#define PHFRINFC_LLCP_SLOW_SYMMETRY                8
/*@}*/
...
static NFCSTATUS phFriNfc_Llcp_InternalDeactivate( phFriNfc_Llcp_t *Llcp )
{
       ...
       (Llcp->state == PHFRINFC_LLCP_STATE_ACTIVATION))
   {
      ...
      status=phFriNfc_LlcpMac_Deactivate(&Llcp->MAC);
      phFriNfc_LlcpMac_Reset(&Llcp->MAC,
                              Llcp->pLowerDevice,
...
static NFCSTATUS phFriNfc_Llcp_SendSymm( phFriNfc_Llcp_t *Llcp )
{
   ...
   LLCP_PRINT("phFriNfc_Llcp_SendSymm - Check for pending messages to send");
   /* Check for pending messages to send */
   bPendingFlag = phFriNfc_Llcp_HandlePendingSend(Llcp);

   if (bPendingFlag == FALSE)
   {
      LLCP_PRINT("phFriNfc_Llcp_SendSymm - phFriNfc_Llcp_HandlePendingSend returned FALSE");
      ...
      LLCP_PRINT("phFriNfc_Llcp_SendSymm - No send pending, send a SYMM instead");
   ...
   {
      LLCP_PRINT("phFriNfc_Llcp_SendSymm - A pending send has been sent, there is no need to send SYMM");
...
static void phFriNfc_Llcp_Timer_CBRecv(uint32_t TimerId, void *pContext)
{
...
static void phFriNfc_Llcp_Timer_CBSend(uint32_t TimerId, void *pContext)
{
   phFriNfc_Llcp_t               *Llcp = (phFriNfc_Llcp_t*)pContext;
                                                 ...
                                                 phNfc_sData_t   *psParamsTLV)
{
   ...
   LLCP_DEBUG("%s",__func__);

   if(psParamsTLV->length > (uint32_t)(PHFRINFC_LLCP_PACKET_MAX_SIZE - PHFRINFC_LLCP_PACKET_HEADER_SIZE) )
   {
      LLCP_PRINT("Error: over llcp packet size");
   ...
   else
   {
      ...
      if (status != NFCSTATUS_SUCCESS)
      {
         /* Error: invalid parameters TLV */
            LLCP_PRINT("Error: invalid parameters TLV");
         status = NFCSTATUS_FAILED;
      }
      else
      {
...
static bool_t phFriNfc_Llcp_HandlePendingSend ( phFriNfc_Llcp_t *Llcp )
{
   ...
   LLCP_DEBUG("\nphFriNfc_Llcp_HandlePendingSend - LLCP Val: %x",Llcp);
   /* Handle pending disconnection request */
   if (Llcp->bDiscPendingFlag == TRUE)
   {
      LLCP_PRINT("\nphFriNfc_Llcp_HandlePendingSend - Handle pending disconnection request");
   ...
   {
      LLCP_PRINT("\nphFriNfc_Llcp_HandlePendingSend - Handle pending frame reject request");
   ...
   {
      LLCP_PRINT("\nphFriNfc_Llcp_HandlePendingSend - Handle pending service frame");
   ...
   {
       LLCP_PRINT("\nphFriNfc_Llcp_HandlePendingSend - psSendHeader == NULL ! Hence No Send");
   }
                              ...
                              void                             *pContext )
{
   ...
   {
      /* Ready to send */
      LLCP_PRINT("phFriNfc_Llcp_Send - Ready to Send");
      if(NULL != psInfo)
      {
         LLCP_PRINT_BUFFER("phFriNfc_Llcp_Send -  Buffer ", &(psInfo->buffer[0]), psInfo->length);
      }
      else
      {
         LLCP_PRINT("phFriNfc_Llcp_Send - Ready to Send - psInfo is NULL!");
      }
      result = phFriNfc_Llcp_InternalSend(Llcp, psHeader, psSequence, psInfo);
   }
   else if (Llcp->state == PHFRINFC_LLCP_STATE_OPERATION_RECV)
   {
      LLCP_PRINT("phFriNfc_Llcp_Send - Not ready to send, save send params for later use ");
   ...
   {
      LLCP_PRINT("phFriNfc_Llcp_Send - Incorrect state for sending!!! ");
      Llcp->pfSendCB = NULL;
      Llcp->pSendContext = NULL;
