                                   ...
                                   )
{
    ...
    if(status == NFCSTATUS_SUCCESS)
    {
...
#endif/*#ifdef MERGE_SAK_SW1*/
        {
            ...
            Hal4Ctxt->hal_discovery_mode = NXP_NFC_DISCOVERY_NORMAL;
    ...
    {
        /*CG*/ ALOGE("HAL4NFC : phHal4Nfc_OpenComplete  failed");
        Hal4Ctxt->Hal4CurrentState = eHal4StateClosed;
        Hal4Ctxt->Hal4NextState = eHal4StateInvalid;
        if (gpphHal4Nfc_Hwref)
        {
            // CG FIX : SC38953
            if ((status & PHNFCSTBLOWER) == (NFCSTATUS_BOARD_COMMUNICATION_ERROR & PHNFCSTBLOWER))
            {
                ALOGE("launch phHciNfc_Release");
                phHciNfc_Release(
...
int dlopen_firmware() {
    ...
    handle = phOsalNfc_DlOpen("/system/vendor/firmware/libpn544_fw.so");
    if (handle == NULL) {
        return -1;
    }

    p0 = phOsalNfc_DlGetAddress(handle, "nxp_nfc_full_version");
    if (p0 == NULL) {
        return -1;
    }

    p1 = phOsalNfc_DlGetAddress(handle, "nxp_nfc_fw");
    if (p1 == NULL) {
        return -1;
    }

    nxp_nfc_full_version = (unsigned char *)p0;
    nxp_nfc_fw = (unsigned char *)p1;
                         ...
                         )
{
    ...
    {
#ifdef ANDROID
        /* Recover addresses of symbols for firmware update */
        dlopen_firmware();
#endif
        ...
        if(NULL == ((phHal_sHwReference_t *)psHwReference)->hal_context)
        {
            ...
            // CG FIX : SC29488
            // In case Hci Init failed, this pointer is invalid !
...
// CG FIX : SC29488
            ...
            // Update the value. In case of failure this value is NULL.
            ((phHal_sHwReference_t *)psHwReference)->hal_context = Hal4Ctxt;
// END CG FIX : SC29488
        }
    }
                          ...
                          )
{
    ...
    else/*Status is Initialised*/
    {
        ...
        switch(IoctlCode)
        {
            ...
            {
#define NFC_ADDRESS_SMX_PRESENCE 0x9B66
                if((eHal4StateSelfTestMode == Hal4Ctxt->Hal4CurrentState)
            ...
            case NFC_SWP_STATUS:
            {
                RetStatus = phHciNfc_Get_Link_Status(Hal4Ctxt->psHciHandle,
                    (void *)psHwReference, phHal_eUICCHost, pOutParam->buffer);
                break;
            }
            case NFC_ENABLE_DTA_MODE:
            {

                Hal4Ctxt->hal_discovery_mode = !(pInParam->buffer[0])?
                                     NXP_NFC_DISCOVERY_NORMAL : NXP_NFC_DISCOVERY_DTA ;
                                    ...
                                    )
{
    ...
    else
    {
        ...
        switch(type)
        {
            ...
            {
                PHDBG_INFO("Hal4:Target released");
                phHal4Nfc_TargetReleaseComplete(Hal4Ctxt,pInfo);
                break;
            }
#endif/*NXP_KOVIO*/
                                       ...
                                       )
{
    ...
    phNfc_sData_t *SelfTestResults = NULL;
    pphHal4Nfc_IoctlCallback_t pUpper_IoctlCb = NULL;
    void  *pUpper_Context = NULL;

    /* check for Success */
    if((NULL == pInfo) || (NULL == Hal4Ctxt))
    {
        phOsalNfc_RaiseException(phOsalNfc_e_PrecondFailed,1);
        return;
    }
                                    ...
                                    )
{
...
#endif/*#ifdef MERGE_SAK_SW2*/
    {
        ...
        if ((NFC_MEM_READ == Hal4Ctxt->Ioctl_Type)
            ...
            || (NFC_SWP_STATUS == Hal4Ctxt->Ioctl_Type)
            || (DEVMGMT_NFCWI_TEST == Hal4Ctxt->Ioctl_Type)
            )
        {
            Hal4Ctxt->sUpperLayerInfo.pIoctlOutParam->length
                = sizeof (uint8_t);
