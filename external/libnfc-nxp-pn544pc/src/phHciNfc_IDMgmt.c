...
NFCSTATUS
phHciNfc_IDMgmt_Valid_Gate(
                                phHciNfc_sContext_t     *psHciContext,
                                phHciNfc_GateID_t       gate_info
                             )
{
    ...
    if( (NULL != psHciContext)
        && ( phHciNfc_UnknownGate != gate_info )
        && ( NULL != psHciContext->p_identity_info )
      )
    {
        uint8_t     gate_index = 0x00;
        uint8_t     *gate_list = NULL;

        p_identity_info = (phHciNfc_IDMgmt_Info_t *)
                            psHciContext->p_identity_info ;
        gate_list = p_identity_info->gates_list;
        for(gate_index = 0; gate_index < GATES_LIST_LEN ; gate_index++)
        {
            ...
            if((NXP_NFC_ROM_INFO((
               ...
               < NFC44_ROM33_VER ))
            {
                if(( phHciNfc_CETypeAGate == gate_info)
                    || ( phHciNfc_CETypeBGate == gate_info) )
                {
                    break;
                }
            }
#endif
            if( *(gate_list + gate_index) == (uint8_t)gate_info)
            {
