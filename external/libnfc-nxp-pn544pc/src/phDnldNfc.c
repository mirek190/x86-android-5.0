...
#define DO_DELAY(period)        { unsigned static i=0; for(;i<(period);i++) {}; }

#ifndef PHDNLD_DNLD_DELAY
#define PHDNLD_DNLD_DELAY        0x2000U
#endif

#elif !defined (NXP_FW_RESET_DELAY)

#define PHDNLD_DNLD_DELAY        0x2000U

#else

#define PHDNLD_DNLD_DELAY        NXP_FW_RESET_DELAY
...
//#define PHDNLD_CMD_RESET_DNLD           0x42U
// HECI Mod
#define PHDNLD_CMD_RESET_TO_DOWNLOAD    0x42U
// END HECI Mod
...
#ifdef NXP_FW_DNLD_DIAGNOSTICS

#define   NXP_FW_DNLD_TRIG_ADDR            0x9FF4U
#define   NXP_FW_DNLD_TOTAL_FAILURE_ADDR   0x9FF5U
#define   NXP_FW_DNLD_FAILURE_ADDR         0x9FF6U

static uint8_t  gphDnldFailure = 0x00;

#endif /* #ifdef NXP_FW_DNLD_DIAGNOSTICS */
...
#if defined(NXP_FW_SW_VMID_TRIM) || defined(NXP_FW_DNLD_DIAGNOSTICS)
    phHal_sHwReference_t    *p_hw_ref =
                            ((phHal_sHwReference_t *) pHwRef);
    uint32_t                trim_addr = 0;
#endif /* End of Define check */
    static unsigned         sec_type = 0;
    uint8_t                 i = 0;
...
*/
                      case NXP_FW_VMID_TRIM_CHK_ADDR:
                        {
                          psDnldContext->vmid_trim_update =
                                p_sm_trim_data[p_sec_info->p_trim_data[i+1]] ;
                          break;
                      }
                      case NXP_FW_VMID_CARD_MODE_ADDR:
                      case NXP_FW_VMID_RD_MODE_ADDR :
                      {
...
#if defined(NXP_FW_DNLD_DIAGNOSTICS)
                      case NXP_FW_DNLD_TRIG_ADDR:
                      {
                          dnld_data->data_packet[p_sec_info->p_trim_data[i+1]] =
                                    p_sm_trim_data[p_sec_info->p_trim_data[i+1]] + 1;
                          break;
                      }
                      case NXP_FW_DNLD_FAILURE_ADDR:
                      {
                          if (!p_hw_ref->device_info.fw_version)
                          {
                              dnld_data->data_packet[p_sec_info->p_trim_data[i+1]] =
                                    p_sm_trim_data[p_sec_info->p_trim_data[i+1]] + 1;
                          }
                      ...
                      case NXP_FW_DNLD_TOTAL_FAILURE_ADDR:
                      {
                          if (!p_hw_ref->device_info.fw_version)
                          {
                             dnld_data->data_packet[p_sec_info->p_trim_data[i+1]] =
                 ...
                 } /* End of Switch Block if Enabled */
            }
            if(NULL != psDnldContext->trim_store.buffer)
            {
...
{
    uint8_t  chunk_present = FALSE;
    PHNFC_UNUSED_VARIABLE(frame_data);
    // HECI Mod
    if( (max_frame - FRAME_HEADER_LEN) >= (frame_length
            - frame_offset))
    //if( (max_frame - FRAME_HEADER_LEN) >= (uint16_t)(frame_length - frame_offset))
    // End HECI Mod
 {
                      ...
                      )
{
    ...
    PHNFC_UNUSED_VARIABLE(frame_offset);


    switch(raw_frame_hdr->frame_type)
    {
    // HECI Mod
    case PHDNLD_CMD_RESET_TO_DOWNLOAD:
    // END HECI Mod
        case PHDNLD_CMD_RESET:
    //FA HECI Mod
    /*case PHDNLD_CMD_RESET_DNLD:
        {
            DO_DELAY(PHDNLD_DNLD_DELAY);
            break;
        }*/
        ...
        case PHDNLD_CMD_SEC_WRITE:
        {
            ...
            if( TRUE == chain )
            {
                ...
                if(NFCSTATUS_PENDING == status)
                {
                    ...
                    psDnldContext->tx_info.tx_offset =  (uint16_t)
...
#define PHDNLD_MARKER_MASK         0x0FU
            ...
            )
       {
                        ...
                        (frame_type & PHDNLD_MARKER_MASK))
            {
                if(TRUE != skip_frame)
                {
                   psDnldContext->raw_mode_upgrade = (uint8_t)
                       (psDnldContext->raw_mode_upgrade &
                              ~(frame_type & ~PHDNLD_MARKER_MASK)) ;
                }
        ...
        else
        {
            ...
            switch(resp_data->frame_type)
            {
                case PHDNLD_RESP_SUCCESS:
                {
                    ...
                    switch ( psDnldContext->prev_cmd )
                    {
                        ...
                        case PHDNLD_CMD_CHECK_INTEGRITY :
                        {
                            ...
                            {
                                /*
                                 * Check Integrity Success hence perform the setup and
                                 * Continue with the advanced Operation.
                                 */
                                psDnldContext->raw_mode_upgrade =
                        ...
                        default:
                        {
                            ...
                            {
                                /*
                                 * Check/Patch Success hence perform the setup and
                                 * Continue with the normal/advanced Operation.
                                 */
                                psDnldContext->raw_mode_upgrade =
                ...
                case PHDNLD_RESP_CHKSUM_ERROR:
                {
                    ...
                    {
                        /*
                         * Checksum Error hence cannot continue with
                         * any operation.
                         */
                        psDnldContext->dnld_retry = NXP_MAX_DNLD_RETRY;
                        status = PHNFCSTVAL(CID_NFC_DNLD,
                       ...
                       )
{
        ...
        )
    {
        ...
        if(status != NFCSTATUS_SUCCESS)
        {
            ...
            comp_info.status = status;
            DNLD_DEBUG(" FW_DNLD: Termination in Send Complete, Status = %X \n", status);
            status = phDal4Nfc_Unregister(
                        psDnldContext->lower_interface.pcontext, pHwRef);
            phDnldNfc_Release_Lower(psDnldContext, pHwRef);
            phDnldNfc_Release_Resources(&psDnldContext);
        ...
        else
        {
            ...
            if ((PHDNLD_CMD_SET_HIF != psDnldContext->prev_cmd)
                ...
                // HECI Mod
                //&& (PHDNLD_CMD_RESET_DNLD != psDnldContext->prev_cmd))
                 && (PHDNLD_CMD_RESET_TO_DOWNLOAD != psDnldContext->prev_cmd))
                // End HECI Mod
            {
                psDnldContext->rx_info.rx_total = 0;
                status = phDnldNfc_Receive( psDnldContext, pHwRef,
            ...
            else
            {
                ...
                if(( PHDNLD_CMD_RESET == psDnldContext->prev_cmd )|| (PHDNLD_CMD_RESET_TO_DOWNLOAD == psDnldContext->prev_cmd))
                // END HECI Mod
                {
                    DO_DELAY(PHDNLD_DNLD_DELAY);
                }
