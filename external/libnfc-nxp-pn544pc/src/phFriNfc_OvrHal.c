...
*/
#define MAX_MIF_PACKET_LEN                      0x1BU
...
#define NXP_NFC_ISO_EXCH_TIMEOUT              300U
#define NXP_NFC_FELICA_DEFAULT_TIMEOUT        255U
#define NXP_NFC_DEFAULT_EXCH_TIMEOUT          300U
    ...
    else
    {
        ...
        switch(RemoteDevInfo->RemDevType)
        {
            ...
            case phHal_eISO14443_3A_PICC:
            {
                ...
                OvrHal->TranceiveInfo.timeout = NXP_NFC_ISO_EXCH_TIMEOUT;
                break;
            }
            case phHal_eISO14443_A_PICC:
            case phHal_eISO14443_4A_PICC:
            {
                if(OvrHal->TranceiveInfo.timeout < NXP_NFC_ISO_EXCH_TIMEOUT)
                {
                    OvrHal->TranceiveInfo.timeout = NXP_NFC_ISO_EXCH_TIMEOUT;
                }
                OvrHal->TranceiveInfo.sSendData.buffer = &SendBuf[i++];
            ...
            {
                OvrHal->TranceiveInfo.timeout = NXP_NFC_FELICA_DEFAULT_TIMEOUT;
                OvrHal->TranceiveInfo.sSendData.buffer = &SendBuf[i++];
                OvrHal->TranceiveInfo.sSendData.length = length;
                break;
            }
            default:
            {
                OvrHal->TranceiveInfo.timeout = NXP_NFC_DEFAULT_EXCH_TIMEOUT;
                OvrHal->TranceiveInfo.sSendData.buffer = &SendBuf[i++];
                OvrHal->TranceiveInfo.sSendData.length = length;
                break;
            }
...

{
    ...
    if (NULL != OvrHal)
    {
        if((NULL != pRecvdata) && (OvrHal->TranceiveInfo.sRecvData.buffer != NULL) &&
             (pRecvdata->buffer != NULL) && (OvrHal->pndef_recv_length != NULL)
             )
        {
            if ((OvrHal->TranceiveInfo.sRecvData.buffer != pRecvdata->buffer))
            {
                /* Work-around for the NFCIP Tranceive API */
                memcpy(OvrHal->TranceiveInfo.sRecvData.buffer,
                                pRecvdata->buffer, pRecvdata->length);
            }
            *(OvrHal->pndef_recv_length) = pRecvdata->length;
        }
                                       ...
                                       NFCSTATUS status)
{
    ...
    if (NULL != OvrHal)
    {
        ...
        if((NULL != pDataInfo) && (OvrHal->sReceiveData.buffer != NULL) &&
            (pDataInfo->buffer != NULL) && (OvrHal->pndef_recv_length != NULL))
        {
...
NFCSTATUS
phFriNfc_OvrHal_SetParam(phFriNfc_OvrHal_t *OvrHal,
                         phNfc_eOverHal_SetParam_t param_type,
                         void  *p_param_info
                        )
{
        ...
        (NULL != p_param_info))
    {
        switch(param_type)
        {
            case OVRHAL_TIMEOUT_CONFIG:
            {
                OvrHal->TranceiveInfo.timeout = *((uint16_t *)p_param_info);
