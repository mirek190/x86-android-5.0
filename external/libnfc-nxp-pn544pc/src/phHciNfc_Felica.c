...
#define FELICA_POLL_REQ_DATA_SIZE           0x06U
                            ...
                            )
{
    ...
    switch(index)
    {
...
#if defined(NXP_NFC_POLL_FELICA_SYSCODE)
                p_fel_tag_info->SystemCode[i] =
                        *(reg_value + reg_length - 1 - i);
                i++;
                p_fel_tag_info->SystemCode[i] =
                        *(reg_value + reg_length - 1 - i);
#else
                /* System Code in the Registry may be invalid during the initial
                 * detection of the Felica Card */
                p_fel_tag_info->SystemCode[i] = 0x00;
                i++;
                p_fel_tag_info->SystemCode[i] = 0x00;
        ...
        case NXP_FEL_POLREQ_SYS_CODE:
        {
            if (NXP_FEL_SYS_CODE_LEN == reg_length)
            {
                uint8_t     i = 0;

                p_fel_info->pollreq_systemcode = ( (*(reg_value + i) << BYTE_SIZE ) |
                     *(reg_value + i + 1));

                HCI_PRINT_BUFFER("\tFelica Poll Request system code ", reg_value, reg_length);
                /* Update current system code values */
            }
            else
            {
...
{
    NFCSTATUS           status = NFCSTATUS_SUCCESS;
#if defined(FELICA_REQUEST_MODE)
    static uint8_t      pres_chk_data[(PHHAL_FEL_ID_LEN + 2)] = {0};
#else
    static uint8_t      pres_chk_data[FELICA_POLL_REQ_DATA_SIZE] = {0};
#endif
                              ...
                              void                  *pHwRef)
{
    ...
    else
    {
        ...
        else
        {
            ...
            pres_chk_data[i++] = 0xFF;
            pres_chk_data[i++] = 0xFF;
#if( FELICA_SYS_CODE_RESPONSE_ENABLE >= 0x01 )
            pres_chk_data[i++] = 0x01;
#else
            pres_chk_data[i++] = 0x00;
#endif
            pres_chk_data[i++] = 0x00;
...
NFCSTATUS
phHciNfc_Felica_Syscode_Config(
                              phHciNfc_sContext_t   *psHciContext,
                              void                  *pHwRef,
                              uint8_t               *sys_code
                              )
{
    ...
    {
        phHciNfc_Felica_Info_t     *p_felica_info=NULL;
        phHciNfc_Pipe_Info_t        *p_pipe_info=NULL;
        uint8_t                     pipeid = 0;

        p_felica_info = (phHciNfc_Felica_Info_t *)
                                psHciContext->p_felica_info ;
        p_pipe_info = p_felica_info->p_pipe_info;
        p_pipe_info->reg_index = NXP_FEL_POLREQ_SYS_CODE;
        p_pipe_info->param_info = sys_code;
        p_pipe_info->param_length = NXP_FEL_SYS_CODE_LEN;
        pipeid = p_felica_info->pipe_id ;
                              ...
                              )
{
    ...
    phHciNfc_sContext_t         *psHciContext =
                                (phHciNfc_sContext_t *)psContext ;
    uint16_t                    timeout = NXP_FELICA_XCHG_TIMEOUT;
    if( (NULL == psHciContext) || (NULL == pHwRef) )
    {
        status = PHNFCSTVAL(CID_NFC_HCI, NFCSTATUS_INVALID_PARAMETER);
    }
    ...
    else
    {
        ...
        /* setting the felica timeout to default value */
        if((psHciContext->p_xchg_info != NULL)
          && ((psHciContext->p_xchg_info->timeout > timeout)
            || (0 == psHciContext->p_xchg_info->timeout))
           )
        {
            ...
            switch(cmd)
            {
                case NXP_FELICA_RAW:
                {
                           ...
                           >= NFC44_ROM33_VER ))
                    {
                        ...
                        TO -              Time out (1 byte)
                        Multipler -       Multiplier (1 byte)
                        Data  -           params received from this function
                        */
                        while(timeout > NXP_FELICA_XCHG_TIMEOUT)
                        {
                            timeout = timeout >> 1;
                        ...
                        /* Timeout Multiplier */
                        hcp_message->payload[i++] = (multiplier-1);
                    }
                    else
                    {
                        /*
                        ...
                        */
                        ...
                        /* Time out */
                        hcp_message->payload[i++] = (uint8_t)((timeout > NXP_FELICA_XCHG_TIMEOUT)?NXP_FELICA_XCHG_TIMEOUT:timeout);
                        /* Status */
                    ...
                    length = (length + i + p_pipe_info->param_length);
                    break;
                }
                case NXP_FELICA_CMD:
                {
