    ...
    else
    {
        ...
        else
        {
            if(status == NFCSTATUS_SUCCESS)
            {
                ...
                && (TRUE == ps_rem_dev_info->ConnectWithPresence))
            {
                                         ...
                                         )
{
    ...
    ret_val = phLibNfc_ConvertToReadOnlyNdef_Extension(hRemoteDevice,
                                                       NULL,
                                                       pNdefReadOnly_RspCb,
                                                       pContext);
    return ret_val;
...
NFCSTATUS
phLibNfc_ConvertToReadOnlyNdef_Extension(
    phLibNfc_Handle         hRemoteDevice,
    void*                   pParam,
    pphLibNfc_RspCb_t       pNdefReadOnly_RspCb,
    void*                   pContext
    )
{
    ...
    else
    {
        ...
        NdefMap = gpphLibContext->ndef_cntx.psNdefMap;
        ...
        if(eLibNfcHalStatePresenceChk != gpphLibContext->LibNfcState.next_state)
        {
            phHal_sRemoteDevInformation_t      *ps_rem_dev_info =
                                                   (phHal_sRemoteDevInformation_t *)hRemoteDevice;
            uint8_t                             fun_id;
            NdefMap->StdMifareContainer.pReadOnlyParam = pParam;
                        ...
                        && (0x00 != ps_rem_dev_info->RemoteDevInfo.Iso14443A_Info.Sak))
                    {
                        for (fun_id = 0; fun_id < PH_FRINFC_NDEFMAP_CR; fun_id++)
                        {
                            ...
                            ret_val = phFriNfc_NdefMap_SetCompletionRoutine (
                                      NdefMap,fun_id, phLibNfc_Ndef_ReadOnly_Cb,
                                      (void *)gpphLibContext);
                        }
                        ret_val = phFriNfc_NdefMap_ConvertToReadOnly(NdefMap);
                    }
                    else
                    {
                        gpphLibContext->ndef_cntx.NdefSendRecvLen = NDEF_SENDRCV_BUF_LEN;
