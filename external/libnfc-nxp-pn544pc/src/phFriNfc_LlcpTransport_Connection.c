                                                                  ...
                                                                  NFCSTATUS    status)
{
   ...
   if(status == NFCSTATUS_SUCCESS)
   {
      ...
      case phFriNfc_LlcpTransportSocket_eSocketAccepted:
         {
            LLCP_PRINT("\nLLCP: phFriNfc_LlcpTransport_ConnectionOriented_SendLlcp_CB ACCEPT SUCCESS");
            if(NULL != psTransport->pSocketTable[psTransport->socketIndex].pfSocketAccept_Cb)
            {
                LLCP_DEBUG("\nLLCP: phFriNfc_LlcpTransport_ConnectionOriented_SendLlcp_CB ACCEPT CB NULL: TRANSPORT SOCKET INDEX %x",psTransport->socketIndex);
      ...
      case phFriNfc_LlcpTransportSocket_eSocketConnected:
         {
            if(!psTransport->pSocketTable[socketIndex].bSocketSendPending)
            {
               phFriNfc_LlcpTransport_ConnectionOriented_TriggerSendCb(&psTransport->pSocketTable[socketIndex], status);
            }
         }break;
      default:
         /* Nothing to do */
...
NFCSTATUS phFriNfc_LlcpTransport_ConnectionOriented_HandlePendingOperations(phFriNfc_LlcpTransport_Socket_t *pSocket)
{
   ...
   LLCP_DEBUG("\nphFriNfc_LlcpTransport_ConnectionOriented_HandlePendingOperations: ENTRY   pSocket Val: %x",pSocket);
   ...
   else if(pSocket->bSocketAcceptPending == TRUE)
   {
      if (!testAndSetSendPending(psTransport))
      {
         ...
         /* Send Pending */
         LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_HandlePendingOperations - CC FRAME - psTransport->bSendPending = TRUE");
   ...
   else if(pSocket->bSocketConnectPending == TRUE)
   {
      if (!testAndSetSendPending(psTransport))
      {
         ...
         LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_HandlePendingOperations - CONNECT FRAME -psTransport->bSendPending = TRUE");
   ...
   else if(pSocket->bSocketDiscPending == TRUE)
   {
      if (!testAndSetSendPending(psTransport))
      {
         ...
         /* Send Pending */
         LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_HandlePendingOperations-DISC FRAME - psTransport->bSendPending = TRUE");
   ...
   LLCP_DEBUG("\nphFriNfc_LlcpTransport_ConnectionOriented_HandlePendingOperations: EXIT Result: %x",result);
   return result;
}
...
static NFCSTATUS static_performSendInfo(phFriNfc_LlcpTransport_Socket_t * psLlcpSocket)
{
   ...
   LLCP_PRINT("static_performSendInfo - psTransport->bSendPending = TRUE");
...
static NFCSTATUS phFriNfc_Llcp_Send_ReceiveReady_Frame(phFriNfc_LlcpTransport_Socket_t*    pLlcpSocket)
{
   ...
   else
   {
      ...
      else if (NFCSTATUS_PENDING == status)
      {
         ...
         LLCP_PRINT("phFriNfc_Llcp_Send_ReceiveReady_Frame - psTransport->bSendPending = TRUE");
         /* Store the index of the socket */
...
static NFCSTATUS phFriNfc_Llcp_Send_ReceiveNotReady_Frame(phFriNfc_LlcpTransport_Socket_t*   pLlcpSocket)
{
   ...
   else
   {
      ...
      else if (NFCSTATUS_PENDING == status)
      {
         /* Send Pending */
         pLlcpSocket->psTransport->bSendPending = TRUE;
         LLCP_PRINT("phFriNfc_Llcp_Send_ReceiveNotReady_Frame - psTransport->bSendPending = TRUE");
         /* Store the index of the socket */
         pLlcpSocket->psTransport->socketIndex = pLlcpSocket->index;
      }
   }
...

{
   ...
   LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Accept - start");
   ...
   if(testAndSetSendPending(pLlcpSocket->psTransport))
   {
      LLCP_DEBUG("phFriNfc_LlcpTransport_ConnectionOriented_Accept -  pLlcpSocket->socket_dSap = %x",pLlcpSocket->socket_dSap);
      LLCP_DEBUG("phFriNfc_LlcpTransport_ConnectionOriented_Accept -  pLlcpSocket->socket_sSap = %x",pLlcpSocket->socket_sSap);
      LLCP_DEBUG("phFriNfc_LlcpTransport_ConnectionOriented_Accept -  pLlcpSocket->index = %x",pLlcpSocket->index);

      LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Accept -  send is pending");
      LLCP_PRINT_BUFFER(" Buffer ", &(pLlcpSocket->sSocketSendBuffer.buffer[0]),pLlcpSocket->sSocketSendBuffer.length);
      ...
      LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Accept -  Update Send Buffer length value");
      LLCP_PRINT_BUFFER(" Buffer ", &(pLlcpSocket->sSocketSendBuffer.buffer[0]),pLlcpSocket->sSocketSendBuffer.length);
   ...
   {
      LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Accept - psTransport->bSendPending != TRUE");
      ...
      status =  phFriNfc_LlcpTransport_LinkSend(pLlcpSocket->psTransport,
                                   ...
                                   pLlcpSocket->index,
                                   pLlcpSocket->psTransport);
      LLCP_DEBUG("\nLLCP: phFriNfc_LlcpTransport_ConnectionOriented_Accept  SEND ACCEPT STATUS %x",status);
      ...
      else if(NFCSTATUS_PENDING == status)
      {
         ...
         /* Store the index of the socket */
         pLlcpSocket->psTransport->socketIndex = pLlcpSocket->index;
         LLCP_DEBUG("\nLLCP: phFriNfc_LlcpTransport_ConnectionOriented_Accept TRANSPORT SOCKET INDEX %x",pLlcpSocket->psTransport->socketIndex);
      }
   }
                                                             ...
                                                             void*                                      pContext)
{
   ...
   LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Connect - start");
   ...
   if(testAndSetSendPending(pLlcpSocket->psTransport))
   {
      LLCP_DEBUG("phFriNfc_LlcpTransport_ConnectionOriented_Connect -  pLlcpSocket->socket_dSap = %x",pLlcpSocket->socket_dSap);
      LLCP_DEBUG("phFriNfc_LlcpTransport_ConnectionOriented_Connect -  pLlcpSocket->socket_sSap = %x",pLlcpSocket->socket_sSap);
      LLCP_DEBUG("phFriNfc_LlcpTransport_ConnectionOriented_Connect -  pLlcpSocket->index = %x",pLlcpSocket->index);
      LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Connect - send is pending");
      ...
      LLCP_PRINT_BUFFER(" Buffer After Update Send Buffer length value ",
                           &(pLlcpSocket->sSocketSendBuffer.buffer[0]),pLlcpSocket->sSocketSendBuffer.length);

      status = NFCSTATUS_PENDING;
   }
   else
   {
      LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Connect - psTransport->bSendPending != TRUE");
      ...
      status =  phFriNfc_LlcpTransport_LinkSend(pLlcpSocket->psTransport,
                                   ...
                                   pLlcpSocket->index,
                                   pLlcpSocket->psTransport);
      LLCP_DEBUG("\nLLCP: phFriNfc_LlcpTransport_ConnectionOriented_Connect  SEND CONNECT STATUS %x",status);
      if (status != NFCSTATUS_SUCCESS && status != NFCSTATUS_PENDING) {
          clearSendPending(pLlcpSocket->psTransport);
      }
      else if(NFCSTATUS_PENDING == status)
      {
        ...
        /* Store the index of the socket */
        pLlcpSocket->psTransport->socketIndex = pLlcpSocket->index;
        LLCP_DEBUG("\nLLCP: phFriNfc_LlcpTransport_ConnectionOriented_Connect TRANSPORT SOCKET INDEX %x",pLlcpSocket->psTransport->socketIndex);
      }
   }
                                                               ...
                                                               void*                                      pContext)
{
   ...
   LLCP_PRINT("phLibNfc_LlcpTransport_ConnectionOriented_Disconnect - start");
   ...
   else
   {
      ...
      } else
      {
            ...
            LLCP_PRINT("phLibNfc_LlcpTransport_ConnectionOriented_Disconnect - psTransport->bSendPending = TRUE");
            /* Store the index of the socket */
            pLlcpSocket->psTransport->socketIndex = pLlcpSocket->index;
      }
   }

   LLCP_PRINT("phLibNfc_LlcpTransport_ConnectionOriented_Disconnect - end");
   return status;
}
...
NFCSTATUS phFriNfc_LlcpTransport_ConnectionOriented_Close(phFriNfc_LlcpTransport_Socket_t*   pLlcpSocket)
{
   ...
   LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Close - start");

   if(pLlcpSocket->eSocket_State == phFriNfc_LlcpTransportSocket_eSocketConnected)
   {
      LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Close - Socket Connected - calling - phLibNfc_LlcpTransport_ConnectionOriented_Disconnect");
   ...
   else
   {
      ...
      LLCP_PRINT("phFriNfc_LlcpTransport_ConnectionOriented_Close - Socket not Connected - calling - phFriNfc_LlcpTransport_ConnectionOriented_Abort");
      phFriNfc_LlcpTransport_ConnectionOriented_Abort(pLlcpSocket);
