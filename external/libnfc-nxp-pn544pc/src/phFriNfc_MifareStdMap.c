...
static void phFriNfc_MifStd_H_ChkNdefCmpltSects(phFriNfc_NdefMap_t *NdefMap)
{
    ...
    if(NdefMap->CardType == PH_FRINFC_NDEFMAP_MIFARE_STD_4K_CARD)
    {
        for(index = PH_FRINFC_MIFARESTD_SECTOR_NO1;
            index < PH_FRINFC_MIFARESTD4K_TOTAL_SECTOR; index++)
        {
            /* For Mifare 4k, Block 0 to 31 contains 4 blocks */
            /* sector 0 and 16 are aid blocks */
               ...
               phFriNfc_NdefMap_t *NdefMap)
{
    ...
    phNfc_sData_t *ScrtKeyB = NdefMap->StdMifareContainer.pReadOnlyParam;
    ...
    else
    {
        ...
        if((ScrtKeyB ==NULL)||
           (ScrtKeyB->buffer == NULL) ||
           (ScrtKeyB->length != PH_FRINFC_MIFARESTD_KEY_LEN))
        {
        ...
        {
            memcpy (NdefMap->StdMifareContainer.UserScrtKeyB, ScrtKeyB->buffer, PH_FRINFC_MIFARESTD_KEY_LEN);
        }
