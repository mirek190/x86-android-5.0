...
/* Event */
#define NXP_EVT_NFC_SND_DATA                NXP_EVT_WR_SND_DATA
#define NXP_EVT_NFC_ACTIVATED               0x02U
#define NXP_EVT_NFC_DEACTIVATED             0x03U
#define NXP_EVT_NFC_RCV_DATA                NXP_EVT_WR_RCV_DATA
#define NXP_EVT_NFC_CONTINUE_MI             NXP_EVT_WR_CONTINUE_MI
                         ...
                         )
{
    ...
    else
    {
        ...
        else
        {
            ...
            hcp_message->payload[i] = sData->more_info;
            i++;
    ...
    switch(index)
    {
        ...
        case NXP_NFCIP_PARAM:
        {
            if (sizeof(*reg_value) == reg_length)
            {
                ...
                if( PHHAL_MAX_DATASIZE == p_nfcipinfo->max_frame_len)
                {
                    /*
                     * Digital Protocol Definition for Maximum Data size if
                     * the size is Maximum Frame Length is 256 should be reverted
                     * back to 254 bytes
                     * */
                    p_nfcipinfo->max_frame_len = p_nfcipinfo->max_frame_len - 2;
                }
                if (NULL != psHciContext->p_target_info)
                {
                    phHal_sNfcIPInfo_t       *p_remtgt_info = NULL;
