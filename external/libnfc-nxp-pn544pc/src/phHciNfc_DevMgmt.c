...
/* Multiple Secure Element AID Table
 * Configuration Commands
 * */
#define NXP_AID_READ            0x3AU
#define NXP_AID_WRITE           0x3BU
...
typedef enum phHciNfc_DevMgmt_Seq{
    ...
    DEV_MGMT_PL_PHASE,
    DEV_MGMT_PL_STATE,
    DEV_MGMT_HCI_INTERNAL_RF_STATE,
    DEV_MGMT_HW_RF_FIELD_STATE,
#endif /* #if (NXP_HAL_PL_STATE_DEBUG & 0x01U ) */
    DEV_MGMT_FELICA_RC,
                    ...
                    )
{
    ...
    else
    {
        ...
        else
        {
            ...
            if( NULL != p_config_val)
            {
                ((phHciNfc_DevMgmt_Info_t *)
                        psHciContext->p_device_mgmt_info)->p_val = p_config_val;
            }

            params[i++] = (uint8_t) (address >> ( BYTE_SIZE * 2 ));
            ...
            switch(p_device_mgmt_info->current_seq )
            {
                ...
                case DEV_MGMT_GPIO_PDIR:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_GPIO_PDIR , config , NULL );
                ...
                case DEV_MGMT_GPIO_PEN:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_GPIO_PEN , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                     DEV_MGMT_GET_EEPROM_INFO;
                ...
                case DEV_MGMT_FELICA_RC:
                {
                    config = 0x00;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                                 NFC_FELICA_RC_ADDR , config, NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                DEV_MGMT_GET_EEPROM_INFO;
                ...
                case DEV_MGMT_GET_EEPROM_INFO:
                {
                    ...
                    if(NFCSTATUS_PENDING == status )
                    {
#if  ( NXP_NFC_IFC_TIMEOUT & 0x01U )
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_IFC_TO_TX_H;
#else
                        p_device_mgmt_info->next_seq =
                                        DEV_MGMT_TX_LDO;
#endif /* #if  ( NXP_NFC_IFC_TIMEOUT & 0x01U ) */
                    }
                    break;
                }
#if  ( NXP_NFC_IFC_TIMEOUT & 0x01U )
                ...
                {
                    config =  (uint8_t)
                        ( NXP_NFC_IFC_TX_CONFIG_DEFAULT >> BYTE_SIZE ) /* 0x03 */;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_IFC_TO_RX_H , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_IFC_TO_RX_L;
                ...
                {
                    config = (uint8_t)
                        ( NXP_NFC_IFC_TX_CONFIG_DEFAULT & BYTE_MASK ) /* 0xE8 */;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_IFC_TO_RX_L , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_IFC_TO_TX_H;
                ...
                {
                    config =  (uint8_t)
                        ( NXP_NFC_IFC_CONFIG_DEFAULT >> BYTE_SIZE ) /* 0x20 */;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                        NFC_ADDRESS_IFC_TO_TX_H , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_IFC_TO_TX_L;
                ...
                {
                    config = (uint8_t)
                        ( NXP_NFC_IFC_CONFIG_DEFAULT & BYTE_MASK ) /* 0x3A */;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_IFC_TO_TX_L , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_TX_LDO;
                ...
                case DEV_MGMT_TX_LDO:
                {
...
#endif
                    {
                        ...
                        status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                                NFC_ADDRESS_HW_CONF , config , NULL );
                        if(NFCSTATUS_PENDING == status )
                        {
                ...
                case DEV_MGMT_ANAIRQ_CONF:
                {
                    config = 0x04;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_ANAIRQ_CONF , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_PMOS_MOD;
                ...
                case DEV_MGMT_PMOS_MOD:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_PMOS_MOD , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_CLK_REQ;
                ...
                case DEV_MGMT_CLK_REQ:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_CLK_REQ , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_INPUT_CLK;
                ...
                case DEV_MGMT_INPUT_CLK:
                {
                    ...
                    config = p_hw_config->input_clk;
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_CLK_INPUT , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                      DEV_MGMT_UICC_PWR_REQUEST;
                ...
                case DEV_MGMT_UICC_PWR_REQUEST:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_SWP_PWR_REQ , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_RD_A_ACCESS;
                ...
                case DEV_MGMT_UICC_RD_A_ACCESS:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_UICC_RD_A_ACCESS , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_RD_B_ACCESS;
                ...
                case DEV_MGMT_UICC_RD_B_ACCESS:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_UICC_RD_B_ACCESS , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_CE_A_ACCESS;
                ...
                case DEV_MGMT_UICC_CE_A_ACCESS:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_UICC_CE_A_ACCESS , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_CE_B_ACCESS;
                ...
                case DEV_MGMT_UICC_CE_B_ACCESS:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_UICC_CE_B_ACCESS , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_CE_BP_ACCESS;
                ...
                case DEV_MGMT_UICC_CE_BP_ACCESS:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_UICC_CE_BP_ACCESS , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_CE_F_ACCESS;
                ...
                case DEV_MGMT_UICC_CE_F_ACCESS:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_UICC_CE_F_ACCESS , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_CE_F_CLT_SUPPORT;
                ...
                case DEV_MGMT_UICC_CE_F_CLT_SUPPORT:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_UICC_CE_F_CLT_SUP , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_UICC_BIT_RATE;
                ...
                case DEV_MGMT_UICC_BIT_RATE:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_SWP_BITRATE , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                ...
                case DEV_MGMT_SET_PWR_STATUS:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_PWR_STATUS , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_SET_INFO_EVT;
                ...
                case DEV_MGMT_LLC_GRD_TO_H:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_LLC_GRD_TO_H , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_LLC_GRD_TO_L;
                ...
                case DEV_MGMT_LLC_GRD_TO_L:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_LLC_GRD_TO_L , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                ...
                case DEV_MGMT_LLC_ACK_TO_H:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_LLC_ACK_TO_H , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_LLC_ACK_TO_L;
                ...
                case DEV_MGMT_LLC_ACK_TO_L:
                {
                    ...
                    status = phHciNfc_DevMgmt_Configure( psHciContext, pHwRef,
                            NFC_ADDRESS_LLC_ACK_TO_L , config , NULL );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_EVT_AUTONOMOUS;
                         ...
                         )
{
    ...
    phHciNfc_Pipe_Info_t            *p_pipe_info = NULL;
    phHciNfc_DevMgmt_Info_t         *p_device_mgmt_info=NULL;
    uint8_t                         pipe_id = 0x00;
    uint8_t                         params[MAX_AID_LEN+2];
    ...
    else
    {
        ...
        {
            pipe_id = p_pipe_info->pipe.pipe_id ;
            index = p_device_mgmt_info->table_index;

            switch(config_type )
            {
                case NFC_SE_AID_CONFIG:
                {

#define POWER_AUTH_OFFSET 0x04U

                    uint8_t                      i = 0x00;
                    phHal_MultiEmulationCfg_t    *p_me_cfg_info =
                          ((phHal_MultiEmulationCfg_t *)config_table);
                    phHal_MultiEmulationAidCfg_t *p_aid_info = NULL;

                    if(index < p_me_cfg_info->numAidEntries )
                    {
                       params[i++] = index;
                    }

                    p_aid_info =  &p_me_cfg_info->aidList[index];

                    params[i++] = (uint8_t)( (((uint8_t) p_aid_info->aidLocation )
                                               & NXP_NFC_EMU_LOCATION_MASK )
                             | ( (((uint8_t) p_aid_info->powerState) << POWER_AUTH_OFFSET )
                                                   & NXP_NFC_POWER_AUTH_MASK) );
                    for( ; (i-2) < p_aid_info->aidLength; i++ )
                    {
                       params[i] = p_aid_info->aid[i-2];
                    }
                    p_pipe_info->param_info = &params ;
                    p_pipe_info->param_length = i ;
                    status = phHciNfc_Send_DevMgmt_Command( psHciContext, pHwRef,
                                                        pipe_id, (uint8_t)NXP_AID_WRITE );
                    if(NFCSTATUS_PENDING == status)
                    {
                        if (index < p_me_cfg_info->numAidEntries - 1)
                        {
                           p_device_mgmt_info->table_index++;
                        }
                        else
                        {
                           p_device_mgmt_info->table_index = 0x00U;
                           status = NFCSTATUS_SUCCESS;
                        }
                    }
                    break;
                }
                case NFC_HW_TABLE_CONFIG:
                {
                    phHal_HWTableConfig_t    *p_hw_cfg_info =
                          ((phHal_HWTableConfig_t *)config_table);

                    p_pipe_info->param_info = (p_hw_cfg_info->pAddrTable + index) ;
                    p_pipe_info->param_length = sizeof(phNfc_HWAddrCfg_t) ;
                    status = phHciNfc_Send_DevMgmt_Command( psHciContext, pHwRef,
                                                        pipe_id, (uint8_t)NXP_DBG_WRITE );
                    if(NFCSTATUS_PENDING == status)
                    {
                        if (index < p_hw_cfg_info->hwAddrEntries - 1)
                        {
                           p_device_mgmt_info->table_index++;
                        }
                        else
        ...
        } /* End of Null Pipe information Check */
                         ...
                         )
{
    ...
    PHNFC_UNUSED_VARIABLE(auto_type);
    if( ( NULL == psHciContext )
        || (NULL == pHwRef )
        )
    {
                         ...
                         )
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#if (NXP_HAL_PL_STATE_DEBUG & 0x01U )
                case DEV_MGMT_PL_PHASE:
                {
                    status = phHciNfc_DevMgmt_Get_Info( psHciContext, pHwRef,
                        NFC_ADDRESS_PL_PHASE , &p_device_mgmt_info->temp_store );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_PL_STATE;
                ...
                case DEV_MGMT_PL_STATE:
                {
                    status = phHciNfc_DevMgmt_Get_Info( psHciContext, pHwRef,
                        NFC_ADDRESS_PL_STATE , &p_device_mgmt_info->temp_store );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_HCI_INTERNAL_RF_STATE;
                ...
                case DEV_MGMT_HCI_INTERNAL_RF_STATE:
                {
                    status = phHciNfc_DevMgmt_Get_Info( psHciContext, pHwRef,
                        NFC_ADDRESS_HCI_INT_RF_STATE , &p_device_mgmt_info->temp_store );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_HW_RF_FIELD_STATE;
                ...
                case DEV_MGMT_HW_RF_FIELD_STATE:
                {
                    status = phHciNfc_DevMgmt_Get_Info( psHciContext, pHwRef,
                        NFC_ADDRESS_HW_RF_STATE , &p_device_mgmt_info->temp_store );
                    if(NFCSTATUS_PENDING == status )
                    {
                        p_device_mgmt_info->next_seq =
                                                DEV_MGMT_EVT_AUTONOMOUS;
                             ...
                             )
{
    ...
    else
    {
        ...
        switch(DevMgmt_seq)
        {
            ...
            {
#if (NXP_HAL_PL_STATE_DEBUG & 0x01U )
                p_device_mgmt_info->current_seq = DEV_MGMT_PL_PHASE ;
                p_device_mgmt_info->next_seq = DEV_MGMT_PL_PHASE ;
#endif /* #if (NXP_HAL_PL_STATE_DEBUG & 0x01U ) */
                break;
            }
            case REL_SEQ:
