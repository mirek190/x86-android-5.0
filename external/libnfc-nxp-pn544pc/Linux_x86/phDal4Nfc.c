...
#include "ext/src/tools/core/framework_Interface.h"
// END HECI Mod
...
/*-----------------------------------------------------------------------------------
                                       DAL MACROS
------------------------------------------------------------------------------------*/
#define NXP_NFC_DEFAULT_I2C_DEVICE_ADDRESS 0x57U
#ifndef NXP_NFC_I2C_DEVICE_ADDRESS
#define NXP_NFC_I2C_DEVICE_ADDRESS NXP_NFC_DEFAULT_I2C_DEVICE_ADDRESS
#endif
/*-----------------------------------------------------------------------------------
                                       TYPES
------------------------------------------------------------------------------------*/
...
//FA
// HECI Mod
static void *g_downloadContext = NULL;
// END HECI Mod

static phDal4Nfc_link_cbk_interface_t gLinkFunc;

static int nfc_hif_mode = NFC_HIF_MODE_OFF;
/*-----------------------------------------------------------------------------------
                                     PROTOTYPES
------------------------------------------------------------------------------------*/
...
NFCSTATUS phDal4Nfc_ConfigRelease(void *pHwRef)
{
   ...
   if (gDalContext.hw_valid == TRUE)
   {
      ...
      /* Close the link */
      //FA gLinkFunc.close();
      //FA release fake HW reference
       phOsalNfc_FreeMemory(pHwRef);
...
NFCSTATUS phDal4Nfc_Write( void *pContext, void *pHwRef,uint8_t *pBuffer, uint16_t length)
{
    ...
    result = NFCSTATUS_PENDING;
        ...
        if (g_downloadContext)
        {
            result = phDownloadWrapperCore_WritePacket(g_downloadContext,pBuffer,length,pHwRef);
        }else
        {
...
NFCSTATUS phDal4Nfc_Read( void *pContext, void *pHwRef,uint8_t *pBuffer, uint16_t length)
{
    ...
    if (g_downloadContext)
    {
       result = phDownloadWrapperCore_ReadPacket(g_downloadContext,pBuffer,length,pHwRef);
    }else
    {
       result = NFCSTATUS_FAILED;
    }
...
NFCSTATUS phDal4Nfc_Config(pphDal4Nfc_sConfig_t config,void **phwref)
{
   ...
   //FA fake HW reference useless with HECI driver but must not be NULL
   myphwref = phOsalNfc_GetMemory(sizeof(uint8_t));
   *phwref = myphwref;
   DAL_PRINT("Initialize the DAL context");
   ...
   phDal4Nfc_Reset (NFC_HIF_MODE_RST);
...
NFCSTATUS phDal4Nfc_Reset(long level)
{
   ...
   switch(level)
   {
        ...
        {
           //FA retstatus = gLinkFunc.reset(level);
            if (!g_downloadContext)
            {
                retstatus = phDownloadWrapperCore_InitDownloadMode(&g_downloadContext,pgDalContext,pgDalContext->pHwRef);
            }else
            {
...
#endif
        case NFC_HIF_MODE_RST:
        {
        //FA
            //retstatus = gLinkFunc.reset(NFC_HIF_MODE_ON);
            //retstatus = gLinkFunc.reset(NFC_HIF_MODE_OFF);
            //retstatus = gLinkFunc.reset(NFC_HIF_MODE_ON);
            //break;
            if (g_downloadContext)
            {
                retstatus = phDownloadWrapperCore_DeInitDownloadMode(g_downloadContext);
                g_downloadContext = NULL;
            }else
            {
                // Already in download mode
                retstatus = NFCSTATUS_SUCCESS;
        ...
        default:
        {
            DAL_DEBUG(" Invalid Reset Option: %ld",level);
   ...
   {
       nfc_hif_mode = (int) level;
   }

   DAL_DEBUG("<-- %s",__FUNCTION__);
   return retstatus;
}
...
int phDal4Nfc_ReaderThread(void * pArg)
{
    ...
    int i2c_error_count;
    int i2c_workaround;
    int i2c_device_address = NXP_NFC_DEFAULT_I2C_DEVICE_ADDRESS;
    if (gDalContext.pDev != NULL)
    {
        i2c_workaround = gDalContext.pDev->enable_i2c_workaround;
        if (NXP_NFC_I2C_DEVICE_ADDRESS)
        {
            i2c_device_address = NXP_NFC_I2C_DEVICE_ADDRESS;
        }
