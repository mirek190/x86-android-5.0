...
void framework_Log(const char* format, ...)
{
    ...
    __android_log_print(ANDROID_LOG_INFO,"HECI",format,varg);
    va_end(varg);
}
...
void framework_Warn(const char* format, ...)
{
    va_list varg;
    va_start(varg, format);
    __android_log_print(ANDROID_LOG_WARN,"HECI",format,varg);
    va_end(varg);
}
...
void framework_Error(const char* format, ...)
{
    va_list varg;
    va_start(varg, format);
    __android_log_print(ANDROID_LOG_ERROR,"HECI",format,varg);
    va_end(varg);
}
#endif
