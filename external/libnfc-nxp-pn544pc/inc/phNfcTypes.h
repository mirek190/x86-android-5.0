...
#define PHHAL_MAX_DATASIZE              0x100U       /* 256 * Maximum Data size sent
                                                     * by the HAL
                                                     */

#define NFC_APDU_HDR_LEN                0x03U      /*  ISO DEP Request/Response Header Length */

#define NFC_DEP_HDR_LEN                 0x03U       /* DEP Request/Response Header Length */
...
#define NXP_NFC_ROM_INFO(fw_ver)  (( (fw_ver & 0x00F00000) >>( (2*BYTE_SIZE) + 4)))

#define NFC44_ROM32_VER                 0x0AU
#define NFC44_ROM33_VER                 0x0BU

#define NXP_NFC_MAX_AID_SUPPORT         0x20U
#define NXP_NFC_MAX_PROTOCOL_SUPPORT    0x02U

#define NFC_RF_DISCOVERY_CONTINUE       0x00U
#define NFC_RF_DISCOVERY_REPOLL         0x01U
#define NFC_RF_DISCOVERY_HALT           0xFFU

#define NXP_NFC_DISCOVERY_NORMAL        0x00U
#define NXP_NFC_DISCOVERY_SPM           0x01U
#define NXP_NFC_DISCOVERY_DTA           0x0DU
#define NXP_NFC_DISCOVERY_INVALID       0xFFU

#if (NXP_NFC_ISO144434A_PPS_CONFIG >= 0x01)
#define NFC_PPS_DATARATE_106            0x00U
#define NFC_PPS_DATARATE_212            0x01U
#define NFC_PPS_DATARATE_424            0x02U
#define NFC_PPS_DATARATE_848            0x03U
#define NFC_PPS_DATARATE_INVALID        0xFFU
#endif
/*@}*/
...
typedef enum
{
   ...
   ENUM_LINK_TYPE_USB,
   ENUM_LINK_TYPE_TCP,
   ENUM_LINK_TYPE_COM10,
   ENUM_LINK_TYPE_COM11,
   ENUM_LINK_TYPE_COM12,
   ENUM_LINK_TYPE_COM13,
   ENUM_LINK_TYPE_COM14,
   ENUM_LINK_TYPE_COM15,
   ENUM_LINK_TYPE_COM16,
   ENUM_LINK_TYPE_COM17,
   ENUM_LINK_TYPE_COM18,
   ENUM_LINK_TYPE_COM19,
   ENUM_LINK_TYPE_COM20,
   ENUM_LINK_TYPE_COM21,
   ENUM_LINK_TYPE_COM22,
...
typedef struct phNfc_sSupProtocol_t
{
    ...
    unsigned int SPM       : 1;  /**< Tag Notification in Special Polling Mode. */
...
typedef struct phNfc_sDeviceCapabilities_t
{
    ...
    uint8_t                 full_version[NXP_FULL_VERSION_LEN+1];
...
typedef enum phNfc_eFelicaCmdList_t
{
    ...
    phNfc_eFelica_Update          = 0x01, /**< Felica Update command:\n
                                                 - This command updates the data onto the Felica
                                                  remote device */
    phNfc_eFelica_Polling         = 0x02, /**< Felica Polling command:\n
                                                 - This command sends the Polling request for Felica
                                                  remote device */
...
typedef struct phNfc_sKovioInfo_t
{
    uint8_t         Uid[16];
    uint8_t         UidLength;
} phNfc_sKovioInfo_t;
 ...
 *  \note
 *
 *  \if HAL \sa \ref phHal4Nfc_ConfigureDiscovery and \ref phHal4Nfc_Connect \endif
 *
 */
typedef struct phNfc_sRemoteDevInformation_t
{
    ...
    uint8_t                    ConnectWithPresence;    /**  [out]
                                                     *   This flag indicates if the connect
                                                     *   is performed by checking the presence
                                                     *   of the card in the field
                                                     *  */
...
typedef enum phNfc_eReleaseType
{
    ...
    NFC_SMARTMX_RELEASE,      /**< Release SmartMX from wired mode to previous mode
                                  (Virtual or Off) */
    NFC_SMARTMX_RELEASE_SPM /**< Release SmartMX from wired mode to Special Discovery mode
                                  (With SmartMX in Off Mode ) */
} phNfc_eReleaseType_t;

#define   NXP_NFC_POWER_AUTH_MASK                 0x30U
#define   NXP_NFC_EMU_LOCATION_MASK               0x07U
...
typedef struct phNfc_sADD_Cfg_t
{
    ...
    uint8_t                     NfcIP_Mode ;      /**< Select the P2P
                                                    ...
                                                    selected */
#if (NXP_NFC_TGT_SPEED_CONFIG >= 0x01)
    uint8_t                     NfcIP_Tgt_Mode_Config; /**< Select the P2P
                                                        * speeds using phNfc_eP2PMode_t type for
                                                        * Target mode Support. */
#endif
    uint8_t                     NfcIP_Tgt_Disable;   /**< Flag to
                                                       disable the NFCIP1 TARGET */

#if (NXP_NFC_POLL_AUTONOMOUS >= 0x01)
    uint8_t                     Autonomous_Emulation; /**< Flag to disable any Notification to the Host
                                                      * Until Host is interested */
#endif

#if (NXP_NFC_POLL_FELICA_SYSCODE >= 0x01)

    uint16_t                    FelicaPollSystemCode;   /**< Felica System Code
                                                      to be polled by the Reader */
#endif
