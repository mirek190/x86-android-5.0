 ...
 *
 * \if hal
 *  \brief Information related to Secure Element and
 *         the Secure Element interface of the NFC Device
 * \else
 *  \brief HAL-Specific
 * \endif
 *
 *  The <em> Secure Element Information Structure </em> holds information
 *  related to the Secure Element interface and the Secure Elements of the NFC IC
 *  obtained during initialization time.
 *
 */

typedef struct phHal_sSecureElementInfo
{
    /**<  SWP Interface Line HIZ State TRUE/FALSE */
    uint8_t                 swp_hiz_state;
    /**<  SWP Interface Line HIZ State TRUE/FALSE */
    uint8_t                 swp_hiz_io;
    /**<  SWP Interface status TRUE/FALSE */
    uint8_t                 swp_link_status;
    /**<  WI Interface status TRUE/FALSE */
    uint8_t                 wi_link_status;

}phHal_sSecureElementInfo_t;
...
typedef struct phHal_sHwReference
{
    ...
    /**<  Secure Element and its Interface information. */
    phHal_sSecureElementInfo_t      se_if_info;
 ...
 *
 * \if hal
 * \brief EEProm configuration - EEPROM Configuration Setting for NFC Platform
 * \else
 * \brief HAL-Specific
 * \endif
 *
 *  This structure is used internally by the HAL implementation and is filled
 *  up based on required eeprom configuration settings from a configuration file
 * \note None.
 *
 */

typedef struct  phNfc_HWAddrCfg
{
#define NXP_NFC_EE_ADDR_LEN  0x03U

    uint8_t         ee_addr[NXP_NFC_EE_ADDR_LEN];
 ...
 *
 * \if hal
 * \brief Hardware configuration Table - Configuration Parameters Table for the NFC Device
 * \else
 * \brief HAL-Specific
 * \endif
 *
 *  This structure is used internally by the HAL implementation and is filled
 *  up based on various configuration parameters
 * \note None.
 *
 */
typedef struct phHal_HWTableConfig
{
    phNfc_HWAddrCfg_t *pAddrTable;
    uint16_t           hwAddrEntries;
}phHal_HWTableConfig_t;
...
typedef struct phHal_sHwConfig
{
    ...
    phHal_HWTableConfig_t   addr_table; /**<   NFC EEPROM Configuration Setting */
...
#define    phHal_eIso14443_3B_Raw   phNfc_eIso14443_3B_Raw
...
typedef phNfc_sKovioInfo_t  phHal_sIsoKovioInfo_t;
/** \ingroup grp_hal_nfci
*
* \if hal
...
* \endif
*
*  The <em> \ref phHalNfc_eDataRate_t enum </em> lists all the Data Rate
...
#define    phHal_eISO14443_3B_PICC phNfc_eISO14443_3B_PICC
...
typedef enum phHal_Event {
    ...
    /* NXP EVENTS */
    NFC_EVT_MIFARE_ACCESS          = 0x35U,
    NFC_EVT_APDU_RECEIVED          = 0x36U,
    NFC_EVT_EMV_CARD_REMOVAL       = 0x37U,
    NFC_EVT_ESE_WTX_EVENT          = 0x38U
...
* \endif
*
*  The <em> \ref phHal_sNfcIPCfg_t </em> holds the P2P related information
...
typedef enum phHal_eConfigType
{
    ...
    NFC_SE_AID_CONFIG, /** < SE AID Configuration */
    NFC_HW_TABLE_CONFIG /** < HW Table Configuration */
}phHal_eConfigType_t;
...
*
* \if hal
*  \brief Information for the Configuring the Multiple Emulation based on AID
* \else
*  \brief HAL-Specific
* \endif
*
*\brief This Structure <em> \ref phHal_MultiEmulationAidCfg </em>  defines
* Multi Emulation AID Table Configuration Type.
*/

typedef struct phHal_MultiEmulationAidCfg
{
    phNfc_eEmuLocation_t   aidLocation;
    phNfc_ePowerState_t    powerState;
    uint8_t                aid[MAX_AID_LEN];
    uint8_t                aidLength;
} phHal_MultiEmulationAidCfg_t;

/**
* \ingroup grp_hal_common
*
* \if hal
*  \brief Information for the Configuring the Multiple Emulation based on Protocol
* \else
*  \brief HAL-Specific
* \endif
*
*\brief This Structure <em> \ref phHal_MultiEmulationAidCfg </em>  defines
* Multi Emulation Protocol Table Configuration Type.
*/

typedef struct phHal_MultiEmulationProtocolCfg
{
    phNfc_eEmuLocation_t   location;
    phNfc_ePowerState_t    powerState;
    phNfc_eEmuProtocol_t   protocolType;
} phHal_MultiEmulationProtocolCfg_t;

/**
* \ingroup grp_hal_common
*
* \if hal
*  \brief Information for the Configuring the Multiple Emulation
* \else
*  \brief HAL-Specific
* \endif
*
*\brief This Structure <em> \ref phHal_MultiEmulationCfg </em>  defines
* Multi Emulation attributes to be Configured.
*/

typedef struct phHal_MultiEmulationCfg
{
    uint8_t                           multiEmulationEnable;
    uint8_t                           numAidEntries;
    phHal_MultiEmulationAidCfg_t      aidList[NXP_NFC_MAX_AID_SUPPORT];
    uint8_t                           numProtocolEntries;
    phHal_MultiEmulationProtocolCfg_t protocolInfo[NXP_NFC_MAX_PROTOCOL_SUPPORT];
    phNfc_eEmuLocation_t              defaultRoute;
} phHal_MultiEmulationCfg_t;
...
*            \ref phNfc_eP2PMode_t and \ref phHal4Nfc_Disconnect.
*  \endif
*
*/
...
typedef union phHal_uConfig
{
    ...
    phHal_MultiEmulationCfg_t multiEmuConfig;
    phHal_HWTableConfig_t     hwTableConfig;
}phHal_uConfig_t;
