...
unsigned int wps_generate_pin(void)
{
	unsigned int val;
#ifdef ANDROID
	char sigma_wps_pin[PROPERTY_VALUE_MAX];

	if (property_get("sigma.wps_pin", sigma_wps_pin, NULL) != 0) {
		val = atoi(sigma_wps_pin);
		wpa_printf(MSG_DEBUG, "WPS: Retrieved sigma.wps_pin property: %d", val);
		return val;
	}
#endif /* ANDROID */
