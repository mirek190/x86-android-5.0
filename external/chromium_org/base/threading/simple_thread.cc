...
void SimpleThread::ThreadMain() {
  LOG(INFO) << "SimpleThread ThreadMain";
...
void DelegateSimpleThread::Run() {
   ...
   if (!delegate_) {
       LOG(INFO) << "delegate_ is NULL";
   } else {
       LOG(INFO) << "delegate_ is not NULL";
       delegate_->Run();
       delegate_ = NULL;
   }
}
...
void DelegateSimpleThreadPool::Start() {
  ...
  LOG(INFO) << "DelegateSimpleThreadPool Start";
...
void DelegateSimpleThreadPool::AddWork(Delegate* delegate, int repeat_count) {
  LOG(INFO) << "DelegateSimpleThreadPool AddWork";
...
void DelegateSimpleThreadPool::Run() {
  LOG(INFO) << "DelegateSimpleThreadPool Run";
  Delegate* work = NULL;
